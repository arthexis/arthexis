...
def test_staff_task_resolves_named_internal_action_for_visible_tasks():
    ...
    assert any(
        task["slug"] == "release" and task["url"] == "/admin/release/packagerelease/"
        for task in tasks
    )
...
@pytest.mark.django_db
def test_issues_staff_task_links_to_local_feedback_queue_for_authorized_staff():
    user_model = get_user_model()
    user = user_model.objects.create_user(
        username="issues-staff",
        password="secret123",
        is_staff=True,
    )
    permission = Permission.objects.get(
        codename="view_userstory",
        content_type=ContentType.objects.get(app_label="pages", model="userstory"),
    )
...
@pytest.mark.django_db
def test_issues_staff_task_hides_without_feedback_view_permission():
    user_model = get_user_model()
    user = user_model.objects.create_user(
        username="issues-hidden",
        password="secret123",
        is_staff=True,
    )
    ...
    assert all(task["slug"] != "issues" for task in tasks)
