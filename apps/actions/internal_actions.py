...
class InternalActionSpec:
    ...
    Parameters:
        ...
        query_string: Optional query string appended to the resolved URL.
...
INTERNAL_ACTION_SPECS: tuple[InternalActionSpec, ...] = (
    ...
    InternalActionSpec(
        name="issues",
        label="Issues",
        description="Review locally queued feedback issues.",
        admin_url_name="admin:pages_userstory_changelist",
        query_string="issue_destination__exact=local&status__exact=open",
    ),
    InternalActionSpec(
        name="logs",
        label="Logs",
...
def resolve_internal_action_url(action_name: str) -> str:
    ...
    if spec is None:
        return ""
    try:
        url = reverse(spec.admin_url_name)
    except NoReverseMatch:
        return ""
    if spec.query_string:
        separator = "&" if "?" in url else "?"
        return f"{url}{separator}{spec.query_string}"
    return url
