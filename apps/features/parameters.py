...
FEATURE_PARAMETER_DEFINITIONS: dict[str, tuple[FeatureParameterDefinition, ...]] = {
    ...
    "llm-summary-suite": (
        ...
        FeatureParameterDefinition(
            key="enabled_sources",
            label=_("Enabled summary sources"),
            help_text=_(
                "Comma-separated source groups for LCD summaries: logs, state, "
                "journal, or all."
            ),
            default="logs,state,journal",
        ),
        FeatureParameterDefinition(
    ...
    "odoo-crm-sync": (
        FeatureParameterDefinition(
            ...
            help_text=_("Enable or disable discovery sync of local Odoo deployments."),
            choices=(("enabled", _("Enabled")), ("disabled", _("Disabled"))),
            default="enabled",
        ),
        FeatureParameterDefinition(
...
def get_feature_parameter_definitions(
    slug: str,
) -> tuple[FeatureParameterDefinition, ...]:
...
def get_feature_parameter(slug: str, key: str, *, fallback: str = "") -> str:
    ...
    raw_value = (
        get_feature_parameter_value(feature, key, default=definition.default)
        if feature
