...
def get_cached_feature_enabled(
    ...
    *,
    ...
    cache_key = _feature_cache_key_for_current_connection(cache_key)
...
def clear_feature_cache(cache_key: str) -> None:
    """Clear a feature cache entry using the current connection's key scope."""

    cache.delete(_feature_cache_key_for_current_connection(cache_key))


def _feature_cache_key_for_current_connection(cache_key: str) -> str:
    """Scope feature-cache keys to the active database when running tests."""

    database_name = str(connection.settings_dict.get("NAME") or "")
    configured_test_name = str(
        connection.settings_dict.get("TEST", {}).get("NAME") or ""
    )
    if not (
        database_name.startswith("test")
        or database_name == configured_test_name
        or database_name.rsplit("/", maxsplit=1)[-1].startswith("test")
    ):
        return cache_key
    return f"{cache_key}:{connection.alias}:{database_name}"
