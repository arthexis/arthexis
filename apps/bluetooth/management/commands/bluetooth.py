...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--enable", action="store_true", help="Enable Bluetooth adapter power.")
        parser.add_argument("--disable", action="store_true", help="Disable Bluetooth adapter power.")
        parser.add_argument("--discover", action="store_true", help="Run discovery and sync known devices.")
        parser.add_argument("--timeout", type=int, default=4, help="Discovery timeout in seconds.")
        parser.add_argument("--register", metavar="ADDRESS", help="Mark a device as registered by address.")
        parser.add_argument("--unregister", metavar="ADDRESS", help="Mark a device as unregistered by address.")
        parser.add_argument("--list", action="store_true", help="List known devices from the database.")
    ...
    def handle(self, *args, **options):
        try:
            ...
            if options["register"]:
                ...
                self.stdout.write(self.style.SUCCESS(f"Registered device {device.address}"))

            if options["unregister"]:
                device = BluetoothDevice.objects.get(address=options["unregister"])
                ...
                self.stdout.write(self.style.SUCCESS(f"Unregistered device {device.address}"))
