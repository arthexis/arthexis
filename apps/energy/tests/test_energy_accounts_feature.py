...
import apps.energy.models.billing as billing_models
...
def test_customer_account_save_tolerates_odoo_disabled(monkeypatch):
    monkeypatch.setattr(billing_models, "ODOO_APP_INSTALLED", False)
    monkeypatch.setattr(Entity, "save", lambda self, *args, **kwargs: None)

    account = CustomerAccount(name="mixed-case")

    account.save()

    assert account.name == "MIXED-CASE"
    assert account.live_subscription_start_date is None
    assert account.live_subscription_next_renewal is None
