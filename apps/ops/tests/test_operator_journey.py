...
class OperatorJourneyFlowTests(TestCase):
    ...
    def test_product_developer_progression_starts_at_issue_inbox_without_token_setup(
        self,
    ):
        github_journey = OperatorJourney.objects.create(
            name="Product Developer GitHub Access",
    ...
    @patch("apps.ops.operator_journey.django_apps.is_installed")
    def test_setup_github_token_step_auto_completes_without_repos_app(
        self,
        mock_is_installed,
    ):
        mock_is_installed.side_effect = lambda app_label: app_label != "apps.repos"
...
class OperatorJourneyViewTests(TestCase):
    ...
    def _permissions_for_names(self, permission_names):
        permission_filters = [
            permission_name.split(".", maxsplit=1)
        ...
        return [
            ...
            for app_label, codename in permission_filters
        ]

    def test_admin_dashboard_surfaces_operator_tool_buttons(self):
        response = self.client.get(reverse("admin:index"))

        self.assertContains(response, "Journey")
        self.assertContains(response, reverse("ops:operator-cards"))
        self.assertContains(response, "Cards")

    def test_admin_dashboard_hides_operator_tool_buttons_without_model_permissions(
        self,
    ):
        limited_user = get_user_model().objects.create_user(
            username="ops-journey-limited-dashboard",
            password="x",
            is_staff=True,
        )
        ...
        self.assertContains(response, "Journey")
        self.assertNotContains(response, reverse("ops:operator-cards"))

    def test_admin_dashboard_hides_uninstalled_operator_tool_buttons(self):
        request = RequestFactory().get("/admin/")
        request.user = self.user

        nav_urls = {item["url"] for item in _operator_tool_nav(request)}

        self.assertIn(reverse("ops:operator-journey-dashboard"), nav_urls)
        self.assertIn(reverse("ops:operator-cards"), nav_urls)

    def test_operator_tool_views_require_model_permissions(self):
        limited_user = get_user_model().objects.create_user(
            username="ops-journey-limited-tools",
            password="x",
            is_staff=True,
        )
        limited_user.groups.add(self.group)
    ...
    def test_operator_tool_views_render_task_context(self):
        expected = {
            "ops:operator-cards": "Review RFID identity state",
        }

        for url_name, summary in expected.items():
            with self.subTest(url_name=url_name):
                response = self.client.get(reverse(url_name))

                self.assertEqual(response.status_code, 200)
                self.assertContains(response, "Operator tasks")
    ...
    def test_setup_github_token_validation_uses_stored_token(
        ...
        mock_resolve_sigils.assert_called_once_with("literal-token")
