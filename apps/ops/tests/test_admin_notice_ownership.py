...
def test_core_admin_notice_import_is_compatibility_alias():
    ...
    assert importlib.import_module(
        "apps.core.models.admin_notice"
    ) is importlib.import_module("apps.ops.admin_notice")


def test_admin_notice_admin_is_registered_by_ops():
    assert isinstance(admin.site._registry[AdminNotice], AdminNoticeAdmin)
    assert (
        type(admin.site._registry[AdminNotice]).__module__
        == "apps.ops.admin_notice_admin"
    )
...
def test_admin_notice_content_type_uses_ops_owner():
    ...
    assert not ContentType.objects.filter(
        app_label="core", model="adminnotice"
    ).exists()
