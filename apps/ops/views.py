...
from apps.cards.models import RFID, CardFace, RFIDAttempt
...
def _reverse_optional_url(viewname: str, *args, **kwargs) -> str | None:
    try:
...
def _can_manage_github_token(request: HttpRequest, token=None) -> bool:
    ...
    if not django_apps.is_installed(REPOS_APP_SELECTOR):
        return False
...
OPERATOR_CARD_PERMISSIONS = (
    "cards.view_cardface",
    "cards.view_rfid",
    "cards.view_rfidattempt",
)


def _request_user_has_perms(
    request: HttpRequest | None, permissions: tuple[str, ...]
) -> bool:
    user = getattr(request, "user", None)
    has_perm = getattr(user, "has_perm", None)
    if not callable(has_perm):
        return False
    return all(has_perm(permission) for permission in permissions)


def _require_operator_tool_permissions(
    request: HttpRequest | None, permissions: tuple[str, ...]
) -> None:
    if not _request_user_has_perms(request, permissions):
        raise PermissionDenied


def _operator_tool_nav(request: HttpRequest) -> list[dict[str, str]]:
    """Return operator tool shortcuts the current user may view."""

    nav_specs = [
        (
            _("Journey"),
            reverse("ops:operator-journey-dashboard"),
            _("Active onboarding and operational journey tasks."),
            (),
        ),
    ]
    nav_specs.append(
        (
            _("Cards"),
            reverse("ops:operator-cards"),
            _("RFID identities, scan activity, and card artwork."),
            OPERATOR_CARD_PERMISSIONS,
        )
    )
    return [
        {"description": description, "label": label, "url": url}
        for label, url, description, permissions in nav_specs
        if not permissions or _request_user_has_perms(request, permissions)
    ]
...
def _operator_tool_context(
    ...
    *,
    ...
    """Build shared context for lightweight operator tool views."""
    ...
    return {
        **_build_admin_context(request),
        ...
        "summary": summary,
        "title": title,
        "tool_nav": _operator_tool_nav(request),
    }


@staff_member_required
def operator_cards(request: HttpRequest) -> HttpResponse:
    """Render a staff card operations overview."""

    _require_operator_tool_permissions(request, OPERATOR_CARD_PERMISSIONS)
    recent_attempts = RFIDAttempt.objects.order_by("-attempted_at", "-id")[:8]
    recent_rfids = RFID.objects.order_by("-label_id")[:8]
    metrics = [
        {"label": _("RFIDs"), "value": RFID.objects.count()},
        {"label": _("Scan attempts"), "value": RFIDAttempt.objects.count()},
        {"label": _("Card faces"), "value": CardFace.objects.count()},
    ]
    actions = [
        {"label": _("Open card scanner"), "url": reverse("rfid-reader")},
        {"label": _("Manage RFIDs"), "url": reverse("admin:cards_rfid_changelist")},
        {
            "label": _("Manage card faces"),
            "url": reverse("admin:cards_cardface_changelist"),
        },
    ]
    context = _operator_tool_context(
        request,
        title=_("Cards"),
        summary=_(
            "Review RFID identity state and scanner entry points for card-related operator work."
        ),
        metrics=metrics,
        actions=actions,
        operator_tasks=[
            _("Scan or import RFIDs when onboarding cards for users and chargers."),
            _(
                "Review recent attempts when a physical reader or card fails validation."
            ),
            _("Review card faces before production or field handoff."),
        ],
    )
    context.update({"recent_attempts": recent_attempts, "recent_rfids": recent_rfids})
    return render(request, "admin/ops/operator_tool.html", context)
