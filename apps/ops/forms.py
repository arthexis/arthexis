...
class OperatorJourneyProvisionSuperuserForm(forms.Form):
    ...
    def save(self):
        ...
        if github_token:
            github_token_model = _get_github_token_model()
            if github_token_model is None:
                raise ValidationError("GitHub token storage requires the Repos app.")
            github_token_model.objects.update_or_create(
                user=user,
                defaults={
...
class OperatorJourneyGitHubAccessForm(forms.Form):
    ...
    def __init__(self, *args, user, **kwargs):
        ...
        github_token_model = _get_github_token_model()
        if github_token_model is None:
            self._existing_token_record = None
        else:
            self._existing_token_record = (
                github_token_model.objects.filter(user=user, group__isnull=True)
                .order_by("-pk")
                .first()
            )
    ...
    def save(self, *, token: str, username: str) -> GitHubToken:
        ...
        token_record, _created = github_token_model.objects.update_or_create(
            user=self.user,
            group=None,
            defaults=defaults,
        )
