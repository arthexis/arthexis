...
class RaspberryPiImageBuildForm(forms.Form):
    ...
    git_url = forms.CharField(
        max_length=500,
        required=False,
        initial=DEFAULT_ARTHEXIS_GIT_URL,
        help_text=_(
            "Optional authenticated repository cloned on first boot if the bundled suite is unavailable."
        ),
    )
...
@admin.register(RaspberryPiImageBurnJob)
class RaspberryPiImageBurnJobAdmin(admin.ModelAdmin):
    """Admin visibility for durable SD-card burn jobs."""
    ...
    list_filter = ("status", "backup", "created_at", "finished_at")
    search_fields = (
        "uuid",
        "artifact_name",
        "image_path",
    ...
    def has_add_permission(self, request: HttpRequest) -> bool:
        """Burn jobs are queued through the guarded management command."""

        return False

    @admin.display(description=_("Progress"))
    def progress_display(self, obj: RaspberryPiImageBurnJob) -> str:
        """Show compact durable progress in the read-only job list."""
