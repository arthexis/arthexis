...
@pytest.mark.django_db
def test_execute_script_allows_unresolved_sigil_token(monkeypatch):
    monkeypatch.setattr(
        "apps.sigils.script_runtime.resolve_sigils",
    ...
    assert outputs == ["[CP:hostname=SIM-CP-1.public_endpoint]"]


@pytest.mark.django_db
def test_execute_script_raises_policy_error_for_unresolved_user_context(monkeypatch):
    monkeypatch.setattr(
        "apps.sigils.script_runtime.resolve_sigils",
        lambda *args, **kwargs: "[CP:hostname=SIM-CP-1.public_endpoint]",
    )

    with pytest.raises(ScriptPolicyError, match="policy error"):
        execute_script(parse_script("EMIT anything"), context="user")
