...
def parse_aggregate_request(
    ...
    if (
        ...
        return None, None
    aggregate_target, aggregate_func = instance_id.rsplit(":", 1)
...
def resolve_entity_aggregate(
    ...
    raw_target_name = aggregate_target or ""
    all_separator = ""
    if aggregate_func == "all" and ":" in raw_target_name:
        raw_target_name, all_separator = raw_target_name.rsplit(":", 1)
    target_name = raw_target_name.replace("-", "_")
    ...
    for obj in qs.iterator():
        ...
        if source is None:
            continue
        result_values.append(source)
        source_text = str(source).strip()
        if source_text:
            text_values.append(source_text)
        numeric = _coerce_numeric(source)
        if numeric is not None:
            values.append(numeric)

    if aggregate_func == "any":
        return str(bool(text_values))
    if aggregate_func == "first":
        return text_values[0] if text_values else ""
    if aggregate_func == "last":
        return text_values[-1] if text_values else ""
    if aggregate_func == "random":
        return random.choice(text_values) if text_values else ""
    if aggregate_func == "all":
        return all_separator.join(text_values)
    if aggregate_func == "json":
        return json.dumps({"results": result_values}, cls=SafeDjangoJSONEncoder)
    if aggregate_func == "jsonl":
        return "\n".join(
            json.dumps({"result": value}, cls=SafeDjangoJSONEncoder)
            for value in result_values
        )
    if aggregate_func == "sep":
