...
PIPELINE_WINDOW_ACTIONS = frozenset(
    {
        "ALL",
        "ANY",
        "COUNT",
...
WHITESPACE_OPERATOR_DISALLOWED_CHARS = frozenset({'"', "[", "]", ":", "=", "|", ";"})
WHITESPACE_OPERATOR_OPERAND_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]*$")
...
@dataclass(frozen=True)
class WhitespaceOperatorCandidate:
    """Two clean operands separated by whitespace inside one sigil branch."""
...
def _normalize_identifier_whitespace(value: str | None) -> str | None:
    """Normalize identifier whitespace so internal gaps are treated as underscores."""
    if value is None:
        return None
...
def _parse_root_name(token: str, index: int) -> tuple[str, int]:
    ...
    while index < len(token):
        if token[index] in ":=.":
            break
        index += 1
    root_name = _normalize_identifier_whitespace(token[start:index])
    if not root_name:
...
def _parse_filter_field(token: str, index: int) -> tuple[str | None, int]:
    ...
    filter_field = _normalize_identifier_whitespace(token[start:index])
    return filter_field.replace("-", "_"), index
...
def _parse_quoted_segment(
    ...
    if quoted and len(value) >= 2 and value[0] == value[-1] == '"':
        value = value[1:-1]
    if not quoted:
        value = _normalize_identifier_whitespace(value)
    return value, index


def _parse_key_segment(token: str, index: int) -> tuple[str | None, int]:
    """Parse an optional ``.key`` segment.
    ...
    Returns:
        A tuple containing the parsed key and the next unread offset.
    """
    if index >= len(token) or token[index] != ".":
        return None, index
    index += 1
...
def _parse_key_and_param(
    ...
    Raises:
        TokenParseError: This helper does not currently raise parsing errors.
    """
    key, index = _parse_key_segment(token, index)

    param = None
    if index < len(token) and token[index] == "=":
        param = token[index + 1 :]
        index = len(token)
    param = _normalize_identifier_whitespace(param)
...
def _normalize_pipeline_segment(segment: str) -> str:
    collapsed = re.sub(r"\s*:\s*", ":", segment.strip())
    collapsed = _normalize_identifier_whitespace(collapsed)
...
def _is_whitespace_operator_operand(value: str) -> bool:
    return bool(WHITESPACE_OPERATOR_OPERAND_RE.fullmatch(value))


def _parse_whitespace_operator_candidate(
    token: str,
) -> WhitespaceOperatorCandidate | None:
    stripped = token.strip()
    if (
        not stripped
        or "->" in stripped
        or any(char in stripped for char in WHITESPACE_OPERATOR_DISALLOWED_CHARS)
    ):
        return None
...
def _pipeline_token_for_whitespace_action(
    selector: str,
    action: str,
) -> str | None:
    action_upper = action.upper()
    if action_upper == "FILTER" or action_upper not in PIPELINE_WINDOW_ACTIONS:
        return None
    root_name, separator, target = selector.partition(".")
    if not root_name or not _is_valid_pipeline_head(root_name):
        return None
    action_payload = target if separator else ""
    return f"{root_name}:|{action_upper}:{action_payload}"
...
def _parse_pipeline_token_parts(token: str) -> tuple[SigilTokenParts, str | None]:
    ...
    left_segment, right_segment = token.split("|", 1)
    normalized_left_segment = _normalize_pipeline_segment(left_segment)
    normalized_right_segment = _normalize_pipeline_segment(right_segment)
    left_name, left_separator, left_payload = normalized_left_segment.partition(":")
    if not left_separator:
        left_name = normalized_left_segment
        left_payload = ""
    right_name, right_separator, right_payload = normalized_right_segment.partition(":")
    if not right_separator:
        right_name = normalized_right_segment
        right_payload = ""
    left_is_action = _is_pipeline_action_name(left_name)
    right_is_action = _is_pipeline_action_name(right_name)

    root_name = left_name
    root_payload = left_payload
    action_name = right_name
    action_payload = right_payload
    ...
    return (
        SigilTokenParts(root_name, filter_field, instance_id, key, param),
        action_upper,
    )
...
def _request_key_is_known(key: str | None) -> bool:
    return (key or "").lower() in {
        "absolute_uri",
        "cookie",
        "cookies",
        "full_path",
...
def _resolve_token_with_policy(
    ...
    fallback_segments = _split_token_fallback_segments(token)
    if len(fallback_segments) > 1:
        for segment in fallback_segments[:-1]:
            resolved = _resolve_single_token_with_policy(
                segment,
                current=current,
                allowed_roots=allowed_roots,
                allowed_actions=allowed_actions,
            )
            if resolved and resolved != _failed_resolution(segment):
                return resolved
        last_segment = fallback_segments[-1]
        last_resolved = _resolve_single_token_with_policy(
            fallback_segments[-1],
            current=current,
            allowed_roots=allowed_roots,
            allowed_actions=allowed_actions,
        )
        if last_resolved == _failed_resolution(last_segment):
            return _failed_resolution(token)
        return last_resolved

    return _resolve_single_token_with_policy(
        token,
        current=current,
        allowed_roots=allowed_roots,
        allowed_actions=allowed_actions,
...
def _split_token_fallback_segments(token: str) -> list[str]:
    segments: list[str] = []
    start = 0
    depth = 0
    in_quotes = False
    saw_top_level_dot = False
    ...
    while index < len(token):
        ...
        if char == '"':
            ...
            if char == "[":
                ...
                has_param_payload = False
        index += 1
    segments.append(token[start:].strip())
    return [segment for segment in segments if segment]


def _looks_like_fallback_branch_start(remaining: str) -> bool:
    ...
    return root.upper() == root


def _resolve_whitespace_operator_candidate(
    token: str,
    candidate: WhitespaceOperatorCandidate,
    current: models.Model | None = None,
    allowed_roots: set[str] | None = None,
    allowed_actions: set[str] | None = None,
) -> str:
    joined_token = _normalize_identifier_whitespace(token.strip())
    if joined_token and joined_token != token.strip():
        joined_resolved = _resolve_single_token_with_policy(
            joined_token,
            current=current,
            allowed_roots=allowed_roots,
            allowed_actions=allowed_actions,
            allow_whitespace_operator=False,
        )
    ...
    left_action = (
        candidate.left.upper() if _is_pipeline_action_name(candidate.left) else None
    )
    right_action = (
        candidate.right.upper() if _is_pipeline_action_name(candidate.right) else None
    )
    if left_action or right_action:
        if left_action and right_action:
            return _failed_resolution(token)
        action = left_action or right_action or ""
        selector = candidate.right if left_action else candidate.left
        if not _is_pipeline_v2_enabled():
            return _failed_resolution(token)
        if allowed_actions is not None and action not in allowed_actions:
            return _failed_resolution(token)
        pipeline_token = _pipeline_token_for_whitespace_action(selector, action)
        ...
        if resolved == _failed_resolution(pipeline_token):
            return _failed_resolution(token)
        return resolved

    first_resolved = _resolve_single_token_with_policy(
        candidate.left,
        current=current,
        allowed_roots=allowed_roots,
        allowed_actions=allowed_actions,
    ...
    if first_resolved and first_resolved != _failed_resolution(candidate.left):
    ...
    if last_resolved == _failed_resolution(candidate.right):
        return _failed_resolution(token)
    return last_resolved


def _empty_joined_resolution_is_specific(
    joined_token: str,
    current: models.Model | None = None,
    allowed_roots: set[str] | None = None,
) -> bool:
    ...
    if root is None:
        return False
    if root.context_type == SigilRoot.Context.ENTITY:
        return True
    if root.context_type == SigilRoot.Context.CONFIG:
        return _empty_config_resolution_is_specific(root, context)
    if root.context_type == SigilRoot.Context.REQUEST:
        return _request_key_is_known(context.normalized_key)
    return False


def _empty_config_resolution_is_specific(
    root: SigilRoot,
    context: ResolutionContext,
) -> bool:
    prefix = root.prefix.upper()
    if prefix == "ENV":
        candidates = [
            candidate
            for candidate in [context.key_upper, (context.raw_key or "").upper()]
            if candidate
        ]
        return any(candidate in os.environ for candidate in candidates)
    if prefix == "CONF":
        return any(
            hasattr(settings, candidate)
            for candidate in [
                context.normalized_key,
                context.key_upper,
                context.key_lower,
    ...
    if prefix == "SYS":
        ...
        return any(
            candidate in values or resolve_system_namespace_value(candidate) is not None
            for candidate in candidates
        )
    return False


def _resolve_single_token_with_policy(
    token: str,
    current: models.Model | None = None,
    allowed_roots: set[str] | None = None,
    allowed_actions: set[str] | None = None,
    allow_whitespace_operator: bool = True,
) -> str:
    """Resolve a single token branch while enforcing an optional root allow-list."""
    if allow_whitespace_operator:
        whitespace_candidate = _parse_whitespace_operator_candidate(token)
        if whitespace_candidate is not None:
            return _resolve_whitespace_operator_candidate(
                token,
