...
GITHUB_REPO_DOC_LINK_PATTERN = re.compile(
    r"(?P<prefix><a\b[^>]*\bhref=[\"\'])"
    r"https://github\.com/arthexis/arthexis/blob/main/"
    r"(?P<path>(?:apps/docs|docs)/[^\"']+)"
    r"(?P<suffix>[\"\'])",
    re.IGNORECASE,
)
...
def rewrite_github_doc_links(html: str) -> str:
    """Rewrite same-repository GitHub docs links to local docs routes."""

    def _replace(match: re.Match[str]) -> str:
        raw_path = match.group("path").lstrip("/")
        doc_path, separator, fragment = raw_path.partition("#")
        normalized_doc_path = doc_path.replace("\\", "/").strip("/")
        if normalized_doc_path.startswith("apps/docs/"):
            route_name = "docs:apps-docs-document"
            route_doc = normalized_doc_path.removeprefix("apps/docs/").lstrip("/")
        elif normalized_doc_path.startswith("docs/"):
            route_name = "docs:docs-document"
            route_doc = normalized_doc_path.removeprefix("docs/").lstrip("/")
        else:
            return match.group(0)
        if not route_doc:
            return match.group(0)
        try:
            local_url = reverse(route_name, args=[route_doc])
        except NoReverseMatch:
            return match.group(0)
        if separator:
            local_url = f"{local_url}#{fragment}"
        return f"{match.group('prefix')}{escape(local_url)}{match.group('suffix')}"

    return GITHUB_REPO_DOC_LINK_PATTERN.sub(_replace, html)
