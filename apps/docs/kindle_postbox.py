...
def _read_document_text_for_bundle(path: Path) -> str:
    raw_text = _read_raw_document_text_for_bundle(path)
    if path.suffix.lower() in rendering.MARKDOWN_FILE_EXTENSIONS:
        return _markdown_to_kindle_text(raw_text)
    return raw_text


def _read_raw_document_text_for_bundle(path: Path) -> str:
    try:
...
class _KindleTextHTMLParser(HTMLParser):
    """Convert sanitized documentation HTML into Kindle-friendly plain text."""
    ...
    _START_TAG_HANDLERS = {
        ...
        "img": "_start_image",
        "li": "_start_list_item",
        "ol": "_start_ordered_list",
        "pre": "_start_preformatted",
        "tr": "_start_table_row",
        "ul": "_start_unordered_list",
    }
    ...
    def __init__(self) -> None:
        ...
        self.just_started_list_item = False
        self.list_stack: list[tuple[str, int]] = []

    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        attributes = dict(attrs)
        self._start_block_tag(tag)
        if tag in {"td", "th"}:
            self._start_table_cell()
            return
        handler_name = self._START_TAG_HANDLERS.get(tag)
        if handler_name:
            getattr(self, handler_name)(attributes)

    def _start_block_tag(self, tag: str) -> None:
        if tag in self._BLOCK_TAGS:
            if not (tag == "p" and self.just_started_list_item):
                self._newline(2 if tag.startswith("h") else 1)

    def _start_break(self, _attributes: dict[str, str | None]) -> None:
        self._newline()

    def _start_list_item(self, _attributes: dict[str, str | None]) -> None:
        self._newline()
        self._append(self._list_item_prefix())
        self.just_started_list_item = True

    def _start_ordered_list(self, _attributes: dict[str, str | None]) -> None:
        self.list_stack.append(("ol", 1))

    def _start_unordered_list(self, _attributes: dict[str, str | None]) -> None:
        self.list_stack.append(("ul", 0))

    def _start_preformatted(self, _attributes: dict[str, str | None]) -> None:
        self.in_pre = True

    def _start_table_row(self, _attributes: dict[str, str | None]) -> None:
        self._newline()
        self.in_row = True
        self.row_has_cell = False

    def _start_table_cell(self) -> None:
        if not self.in_row:
            return
    ...
    def _start_link(self, attributes: dict[str, str | None]) -> None:
        self.href_stack.append(attributes.get("href") or "")

    def _start_image(self, attributes: dict[str, str | None]) -> None:
    ...
    def handle_endtag(self, tag: str) -> None:
        if tag == "a":
            ...
            if self.list_stack:
                self.list_stack.pop()
            self._newline()
        elif tag in self._BLOCK_TAGS or tag == "table":
            self._newline()
    ...
    def handle_data(self, data: str) -> None:
        ...
        if self.in_row:
            text = text.strip()
        if self.just_started_list_item:
            text = text.lstrip()
        if text.strip():
            self.just_started_list_item = False
        self._append(text)
    ...
    def get_text(self) -> str:
        return _normalize_kindle_text("".join(self.parts))
    ...
    def _newline(self, count: int = 1) -> None:
        ...
        for part in reversed(self.parts):
            if not part:
                continue
            trimmed = part.rstrip("\n")
            trailing += len(part) - len(trimmed)
            if trimmed:
                break
        missing = max(0, count - trailing)
        if missing:
            self.parts.append("\n" * missing)
...
def _markdown_to_kindle_text(markdown_text: str) -> str:
    html, _toc = rendering.render_markdown_with_toc(markdown_text)
    parser = _KindleTextHTMLParser()
    parser.feed(html)
    parser.close()
    return parser.get_text()


def _normalize_kindle_text(text: str) -> str:
