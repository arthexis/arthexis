...
@pytest.mark.django_db
def test_build_operators_manual_bundle_renders_markdown_for_kindle(tmp_path):
    _write(
        tmp_path / "README.md",
        "\n".join(
            [
                "# Root",
                "",
                "Use [Admin](https://example.test/admin).",
                "",
                "This is *important*! Is [Docs](https://example.test/docs)?",
                "This is *admin*'s guide.",
                "",
                "```python",
                "if True:",
                "    print('ready')",
    ...
    assert "|---|" not in output
    assert "Root" in output
    assert "Admin (https://example.test/admin)" in output
    assert "important!" in output
    assert "important !" not in output
    assert "Docs (https://example.test/docs)?" in output
    assert "Docs (https://example.test/docs) ?" not in output
    assert "admin's guide." in output
    assert "admin 's guide." not in output
    assert "if True:\n    print('ready')" in output
    assert "- First item" in output
    assert "- Second item" in output
    assert "-\nFirst item" not in output
