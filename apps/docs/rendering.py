...
def render_markdown_with_toc(
    text: str, *, rewrite_local_github_docs: bool = False
) -> tuple[str, str]:
    ...
    if rewrite_local_github_docs:
        html = assets.rewrite_github_doc_links(html)
...
def render_document_file(file_path: Path) -> tuple[str, str]:
    ...
    if extension in MARKDOWN_FILE_EXTENSIONS:
        return render_markdown_with_toc(text, rewrite_local_github_docs=True)
