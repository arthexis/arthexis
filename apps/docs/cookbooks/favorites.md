...
- Clicking a star invokes the `admin:favorite_toggle` view defined in [`locals/admin.py`](../../locals/admin.py) and returns you to the current page after saving.
- Use the **Favorites** link in the admin navigation sidebar to open the management list (`apps/locals/templates/admin/favorite_list.html`). The route is registered as `admin:favorite_list` in [`locals/admin.py`](../../locals/admin.py#L98-L135).
...
Both views live in [`locals/admin.py`](../../locals/admin.py#L136-L147) and redirect back to the favorites list after completion.
