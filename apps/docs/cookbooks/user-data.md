...
2. Select **User Data** from the header shortcuts (the button routes to the `admin:user_data` view defined in [`locals/user_data/views.py`](../../locals/user_data/views.py)).
3. The data list view renders the template [`locals/templates/admin/data_list.html`](../../locals/templates/admin/data_list.html), providing a searchable table of exported fixtures grouped by app and model.
...
The view assembles sections for every model that exposes the `is_user_data` flag (`apps/locals/user_data/views.py`). Each entry shows:
...
Click **Export** above the table to download a ZIP archive that contains every recorded user datum. The archive is generated by `_user_data_export` in [`locals/user_data/views.py`](../../locals/user_data/views.py) and includes one JSON fixture per object.

- The exported filename follows the pattern `user_data_<user-id>.zip` so you can track who generated the bundle.
- Fixtures are sourced from the per-user directory created by `_data_dir` in [`locals/user_data/fixtures.py`](../../locals/user_data/fixtures.py).
...
3. Submit the form to invoke `_user_data_import` in [`locals/user_data/views.py`](../../locals/user_data/views.py).

The import process loads each fixture and, when available, re-applies the `is_user_data` flag (see `_load_fixture` in [`locals/user_data/fixtures.py`](../../locals/user_data/fixtures.py)). Successful imports redirect back to the list view with the updated objects.

## Marking objects as user data

Several admin surfaces let you toggle whether an object should be captured as user data:

- **Row toggle icon** – Change lists expose a per-row toggle icon so operators can mark one record at a time as user data (`apps/locals/user_data/views.py`).
