...
def schedule_call_timeout(
    ...
    *,
    ...
    on_timeout: Callable[[dict[str, object]], object] | None = None,
) -> None:
    ...
    def _notify() -> None:
        ...
        timed_out_metadata: dict[str, object] | None = None
        with _pending_call_lock:
            ...
            if not target_log:
                ...
                metadata["timeout_notice_sent"] = True
            timed_out_metadata = dict(metadata)
        if target_log and entry_label:
            logs.add_log(target_log, entry_label, log_type=log_type)
        if timed_out_metadata is not None and on_timeout is not None:
            try:
                callback_result = on_timeout(timed_out_metadata)
