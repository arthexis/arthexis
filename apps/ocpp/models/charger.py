...
class Charger(Ownable):
    ...
    auto_start_id_tag = models.CharField(
        _("Auto-start idTag"),
        max_length=20,
        blank=True,
        default="",
        help_text=_(
            "Send a remote-start request with this idTag whenever the charger reports Preparing."
        ),
    )
    ...
    last_charging_limit_source = models.CharField(
    ...
    offline_notification_sent_at = models.DateTimeField(
        ...
        blank=True,
        help_text=_(
            "Last time an offline notification was sent for this charge point."
        ),
    )
    ...
    def ensure_diagnostics_bucket(self, *, expires_at: datetime | None = None):
        ...
        if bucket is None:
            ...
            bucket.expires_at is None or bucket.expires_at < expires_at
        ):
    ...
    def resolved_authorization_policy(self) -> str:
        ...
        configured = (
            str(getattr(settings, "OCPP_AUTHORIZATION_POLICY", "") or "")
            .strip()
            .lower()
        )
    ...
    def convert_energy_to_kwh(
        self, energy: Decimal, unit: str | None = None
    ) -> Decimal:
        return self.normalize_energy_value(energy, unit, default_unit=self.energy_unit)
    ...
    def get_absolute_url(self):
        ...
        if connector_slug == self.AGGREGATE_CONNECTOR_SLUG:
            return self._reverse_public_url(
                ("ocpp:charger-page", "charger-page"),
                args=[serial],
            )
        return self._reverse_public_url(
            ("ocpp:charger-page-connector", "charger-page-connector"),
            args=[serial, connector_slug],
        )

    @staticmethod
    def _reverse_public_url(names: tuple[str, ...], *, args: list[str]) -> str:
        for name in names:
            try:
    ...
    def _full_url(self) -> str:
        ...
        absolute_path = self.get_absolute_url()
        ...
        return f"{scheme}://{domain}{absolute_path}"

    @classmethod
    def normalize_auto_start_id_tag(cls, value: object) -> str:
        """Validate an OCPP idTag used for auto-start credentials."""
        ...
        if not raw_value.isascii() or not raw_value.isprintable():
            raise ValidationError(
                {
                    "auto_start_id_tag": _(
                        "Use only printable ASCII characters for an idTag."
                    )
                }
            )
        ...
        if len(normalized) > 20:
            raise ValidationError(
                {"auto_start_id_tag": _("Use at most 20 characters for an idTag.")}
            )
        return normalized
    ...
    def clean(self):
        ...
        self.auto_start_id_tag = type(self).normalize_auto_start_id_tag(
            self.auto_start_id_tag
        )
