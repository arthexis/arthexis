...
from apps.ocpp.auto_start_accounts import get_or_create_auto_start_account
...
class Command(BaseCommand):
    ...
    def _select_chargers(
        ...
        all_chargers = bool(options.get("all_chargers"))
        queryset = (
            Charger.objects.all()
        ...
        if all_chargers and (serial or cp_raw or use_default_base):
            raise CommandError(
                "Use --all by itself, or select chargers with --sn/--cp/--default-base."
            )

        if use_default_base and not serial and not cp_raw and not all_chargers:
        ...
        return queryset, {
            ...
            "cp_path": cp_path,
            "has_selector": bool(all_chargers)
            or bool(serial)
    ...
    def _require_selector(
        ...
        *,
        message: str = (
            ...
            "and/or --cp, or explicitly using --all."
        ),
    ) -> None:
    ...
    def _handle_enable_autostart(
        self, *, id_tag: object, queryset: QuerySet[Charger]
    ) -> None:
        """Enable Preparing-triggered remote starts and provision their account."""

        try:
            normalized_id_tag = Charger.normalize_auto_start_id_tag(id_tag)
        except ValidationError as exc:
            raise CommandError(exc.messages[0]) from exc
        if not normalized_id_tag:
            raise CommandError("enable-autostart requires a non-empty idTag.")

        _account, created = self._get_or_create_autostart_account(normalized_id_tag)
        charger_ids = list(queryset.values_list("pk", flat=True))
        updated = queryset.update(auto_start_id_tag=normalized_id_tag)
        auto_start.release_chargers(charger_ids=charger_ids)
        account_message = (
            "Created an energy account."
            if created
            else "Using the existing energy account."
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Enabled auto-start on {updated} charger(s). {account_message}"
            )
        )

    def _handle_disable_autostart(self, *, queryset: QuerySet[Charger]) -> None:
        """Disable Preparing-triggered remote starts without removing their account."""

        charger_ids = list(queryset.values_list("pk", flat=True))
        updated = queryset.update(auto_start_id_tag="")
        auto_start.release_chargers(charger_ids=charger_ids)
        self.stdout.write(
            self.style.SUCCESS(f"Disabled auto-start on {updated} charger(s).")
        )

    def _get_or_create_autostart_account(self, id_tag: str):
        """Return the shared auto-start account provisioning result."""

        return get_or_create_auto_start_account(id_tag)

    def _handle_config_action(
        self,
        *,
        chargers: list[Charger],
        key: object,
        ...
        """Send OCPP configuration get/change calls to selected charge points."""

        targets = self._configuration_targets(chargers)
        if mode == "get":
            key_list = self._normalize_config_keys(keys)
            payload: dict[str, object] = {}
            metadata: dict[str, object] = {}
            if key_list:
                payload["key"] = key_list
                metadata["configuration_key_filter"] = key_list
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Requested GetConfiguration from {len(targets)} charger(s)."
                )
            )
            return
        ...
        if mode == "set":
            ...
            if not key_text:
                raise CommandError("Configuration key is required.")
            if value is None:
            ...
            for charger in targets:
                self._send_control_call(
                    ...
                    payload={"key": key_text, "value": value_text},
                    metadata={"key": key_text, "value": value_text},
                    timeout_message=(
                        f"ChangeConfiguration for {key_text} timed out: "
                        "charger did not respond"
                    ),
                )
            self.stdout.write(
                self.style.SUCCESS(
                    f"Requested ChangeConfiguration for {key_text} on "
                    f"{len(targets)} charger(s)."
                )
            )
            return

        raise CommandError(f"Unknown configuration action: {mode}")

    def _configuration_targets(self, chargers: list[Charger]) -> list[Charger]:
        """Return one OCPP control target per charge point, preferring base rows."""

        by_station: dict[str, list[Charger]] = {}
        for charger in chargers:
            by_station.setdefault(charger.charger_id, []).append(charger)

        targets: list[Charger] = []
        seen: set[tuple[str, int | None]] = set()
        for station_chargers in by_station.values():
            base = next(
                (
                    charger
            ...
            if base is None and len(station_chargers) == 1:
                base = self._resolve_aggregate_charger(station_chargers)
            if base is not None:
                station_targets = [base]
            else:
                station_targets = station_chargers
            for charger in station_targets:
                key = (charger.charger_id, charger.connector_id)
                if key in seen:
                    continue
                seen.add(key)
    ...
    @staticmethod
    def _normalize_config_keys(keys: object) -> list[str]:
    ...
    def _action_targets_for_single_station(
            ...
            self._send_control_call(
                ...
                payload={
                    ...
                    "localAuthorizationList": [
                        entry.copy() for entry in authorization_list
                    ],
                },
                metadata={
