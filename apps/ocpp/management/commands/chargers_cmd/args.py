...
def add_selector_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        dest="all_chargers",
        action="store_true",
        help="Select every configured charger. Intended for explicit field/debug operations.",
    )
    parser.add_argument(
        "--sn",
        dest="serial",
...
def build_chargers_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.epilog = (
        ...
        "  charger rfid open --all\n"
        "  charger rfid on --sn CP-01\n"
        "  charger config get --sn CP-01 HeartbeatInterval\n"
        "  charger config set --sn CP-01 LocalAuthorizeOffline false\n"
        "  charger auth set cp-user secret123 --sn CP-01\n"
        "  charger enable-autostart AUTO-START-TAG --sn CP-01\n"
    ...
    rfid_parser.add_argument(
        "rfid_action", choices=["on", "off", "open", "strict", "push", "lock"]
    )

    config_parser = subparsers.add_parser(
        "config",
        help="Get or change OCPP charger configuration keys.",
        description="Get or change OCPP charger configuration keys.",
    )
    ...
    config_get = config_subparsers.add_parser(
        "get",
        parents=[selector_parent],
        help="Send OCPP GetConfiguration.",
        description="Send OCPP GetConfiguration.",
    )
    config_get.add_argument(
        "keys",
        nargs="*",
        help="Optional configuration key names to request. Omit to request all keys.",
    )
    config_set = config_subparsers.add_parser(
        "set",
        parents=[selector_parent],
        help="Send OCPP ChangeConfiguration.",
        description="Send OCPP ChangeConfiguration.",
    )
    config_set.add_argument("key", help="OCPP configuration key to change.")
    config_set.add_argument("value", help="New configuration value.")
