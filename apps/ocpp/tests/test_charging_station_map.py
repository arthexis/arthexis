...
def test_charging_station_map_renders_charger_locations(monkeypatch, client):
    ...
    location = Location.objects.create(
        name="North Depot", latitude=19.432608, longitude=-99.133209
    )
    charger = Charger.objects.create(
        charger_id="MAP-001", connector_id=1, location=location
    )

    monkeypatch.setattr(
        "apps.ocpp.views.public.require_site_operator_or_staff", lambda request: None
    )
    monkeypatch.setattr(
    ...
    assert content.index('id="charging-location-list-panel"') < content.index(
        'id="charging-location-details"'
    )
    assert content.index('id="charging-location-details"') < content.index(
        'id="charging-map"'
    )
