...
class OcppCommandTests(SimpleTestCase):
    ...
    @patch("apps.ocpp.management.commands.ocpp.run_export_transactions", return_value=5)
    def test_transactions_export_accepts_all_chargers_flag(self, run_export) -> None:
        """``--all-chargers`` should explicitly request an unfiltered export."""
    ...
    @patch("apps.ocpp.management.commands.ocpp.run_export_transactions")
    def test_transactions_export_rejects_all_chargers_with_filter(
        self, run_export
    ) -> None:
        """``--all-chargers`` and ``--chargers`` are mutually exclusive."""
    ...
    def test_trace_replay_runs_canonical_subcommand(self, run_replay) -> None:
        ...
        run_replay.return_value = SimpleNamespace(
            imported=3, skipped=1, session_log_written=True
        )
        out = io.StringIO()
