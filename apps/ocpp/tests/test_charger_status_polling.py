...
@pytest.mark.django_db
def test_status_view_sessions_stop_header_omits_charger_time_label(client):
    user = get_user_model().objects.create_user(
        username="status-sessions-stop-label", password="pass"
    )
    client.force_login(user)
    charger = Charger.objects.create(charger_id="STATUS-STOP-LABEL", connector_id=1)
    ...
    assert "status-footer-links" in html
