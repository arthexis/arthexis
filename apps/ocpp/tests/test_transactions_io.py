...
def test_export_transactions_filters_and_serializes_decimals(base_time):
    ...
    assert exported_tx["voltage_start"] == "230.500"
    assert exported_tx["voltage_stop"] == "229.750"
    assert exported_tx["current_import_start"] == "10.125"
    assert exported_tx["current_import_stop"] == "11.000"
    json.dumps(exported)
...
@pytest.mark.django_db
def test_sync_transactions_payload_updates_and_skips_invalid_entries(
    base_time, monkeypatch
):
