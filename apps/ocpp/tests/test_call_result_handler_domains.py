...
@pytest.mark.anyio
@pytest.mark.django_db(transaction=True)
async def test_profiles_domain_persists_variable_result():
    ...
    result = await handle_get_variables_result(
        consumer,
        "var-msg",
        {"charger_id": charger.charger_id, "connector_id": charger.connector_id},
        {
            "getVariableResult": [
                {
    ...
    assert variable.attribute_status == "Accepted"
...
def test_legacy_call_result_handlers_are_packaged():
    from apps.ocpp.call_result_handlers import legacy
    from apps.ocpp.call_result_handlers.legacy import certificates

    assert hasattr(legacy, "__path__")
    assert legacy.handle_certificate_signed_result is (
        certificates.handle_certificate_signed_result
    )
    assert legacy.handle_get_installed_certificate_ids_result is (
        certificates.handle_get_installed_certificate_ids_result
    )
