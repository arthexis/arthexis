...
from apps.ocpp.management import coverage_ocpp21_impl, coverage_ocpp201_impl
...
@pytest.mark.django_db
def test_get_diagnostics_defaults_to_expiring_anonymous_upload_bucket(client, ws):
    charger = Charger.objects.create(charger_id="CID-DIAG")
    connector_value = charger.connector_id
    log_key = store.identity_key(charger.charger_id, connector_value)
    request = RequestFactory().post("/ocpp/actions/")
    context = ActionContext(
        charger.charger_id,
        connector_value,
        charger=charger,
    ...
    result = actions._handle_get_diagnostics(context, {})

    assert isinstance(result, ActionCall)
    charger.refresh_from_db()
    assert charger.diagnostics_bucket is not None
    bucket = charger.diagnostics_bucket
    assert bucket.expires_at is not None
    message = json.loads(ws.sent[0])
    assert message[2] == "GetDiagnostics"
    assert message[3]["stopTime"] == bucket.expires_at.isoformat()

    response = client.post(
        reverse("ocpp:media-bucket-upload", kwargs={"slug": bucket.slug}),
        {
