...
async def test_filtered_get_configuration_result_does_not_replace_snapshot():
    ...
    configuration_count = await database_sync_to_async(
        ChargerConfiguration.objects.filter(charger_identifier=charger.charger_id).count
    )()
...
async def test_cleared_charging_limit_logs_payload():
    ...
    consumer.charger = await database_sync_to_async(Charger.objects.create)(
        charger_id="CP-201"
    )
    consumer.aggregate_charger = None
...
async def test_get_certificate_status_marks_revoked_when_ocsp_revoked(monkeypatch):
    ...
    monkeypatch.setattr(
        consumers_base.certificate_status.requests, "Session", FakeSession
    )
...
async def test_get_certificate_status_ocsp_timeout_uses_responder_unavailable(
    monkeypatch,
):
    charger = await database_sync_to_async(Charger.objects.create)(
        charger_id="CERT-TIMEOUT"
    )
    hash_data = {
        "hashAlgorithm": "SHA256",
        "issuerKeyHash": "def",
...
async def test_get_certificate_status_invalid_crl_payload_is_responder_unavailable(
    monkeypatch,
):
    charger = await database_sync_to_async(Charger.objects.create)(
        charger_id="CERT-CRL"
...
@pytest.mark.anyio
async def test_get_certificate_status_uses_non_thread_sensitive_sync_wrapper(
    monkeypatch,
):
...
@pytest.mark.django_db
def test_notify_customer_information_skips_customer_care_workflow_without_flows():
...
async def test_transaction_event_updates_request_status(monkeypatch, charger_factory):
    ...
    assert started_tx.authorization_status == Transaction.AuthorizationStatus.ACCEPTED
...
async def test_transaction_event_started_notifies_and_persists(charger_factory):
    ...
    assert tx_obj.authorization_status == Transaction.AuthorizationStatus.ACCEPTED
...
@pytest.mark.anyio
@pytest.mark.django_db(transaction=True)
async def test_report_charging_profiles_evse_zero_uses_station_charger(monkeypatch):
    station_charger = await database_sync_to_async(Charger.objects.create)(
        charger_id="RCP-EVSE-0", connector_id=0
    )
    connector_charger = await database_sync_to_async(Charger.objects.create)(
        charger_id="RCP-EVSE-0", connector_id=1
    )
    profile = await database_sync_to_async(ChargingProfile.objects.create)(
    ...
    assert not logs
    updated = await database_sync_to_async(ChargingProfile.objects.get)(
        charger=station_charger, connector_id=0, charging_profile_id=12
    )
    assert updated.stack_level == 1
...
async def test_reservation_status_update_tracks_accepted_to_rejected_transition():
    ...
    accepted_state = await database_sync_to_async(CPReservation.objects.get)(
        pk=reservation.pk
    )
    ...
    rejected_state = await database_sync_to_async(CPReservation.objects.get)(
        pk=reservation.pk
    )
