...
from apps.ocpp.admin.public_routes import (
...
class ChargerAdminViewsMixin:
    ...
    def view_charge_point_dashboard(self, request, queryset=None):
        url = reverse_public_ocpp_route("ocpp:ocpp-dashboard")
        if url:
    ...
    def _apply_location_names(
        self, chargers: list[Charger], location_name: str
    ) -> None:
    ...
    def setup_location_view(self, request):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                self.message_user(
                    request,
                    _("Updated %(count)d chargers with location %(location)s.")
