...
class RemoteControlActionsMixin(ActionServiceMixin):
        ...
        for charger in queryset:
            ...
            if charger.is_local:
                ...
                if self._send_local_ocpp_call(
                    ...
                    payload=payload,
                    pending_payload={},
                    timeout_kwargs={"message": "UnlockConnector request timed out"},
                ):
                    unlocked += 1
                continue
            ...
            success, updates = self._call_remote_action(
                request, local_node, private_key, charger, "unlock-connector"
            )
            if success:
    ...
    def remote_stop_transaction(self, request, queryset):
        ...
        for charger in queryset:
            if charger.is_local:
                ...
                if self._send_local_ocpp_call(
                    ...
                    pending_payload={"transaction_id": tx_obj.pk},
                ):
                    stopped += 1
                continue
            if not charger.allow_remote:
            ...
            if local_node is None:
                ...
                if not local_node or not private_key:
                    ...
                    continue
            success, updates = self._call_remote_action(
                request, local_node, private_key, charger, "remote-stop"
            )
            if success:
                self._apply_remote_updates(charger, updates)
                stopped += 1
        if stopped:
            self.message_user(
                request, f"Sent RemoteStopTransaction to {stopped} charger(s)"
            )
    ...
    def reset_chargers(self, request, queryset):
        ...
        for charger in queryset:
            if charger.is_local:
                if (
                    store.get_transaction(charger.charger_id, charger.connector_id)
                    is not None
                ):
                    self.message_user(
                        request,
                        f"{charger}: reset skipped because a session is active; stop the session first.",
