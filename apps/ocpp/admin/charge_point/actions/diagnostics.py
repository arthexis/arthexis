...
from ....admin.public_routes import (
    PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE,
...
class DiagnosticsActionsMixin(ActionServiceMixin):
    ...
    class DiagnosticsSetupError(Exception):
        """Raised when diagnostics setup cannot build an upload target."""

    def _diagnostics_directory_for(self, user) -> tuple[Path, Path]:
        username = (
            user.get_username()
            if callable(getattr(user, "get_username", None))
            else getattr(user, "username", "")
        )
        username_component = (
            Path(str(username or getattr(user, "pk", "user"))).name or "user"
        )
    ...
    def _diagnostics_filename(self, charger: Charger, location: str, response) -> str:
        ...
        if content_disposition:
            candidate = (
                self._content_disposition_filename(content_disposition) or candidate
            )
        if not candidate:
            candidate = "diagnostics.log"
        suffix = "".join(Path(candidate).suffixes) or ".log"
        base_name = (candidate[: -len(suffix)] if suffix else candidate).rstrip(
            "."
        ) or "diagnostics"
        charger_slug = (
            slugify(
                charger.charger_id
                or charger.display_name
                or str(charger.pk or "charger")
            )
            or "charger"
        )
    ...
    def _unique_diagnostics_path(self, directory: Path, filename: str) -> Path:
        ...
        base = (filename[: -len(suffix)] if suffix else filename).rstrip(
            "."
        ) or "diagnostics"
    ...
    def _download_diagnostics(
        ...
        if not parsed_url.hostname or not self._is_safe_diagnostics_host(
            parsed_url.hostname
        ):
        ...
        try:
            if response.status_code != 200:
                raise self.DiagnosticsDownloadError(
                    _("Diagnostics download returned status %s.") % response.status_code
                )
            destination = self._unique_diagnostics_path(
                diagnostics_dir, self._diagnostics_filename(charger, location, response)
            )
        ...
        asset_url = reverse(
            ...
            kwargs={
                "source": "work",
                "asset": destination.relative_to(user_dir).as_posix(),
            },
        )
    ...
    def _prepare_diagnostics_payload(self, request, charger: Charger, *, expires_at):
        ...
        path = reverse_public_ocpp_route(
            "ocpp:media-bucket-upload",
            kwargs={"slug": bucket.slug},
        )
        if not path:
            raise self.DiagnosticsSetupError(
                _("%(charger)s: %(message)s")
                % {
                    "charger": charger,
                    "message": PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE,
                }
            )
        location = request.build_absolute_uri(path)
    ...
    def _request_get_diagnostics(
        ...
        for charger in queryset:
            ...
            except self.DiagnosticsSetupError as exc:
            ...
            if charger.is_local:
                if self._send_local_ocpp_call(
                    ...
                    payload=payload,
                    pending_payload={"location": payload["location"]},
                ):
                    requested += 1
                continue
    ...
    def get_diagnostics(self, request, queryset):
        ...
        for charger in queryset:
            ...
            try:
                destination, asset_url = self._download_diagnostics(
                    request, charger, location, diagnostics_dir, user_dir
                )
            except self.DiagnosticsDownloadError as exc:
                self.message_user(
                    request,
                    _("%(charger)s: %(error)s") % {"charger": charger, "error": exc},
                    level=messages.ERROR,
                )
                continue
        ...
        if successes:
            ...
            self.message_user(
                request,
                format_html("{}<ul>{}</ul>", summary, details),
