...
from .public_routes import PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE
...
class PublicConnectorPageAdmin(EntityModelAdmin):
    ...
    def regenerate_qr_assets(self, request, queryset):
        ...
        for page in queryset:
            public_url = page.public_url(request)
            if not public_url:
                missing_routes = True
                continue
            page.refresh_qr_assets(public_url)
    ...
    def download_sticker_sheet(self, request, queryset):
        ...
        for page in pages:
            ...
            if not public_url:
                ...
                return HttpResponseRedirect(request.get_full_path())
            page_urls.append((page, public_url))
        ...
        page_index = 0
        for index, (page, qr_url) in enumerate(page_urls):
