from ...models import CPFirmwareRequest
...
class CPFirmwareAdmin(EntityModelAdmin):
    ...
    def _dispatch_firmware_update(
        ...
        download_path = reverse_public_ocpp_route(
            "ocpp:cp-firmware-download",
            args=[deployment.pk, token],
        )
        if not download_path:
            deployment.mark_status("Error", PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE)
        ...
        if result is None:
            ...
            self.message_user(
                request,
                _("The charge point did not respond to the UpdateFirmware request."),
                level=messages.ERROR,
            )
            return False
        ...
        if status_value.lower() != "accepted":
            self.message_user(
                request,
                _("UpdateFirmware for %(charger)s was %(status)s.")
        ...
        self.message_user(
            request,
            _("Queued firmware installation for %(charger)s.") % {"charger": charger},
    ...
    def upload_evcs_firmware(self, request, queryset):
        ...
        if selected_ids:
            ...
            firmware_list = [
                firmware_map[value] for value in selected_ids if value in firmware_map
            ]
        else:
