...
from ...models import ControlOperationEvent
...
class ChargerConfigurationAdmin(admin.ModelAdmin):
    ...
    def _restart_charger(self, charger: Charger) -> tuple[bool, str]:
        ...
        if ws is None:
            ...
            logging.error(
                "Failed to send Reset to charger %s (connector %s)",
                charger.charger_id,
                connector_value,
            )
            return False, message
...
class StationModelAdmin(EntityModelAdmin):
    ...
    class StationModelConfigurationGuideInline(admin.StackedInline):
        model = StationModelConfigurationGuide
        extra = 1
        fields = ("title", "firmware_version", "sort_order", "notes")
        ordering = ("sort_order", "id")
        show_change_link = True

    inlines = (StationModelConfigurationGuideInline,)
    ...
    def view_in_site(self, request):
        url = reverse_public_ocpp_route("ocpp:supported-chargers")
        if url:
        ...
        return HttpResponseRedirect(reverse("admin:ocpp_stationmodel_changelist"))


class StationModelConfigurationGuideStepInline(admin.StackedInline):
    model = StationModelConfigurationGuideStep
    extra = 1
    fields = ("step_number", "title", "instructions_markdown")
    ordering = ("step_number", "id")


@admin.register(StationModelConfigurationGuide)
class StationModelConfigurationGuideAdmin(EntityModelAdmin):
    list_display = ("title", "station_model", "firmware_version", "sort_order")
    search_fields = (
        "title",
        "firmware_version",
        "station_model__vendor",
        "station_model__model",
    )
    list_filter = ("firmware_version", "station_model__vendor")
    autocomplete_fields = ("station_model",)
    inlines = (StationModelConfigurationGuideStepInline,)
