...
class RateLimitedConnectionMixin:
    ...
    def _incoming_omits_subprotocol(self) -> bool:
        get_offered = getattr(self, "_get_offered_subprotocols", None)
        if get_offered is None:
            return False
        try:
            return not bool(get_offered())
        except Exception:
            return False

    @staticmethod
    def _connection_has_explicit_subprotocol(connection) -> bool:
        if getattr(connection, "accepted_subprotocol", None):
            return True
        get_offered = getattr(connection, "_get_offered_subprotocols", None)
        if get_offered is None:
            return False
        try:
            offered = get_offered()
            ...
            return False
        canonicalize = getattr(connection, "_canonicalize_ocpp_subprotocol", None)
        if canonicalize is None:
            return bool(offered)
        return any(canonicalize(proto) for proto in offered)
    ...
    async def _accept_connection(self, subprotocol: str | None) -> bool:
        ...
        if (
            ...
            and subprotocol is None
            and self._incoming_omits_subprotocol()
            and self._connection_has_explicit_subprotocol(existing)
        ):
            store.add_log(
                self.store_key,
                "Rejected no-subprotocol probe while an explicit OCPP connection is active",
                log_type="charger",
            )
    ...
    async def _has_rate_limit_rule(self) -> bool:
        ...
        if rate_limit_model is None:
            return getattr(self, "rate_limit_fallback", None) is not None

        def _resolve_rule() -> bool:
            return (
                rate_limit_model.for_target(
