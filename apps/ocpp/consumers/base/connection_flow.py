...
from ..path_metadata import bounded_last_path
...
class ConnectionFlowMixin:
    ...
    @staticmethod
    def _release_store_connection(key: str, connection) -> None:
        if store.connections.get(key) is connection:
            store.connections.pop(key, None)
    ...
    async def connect(self):
        ...
        self._clear_pending_calls_on_disconnect = False
        ...
        is_handshake_probe = (
            existing_charger is not None
            and subprotocol is None
            and not offered_subprotocols
        )
        ...
        if not await self._accept_connection(subprotocol):
            return
        self._clear_pending_calls_on_disconnect = True
    ...
    async def disconnect(self, close_code):
        ...
        self._release_store_connection(store_key, self)
        pending_key = store.pending_key(charger_id)
        if store_key != pending_key:
            self._release_store_connection(pending_key, self)
        store.end_session_log(store_key)
        store.stop_session_lock()
        if charger_id and getattr(self, "_clear_pending_calls_on_disconnect", False):
