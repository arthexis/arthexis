"""Primary OCPP CSMS websocket consumers and handlers.

This module hosts the concrete OCPP 1.6/2.x websocket consumer implementation.
Protocol-version-specific behavior remains in action handlers, while cross-cutting
concerns (connection admission, dispatch registry, parsing, forwarding) are
extracted into focused modules under ``consumer/``.

Public extension points:
    * ``SinkConsumer`` for permissive sink/webhook usage.
    * ``CSMSConsumer`` for full CSMS session orchestration.
"""
