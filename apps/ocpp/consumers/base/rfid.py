...
class RfidMixin:
    ...
    async def _get_account(self, id_tag: str) -> CustomerAccount | None:
        """Return the customer account for the provided OCPP idTag or RFID."""
        if not id_tag:
            return None
        ...
        charger_pks = {
            ...
            if getattr(charger, "pk", None)
        }

        def _resolve() -> CustomerAccount | None:
            direct_account = CustomerAccount.objects.filter(ocpp_id_tag=id_tag).first()
            if (
                direct_account is not None
                and direct_account.service_account
                and charger_pks
                and Charger.objects.filter(
                    pk__in=charger_pks, auto_start_id_tag=id_tag
                ).exists()
            ):
                return direct_account
            matches = CoreRFID.matching_queryset(id_tag)
            normalized_id_tag = CoreRFID.normalize_code(id_tag)
            exact_rfid_values = {
                normalized_id_tag,
                CoreRFID.reverse_uid(normalized_id_tag),
            } - {""}
            exact_matches = matches.filter(rfid__in=exact_rfid_values)
            if exact_matches.exists():
                allowed_matches = exact_matches.filter(allowed=True)
                return (
                    CustomerAccount.objects.filter(rfids__in=allowed_matches)
                    .distinct()
                    .first()
                )
            if direct_account is not None:
                return direct_account
            if matches.exists():
                allowed_matches = matches.filter(allowed=True)
                return (
                    CustomerAccount.objects.filter(rfids__in=allowed_matches)
                    .distinct()
                    .first()
    ...
    @staticmethod
    def _is_direct_ocpp_account(account: CustomerAccount | None, id_tag: str) -> bool:
        """Return whether an idTag is configured directly on an account."""

        return bool(account and account.ocpp_id_tag == (id_tag or "").strip())
    ...
    async def _apply_rfid_authorization_side_effects(
        ...
        *,
        ...
        if (
            id_tag
            and decision.should_mark_seen
            and not self._is_direct_ocpp_account(account, id_tag)
        ):
    ...
    async def _ensure_rfid_seen(
        ...
        *,
        ...
        def _ensure() -> CoreRFID:
            ...
            if (
                auto_enroll
                and current_tag_created
                and not current_tag.discovered_via_ocpp
            ):
