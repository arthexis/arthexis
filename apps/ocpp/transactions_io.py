        ...
        data["transactions"].append(
            {
                ...
                "voltage_start": _serialize_decimal(tx.voltage_start),
                "voltage_stop": _serialize_decimal(tx.voltage_stop),
                "current_import_start": _serialize_decimal(tx.current_import_start),
                "current_import_stop": _serialize_decimal(tx.current_import_stop),
                "current_offered_start": _serialize_decimal(tx.current_offered_start),
                "current_offered_stop": _serialize_decimal(tx.current_offered_stop),
                "temperature_start": _serialize_decimal(tx.temperature_start),
                "temperature_stop": _serialize_decimal(tx.temperature_stop),
                "soc_start": _serialize_decimal(tx.soc_start),
                "soc_stop": _serialize_decimal(tx.soc_stop),
                "start_time": tx.start_time.isoformat(),
                "stop_time": tx.stop_time.isoformat() if tx.stop_time else None,
                "received_start_time": (
                    tx.received_start_time.isoformat()
                    if tx.received_start_time
                    else None
                ),
                "received_stop_time": (
                    tx.received_stop_time.isoformat() if tx.received_stop_time else None
                ),
                "meter_values": [
                    {
...
def _normalize_transaction_entry(tx: Mapping, charger_map: dict[str, Charger]) -> dict:
    ...
    try:
        charger = (
            charger_map.get(serial)
            or Charger.objects.get_or_create(charger_id=serial)[0]
        )
    ...
    for tx in transactions:
        ...
        for mv in tx["meter_values"]:
            MeterValue.objects.create(
                transaction=transaction, charger=tx["charger"], **mv
            )
        yield transaction
...
def _persist_transactions_with_objects(
