...
VariableKey = tuple[str, str, str, str, str]


def _component_variable_key(entry: dict) -> VariableKey | None:
...
def _result_charger(consumer: CallResultContext, metadata: dict) -> Charger | None:
...
def _mark_firmware_deployment_result(
    deployment_pk: object,
    payload_data: dict,
) -> None:
...
async def _handle_firmware_deployment_result(
    message_id: str,
    metadata: dict,
    payload_data: dict,
) -> bool:
    await database_sync_to_async(_mark_firmware_deployment_result)(
        metadata.get("deployment_pk"),
        payload_data,
    )
...
def _update_variable_snapshot(
    ...
    *,
    ...
    variable_key = _component_variable_key(entry)
...
async def handle_update_firmware_result(
    ...
    return await _handle_firmware_deployment_result(message_id, metadata, payload_data)
...
async def handle_remote_start_transaction_result(
    ...
    if status_value:
        message += f": status={status_value}"
    store.add_log(log_key, message, log_type="charger")
    await database_sync_to_async(auto_start.apply_call_result)(
        metadata=metadata,
        payload=payload_data,
    )
    store.record_pending_call_result(
...
async def handle_get_variables_result(
    ...
    def _apply() -> None:
        charger = _result_charger(consumer, metadata)
        if charger is None:
            return
...
async def handle_set_variables_result(
    ...
    if not isinstance(request_entries, (list, tuple)):
        request_entries = []
    request_lookup: dict[VariableKey, str] = {}
    for entry in request_entries:
        ...
        attribute_value = entry.get("attributeValue")
        value_text = str(attribute_value) if attribute_value is not None else ""
        request_lookup[variable_key] = value_text
    ...
    def _apply() -> None:
        ...
        for entry in results:
            ...
            if variable_key is None:
                continue
            attribute_status = str(entry.get("attributeStatus") or "").strip()
            value_text = request_lookup.get(variable_key, "")
            _update_variable_snapshot(
                charger=charger,
                entry=entry,
                attribute_status=attribute_status,
