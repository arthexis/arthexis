...
def _attach_charger_group_entry(
...
def dashboard(request):
    ...
    for charger in visible_chargers:
        ...
        _attach_charger_group_entry(charger_groups, group_lookup, entry)

    for group in charger_groups:
        parent_entry = group.get("parent")
        if not parent_entry or not group["children"]:
            continue
        total_kw = sum(child["stats"]["total_kw"] for child in group["children"])
        today_kw = sum(child["stats"]["today_kw"] for child in group["children"])
        parent_entry["stats"] = {"total_kw": total_kw, "today_kw": today_kw}
        parent_entry["last_session"] = parent_entry["last_seen"]
        connector_states = [
    ...
    context = {
        ...
        "show_demo_notice": is_watchtower and not request.user.is_authenticated,
        "demo_ws_url": ws_url,
