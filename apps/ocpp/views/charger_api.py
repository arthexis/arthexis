...
def _is_loopback_address(value: object) -> bool:
    """Return whether a supplied direct or trusted-proxy address is loopback."""
...
@never_cache
@require_GET
def kiosk_live_connection(request):
    """Return current OCPP connection state for the host-local kiosk only."""

    client_addresses = [request.META.get("REMOTE_ADDR")]
    if "HTTP_X_REAL_IP" in request.META:
        client_addresses.append(request.META["HTTP_X_REAL_IP"])
    if not all(_is_loopback_address(address) for address in client_addresses):
        return JsonResponse({"detail": "loopback only"}, status=403)

    requested_ids = list(
        dict.fromkeys(
            charger_id.strip()
            for charger_id in request.GET.getlist("charger_id")
            if charger_id.strip()
        )
    )
    charger_ids = requested_ids or list(
        Charger.objects.order_by("charger_id")
        .values_list("charger_id", flat=True)
        .distinct()
    )
    live_ids = [
        charger_id for charger_id in charger_ids if store.is_connected(charger_id)
    ]
    return JsonResponse(
        {
            "connected": bool(live_ids),
            "charger_count": len(live_ids),
...
def charger_list(request):
    ...
    for charger in _visible_chargers(request.user):
        ...
        entry = {
            ...
            "authorization_policy": charger.resolved_authorization_policy(),
