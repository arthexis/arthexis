...
from ..models import StationModel, Transaction, annotate_transaction_energy_bounds
...
def charger_page(request, cid, connector=None):
    ...
    if charger.connector_id is None:
        if sessions:
            ...
            start_times = [
                tx_obj.start_time
                for _session_charger, tx_obj in sessions
...
def charger_log_page(request, cid, connector=None):
    """Render a simple page with the log for a charger."""
    log_type = "charger"
    connector_links = []
    connector_slug = None
    status_url = None
...
def supported_charger_detail(request, station_model_id: int):
    ...
    instructions_html = rendering.render_markdown_with_toc(
        station_model.instructions_markdown or ""
    )[0]
    configuration_guides = list(
        station_model.configuration_guides.all().prefetch_related("steps")
    )
    rendered_guides = []
    for guide in configuration_guides:
        rendered_steps = []
        for step in guide.steps.all():
            step_html = rendering.render_markdown_with_toc(
                step.instructions_markdown or ""
            )[0]
            rendered_steps.append({"step": step, "instructions_html": step_html})
        rendered_guides.append({"guide": guide, "steps": rendered_steps})
    ...
    return render(
        ...
        "ocpp/supported_charger_detail.html",
        {
            ...
            "configuration_guides": rendered_guides,
            "images": images,
            "documents": documents,
            "operator_interface_mode": False,
        },
