...
from apps.nodes.roles import CONTROL_NODE_ROLE, node_is_control
...
RFID_CONTROL_ONLY_REASON = "RFID scanner is Control-only"
    ...
    if node is not None:
        if node_is_control(node):
            return {
                "detected": True,
                "assumed": True,
                "reason": "Control role scanner service",
            }
        return {"detected": False, "reason": RFID_CONTROL_ONLY_REASON}

    node_role = getattr(settings, "NODE_ROLE", None)
    if (
        node_role is not None
        and str(node_role).strip().casefold() != CONTROL_NODE_ROLE.casefold()
    ):
        return {"detected": False, "reason": RFID_CONTROL_ONLY_REASON}
