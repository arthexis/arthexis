...
EXTENSION_OVERFLOW_NOTE = (
    "Additional skill sigils were omitted because the card has ten extension slots."
)
...
def build_agent_card_sector_payloads(
    *,
    ...
    padded_records = {
        sector: _pad_record(record) for sector, record in sector_records.items()
    }
...
def parse_agent_card(
    ...
    records = {
        sector: _parse_record(sector, payloads[sector])
        for sector in APPLICATION_SECTORS
    }
    manifest = records[1]
...
def _expected_reader_proof(
    *,
    ...
    return hmac.new(
        _reader_proof_secret(secret), canonical_payload, hashlib.sha256
    ).hexdigest()
    ...
    *,
    ...
    if trust_tier not in ACTIVATING_TRUST_TIERS:
        return ReaderTrustResult(
            False, trust_tier, "reader is not trusted for activation"
        )
    observed_at = _reader_observed_at_value(reader_event.get("observed_at"))
    if not observed_at:
        return ReaderTrustResult(
            False, trust_tier, "reader event timestamp is required"
        )
    try:
