...
from apps.cards.command_layout import (
    COMMAND_CARD_NAME_BLOCK,
    COMMAND_METADATA_BLOCK,
...
COMMON_MIFARE_CLASSIC_KEYS = reader_mifare.COMMON_MIFARE_CLASSIC_KEYS
_ExpectedMifareAuthFailureFilter = reader_mifare.ExpectedMifareAuthFailureFilter
_is_sector_trailer_block = reader_mifare.is_sector_trailer_block
_key_to_bytes = reader_mifare.key_to_bytes
_normalize_key = reader_mifare.normalize_key
_read_block = reader_mifare.read_block
_suppress_expected_mifare_auth_logs = reader_mifare.suppress_expected_mifare_auth_logs
_write_block = reader_mifare.write_block
...
def _suppress_gpio_warnings() -> None:
    ...
    try:
        ...
        logger.debug(
            "Unable to suppress GPIO warnings before RFID setup", exc_info=True
        )
...
def _write_block_with_candidates(
    mfrc, tag, uid: list[int], block: int, data: list[int]
) -> tuple[bool, str | None]:
...
def _write_writer_metadata(
    ...
    *,
    ...
    payloads = {
        sector_block(WRITER_SECTOR, WRITER_ID_BLOCK_OFFSET): encode_writer_id(
            normalized_writer
        ),
        sector_block(WRITER_SECTOR, WRITER_DATE_BLOCK_OFFSET): encode_writer_date(
            timestamp
        ),
    }
    errors: list[str] = []
    ...
    *,
    ...
    if writer_result.get("errors"):
        return {
            ...
            "errors": [{"writer": writer_result["errors"]}],
        }
    ...
    if initialized_sectors:
        ...
        if any(
            ...
            updates.add("traits")
    fully_initialized = (
        bool(expected_sectors) and initialized_sectors == expected_sectors
    )
...
def write_current_card_lcd_label(
    *,
    ...
    """Write the legacy two-line LCD label blocks to the presented card."""

    try:
        normalized_label = normalize_lcd_label(label)
        normalized_name = normalize_card_name(normalized_label.split("\n", 1)[0])
    ...
    def _write(mfrc, uid, rfid):
        ...
        writer_errors = writer_result.get("errors") or []
        errors.extend(f"writer {error}" for error in writer_errors)
        if errors:
            return {
                "error": "Unable to write RFID card label",
                "rfid": rfid,
                "errors": errors,
                "writer": writer_result,
            }
        tag.lcd_label = normalized_label
        tag.command_card_name = normalized_name
        tag.command_card_metadata = {}
        tag.command_payload_digest = ""
        tag.command_template = None
        tag.command_provenance_key = ""
        tag.command_result_digest = ""
        update_fields = {
            "command_card_name",
...
def write_current_card_command(
    *,
    name: str,
    command: str,
    ...
    """Write a suite command payload and clear result blocks on the presented card."""

    try:
        payloads, metadata = build_command_card_blocks(
            name=name,
            command=command,
            params=params,
            sigils=sigils,
        ...
        logger.warning("RFID command-card payload build failed", exc_info=True)
        return {"error": "Invalid RFID command card payload"}
    ...
    def _write(mfrc, uid, rfid):
        ...
        if errors:
            return {
                "error": "Unable to write RFID command card",
                "rfid": rfid,
                "errors": errors,
                "writer": writer_result,
            }
        tag.command_card_name = normalized_name
        tag.command_card_metadata = metadata.as_dict()
        tag.command_payload_digest = payload_digest
        tag.command_template = None
        tag.command_provenance_key = metadata.provenance_key
        tag.command_result_digest = ""
        tag.lcd_label = normalized_name
        update_fields = {
            "command_card_name",
            "command_card_metadata",
            "command_payload_digest",
...
def write_current_card_command_result(
    *,
    ...
    """Persist command execution result data to the presented command card."""

    try:
        encoded, digest, stored_payload = encode_result_payload(
            result_payload,
    ...
    payloads = {
        block: encoded[index * 16 : index * 16 + 16]
        for index, block in enumerate(blocks)
    }
    normalized_expected = RFID.normalize_code(expected_rfid)

    def _write(mfrc, uid, rfid):
        normalized_rfid = RFID.normalize_code(rfid)
        if normalized_expected and normalized_rfid != normalized_expected:
            return {
                "error": "Presented RFID does not match command execution card",
                "rfid": normalized_rfid,
                "expected_rfid": normalized_expected,
            }
        tag, _created = RFID.register_scan(normalized_rfid, kind=RFID.CLASSIC)
        errors: list[str] = []
        for block, data in payloads.items():
            success, _used_key = _write_block_with_candidates(
                mfrc,
        ...
        if errors:
            return {
                "error": "Unable to write RFID command result",
                "rfid": normalized_rfid,
                "errors": errors,
            }
        tag.command_result_digest = digest
...
def set_current_card_trait(
    *,
    ...
    def _write(mfrc, uid, rfid):
        ...
        if writer_errors:
            return {
                "error": "Unable to write RFID writer metadata",
                "rfid": rfid,
                "errors": [f"writer {error}" for error in writer_errors],
                "writer": writer_result,
            }
        ...
        if errors:
            return {
                "error": "Unable to write RFID trait",
                "rfid": rfid,
                "errors": errors,
            }
...
def _build_tag_response(
    ...
    result = {
        ...
        "name_key": getattr(tag, "name_key", ""),
        "generated_label": getattr(tag, "generated_label", ""),
        "display_label": tag.display_label,
        "lcd_label": getattr(tag, "lcd_label", ""),
        "card_name": getattr(tag, "command_card_name", ""),
        "command_provenance_key": getattr(tag, "command_provenance_key", ""),
        "created": created,
        "color": tag.color,
        "allowed": allowed,
    ...
    if getattr(tag, "command_card_metadata", None):
        result["command_metadata"] = tag.command_card_metadata
    if action_details is not None:
        result["action_output"] = action_details
    status_text = "OK" if allowed else "BAD"
    color_word = (tag.color or "").upper()
    notify_async(
        f"RFID {tag.display_label} {status_text}".strip(),
        f"{rfid} {color_word}".strip(),
    )
    return result
...
def _build_error_payload(exc: Exception) -> dict:
    ...
    payload = {"error": "RFID scanner unavailable"}
    errno_value = getattr(exc, "errno", None)
    if errno_value is not None:
        payload["errno"] = errno_value
    message = str(exc).lower()
    if errno_value in {errno.EBUSY, errno.EAGAIN} or any(
        phrase in message for phrase in _BUSY_ERROR_PHRASES
    ):
        payload["busy"] = True
        payload["reason"] = "RFID scanner busy"
    return payload
...
def _save_tag_layout_metadata(tag, metadata: dict[str, Any]) -> None:
    updates: set[str] = set()
    if "command_metadata" not in metadata:
        card_name = normalize_card_name(metadata.get("card_name", ""))
        if getattr(tag, "command_card_name", "") != card_name:
            tag.command_card_name = card_name
            updates.add("command_card_name")
        for field, empty_value in {
            ...
            if getattr(tag, field, empty_value) != empty_value:
                setattr(tag, field, empty_value)
                updates.add(field)
    ...
    if "writer" in metadata:
        writer = (
            metadata.get("writer") if isinstance(metadata.get("writer"), dict) else {}
        )
...
def _apply_transport_metadata_to_result(metadata: dict[str, Any], result: dict) -> None:
    if "card_name" in metadata:
        result["card_name"] = str(metadata.get("card_name") or "").strip()[:16]
    if "command_metadata" in metadata:
        command_metadata = metadata["command_metadata"]
        result["command_metadata"] = command_metadata
        result["command_provenance_key"] = str(
            (
                command_metadata.get("provenance_key")
        ...
        result.pop("command_metadata", None)
        result.pop("command_provenance_key", None)
    if "lcd_label" in metadata:
...
def _transport_block_numbers() -> tuple[int, ...]:
    return (
        COMMAND_CARD_NAME_BLOCK,
        COMMAND_METADATA_BLOCK,
        sector_block(WRITER_SECTOR, WRITER_ID_BLOCK_OFFSET),
        sector_block(WRITER_SECTOR, WRITER_DATE_BLOCK_OFFSET),
    )
...
def _read_transport_layout(mfrc, uid: list[int]) -> dict[str, Any]:
    ...
    with _suppress_expected_mifare_auth_logs():
    ...
    present_blocks = {entry["block"] for entry in dump}
    if not required_blocks.issubset(present_blocks):
        return {}
    metadata = decode_transport_metadata(dump)
    from apps.cards.command_layout import decode_command_metadata

    command_metadata = decode_command_metadata(
        metadata.get("command_metadata_block", [])
    )
    if command_metadata.valid:
        metadata["command_metadata"] = command_metadata.as_dict()
    return metadata
...
def _command_dump_ready_for_tag(tag, dump: list[dict[str, Any]]) -> bool:
    """Return whether the dump has enough command blocks for this tag."""

    present_blocks = _dump_block_numbers(dump)
    if not {
        COMMAND_CARD_NAME_BLOCK,
        COMMAND_METADATA_BLOCK,
    }.issubset(present_blocks):
        return False
    metadata_entry = next(
        (
            entry
            for entry in dump
            if isinstance(entry, dict) and entry.get("block") == COMMAND_METADATA_BLOCK
        ),
        None,
    )
    if not metadata_entry:
        return False
    metadata = decode_command_metadata(metadata_entry.get("data"))
    if not metadata.valid:
        return False
    required_blocks = {COMMAND_CARD_NAME_BLOCK, COMMAND_METADATA_BLOCK}
    required_blocks.update(command_data_blocks()[: metadata.command_block_count])
    if str(getattr(tag, "command_result_digest", "") or "").strip():
...
def _read_deep_classic_tag_data(mfrc, tag, uid: list[int], result: dict) -> dict:
            ...
            try:
                ...
                for index, (key_value, key_bytes) in enumerate(key_candidates["A"]):
                    ...
                    if status == mfrc.MI_OK:
                        ...
                        if index > 0:
                            key_candidates["A"].insert(
                                0, key_candidates["A"].pop(index)
                            )
                        break
                ...
                if status != mfrc.MI_OK:
                    for index, (key_value, key_bytes) in enumerate(key_candidates["B"]):
                        ...
                        if status == mfrc.MI_OK:
                            ...
                            used_value = key_value
                            if index > 0:
                                key_candidates["B"].insert(
                                    0, key_candidates["B"].pop(index)
                                )
                            break
                ...
                if status == mfrc.MI_OK:
                    ...
                    if read_state == mfrc.MI_OK and data is not None:
                        ...
                        if _command_dump_ready_for_tag(tag, dump):
                            break
    ...
    if _transport_dump_is_complete(dump):
        ...
        _apply_transport_metadata_to_result(metadata, result)
        _save_tag_layout_metadata(tag, metadata)
    command_card = decode_command_card_from_dump(dump)
    if command_card is not None:
        result["command_card"] = command_card.as_dict()
    else:
    ...
    try:
        ...
        if tag.kind == RFID.CLASSIC:
            ...
            if _deep_read_enabled:
                return _read_deep_classic_tag_data(
                    strategy.mfrc, tag, uid_bytes, result
                )
        return result
    except Exception as exc:  # pragma: no cover - hardware dependent
        if rfid:
...
def validate_rfid_value(
    ...
    try:
        ...
        logger.exception("RFID value validation failed")
