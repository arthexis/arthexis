...
class RFIDAttempt(Entity):
    ...
    def record_attempt(
        ...
        *,
        ...
        try:
            from apps.cards.watchlists import record_watchlist_events_for_attempt

            record_watchlist_events_for_attempt(attempt)
        except Exception:
            logger.warning(
                "RFID watchlist evaluation failed for attempt %s",
                attempt.pk,
                exc_info=True,
            )
        return attempt
