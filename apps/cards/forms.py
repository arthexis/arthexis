...
class CardFacePreviewForm(forms.Form):
    ...
    overlay_one_font_size = forms.IntegerField(
        label=_("Overlay 1 size"), min_value=1, required=False
    )
    overlay_two_font_size = forms.IntegerField(
        label=_("Overlay 2 size"), min_value=1, required=False
    )
    ...
    def __init__(self, *args, fonts=None, sigils=None, **kwargs):
        ...
        for token in self.sigil_tokens:
            ...
            self.fields[field_name] = forms.CharField(
                label=f"[{token}]",
                required=False,
                help_text=_("Manual sigil value for preview"),
            )
