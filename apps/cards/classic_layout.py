...
CARD_NAME_SECTOR = 0
CARD_NAME_BLOCK_OFFSET = 1
COMMAND_METADATA_SECTOR = 0
COMMAND_METADATA_BLOCK_OFFSET = 2
WRITER_SECTOR = 1
...
def normalize_card_name(value: object) -> str:
    text = clean_ascii_text(value).strip()
    encoded = text.encode("ascii", errors="ignore")
    if len(encoded) > CARD_NAME_BYTES:
        raise CardLayoutError("card name must fit in 16 ASCII bytes")
    return encoded.decode("ascii")


def encode_card_name(value: object) -> list[int]:
    return encode_fixed_ascii(normalize_card_name(value), CARD_NAME_BYTES)


def decode_card_name(data: list[int] | tuple[int, ...] | bytes | bytearray) -> str:
    return decode_fixed_ascii(list(data)[:CARD_NAME_BYTES])
...
def decode_transport_metadata(dump: object) -> dict[str, Any]:
    ...
    name_block = blocks.get(
        sector_block(CARD_NAME_SECTOR, CARD_NAME_BLOCK_OFFSET),
        zero_block(),
    )
    metadata_block = blocks.get(
        sector_block(COMMAND_METADATA_SECTOR, COMMAND_METADATA_BLOCK_OFFSET),
        zero_block(),
    )
    card_name = decode_card_name(name_block)
    if bytes(metadata_block[:4]) == _COMMAND_LAYOUT_MAGIC:
        lcd_label = card_name
    else:
        lcd_label = decode_lcd_label(name_block + metadata_block)
    metadata = {
        "card_name": card_name,
        "lcd_label": lcd_label,
        "command_metadata_block": metadata_block,
    }
    writer_id = decode_fixed_ascii(
        blocks.get(
            sector_block(WRITER_SECTOR, WRITER_ID_BLOCK_OFFSET),
            zero_block(),
        )[:WRITER_ID_BYTES]
    )
    writer_date = decode_writer_date(
        blocks.get(
            sector_block(WRITER_SECTOR, WRITER_DATE_BLOCK_OFFSET),
            zero_block(),
        )
    )
    if writer_id or writer_date:
        metadata["writer"] = {"id": writer_id, "written_at": writer_date}
    return metadata
