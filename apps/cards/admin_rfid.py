...
from apps.cards.models import RFID, RFIDAttempt, RFIDGeneratedLabel
...
from apps.cards.rfid_names import rfid_name_key
...
def _rfid_needs_generated_label_sync(tag: RFID) -> bool:
    expected_key = rfid_name_key(tag.rfid)
    if not tag.generated_label or tag.name_key != expected_key:
        return True

    allocation = (
        RFIDGeneratedLabel.objects.filter(name_key=expected_key)
        .only("generated_label")
        .first()
    )
    if allocation is None:
        return False
    return tag.generated_label != allocation.generated_label
...
class RFIDForm(forms.ModelForm):
    """RFID admin form with scanner-friendly widgets."""
...
class RFIDAdmin(EntityModelAdmin, ImportExportModelAdmin):
    ...
    actions = [
        ...
        "enable_public_usage",
        "disable_public_usage",
        "rotate_public_usage_token",
        "merge_rfids",
    ...
    readonly_fields = (
        ...
        "qr_test_link",
        "public_usage_link",
        "public_usage_qr_preview",
        "public_usage_qr_actions",
        "public_token",
    ...
    label.admin_order_field = "generated_label"
    label.short_description = "Display label"

    @admin.display(description=_("Public usage"), boolean=True)
    def public_usage_state(self, obj):
        return bool(obj.public_token_enabled and obj.public_token)

    @admin.display(description=_("Public usage link"))
    def public_usage_link(self, obj):
        if not obj.public_token:
            return _("No public token")
        url = obj.public_usage_path()
        return format_html('<a href="{}">{}</a>', url, url)

    @admin.display(description=_("Public usage QR"))
    def public_usage_qr_preview(self, obj):
        if not obj.public_token:
            return _("No public token")
        data_uri = obj.public_usage_qr_data_uri("/")
        if not data_uri:
    ...
    @admin.display(description=_("Public usage QR actions"))
    def public_usage_qr_actions(self, obj):
        if not obj.pk:
            return ""
        if not obj.public_token:
            return _("No public token")
        url = reverse("admin:cards_rfid_public_usage_qr", args=[obj.pk])
        return format_html(
            '<a class="button" href="{}">{}</a>',
            url,
            _("Print or download QR"),
        )

    def public_usage_qr_view(self, request, object_id):
        ...
        if not self.has_view_or_change_permission(request, tag):
            raise PermissionDenied
        if not tag.public_token or not tag.public_token_enabled:
            self.message_user(
                request,
                _("Enable public usage before printing this RFID QR code."),
                level=messages.WARNING,
            )
            return redirect("admin:cards_rfid_change", tag.pk)

        public_url = request.build_absolute_uri(tag.public_usage_path())
        context = {
            **self.admin_site.each_context(request),
            "title": _("Public RFID Usage QR"),
    ...
    @admin.action(description=_("Enable public usage QR"))
    def enable_public_usage(self, request, queryset):
        updated = 0
        for tag in queryset:
        ...
        if updated:
            self.message_user(
                request,
                ngettext(
                    "Enabled public usage QR for %(count)d RFID.",
                    "Enabled public usage QR for %(count)d RFIDs.",
                    updated,
                )
                % {"count": updated},
                level=messages.SUCCESS,
    ...
    @admin.action(description=_("Disable public usage QR"))
    def disable_public_usage(self, request, queryset):
        updated = queryset.update(public_token_enabled=False)
        if updated:
            self.message_user(
                request,
                ngettext(
                    "Disabled public usage QR for %(count)d RFID.",
                    "Disabled public usage QR for %(count)d RFIDs.",
                    updated,
                )
                % {"count": updated},
                level=messages.SUCCESS,
    ...
    @admin.action(description=_("Rotate public usage token"))
    def rotate_public_usage_token(self, request, queryset):
        updated = 0
        for tag in queryset:
        ...
        if updated:
            self.message_user(
                request,
                ngettext(
                    "Rotated public usage token for %(count)d RFID.",
                    "Rotated public usage tokens for %(count)d RFIDs.",
                    updated,
                )
                % {"count": updated},
                level=messages.SUCCESS,
    ...
    def copy_rfids(self, request, queryset):
        ...
        source = queryset.prefetch_related("energy_accounts").first()
        if source is None:
            self.message_user(
                request,
        ...
        if "apply" in request.POST:
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    new_tag.ensure_generated_label()
    ...
    def _render_card_labels(
        ...
        queryset = queryset.order_by("label_id")
        ...
        def draw_label(pdf_canvas, tag, origin_x, origin_y):
            ...
            primary_label = tag.display_label or str(tag.label_id or "")
            ...
            if tag.generated_label:
                details.append(_("Generated label: %s") % tag.generated_label)
            if tag.custom_label:
        ...
        tags = list(queryset)
        for tag in tags:
            if _rfid_needs_generated_label_sync(tag):
    ...
    def get_urls(self):
        ...
        custom = [
            ...
            path(
                "<path:object_id>/public-usage-qr/",
                self.admin_site.admin_view(self.public_usage_qr_view),
                name="cards_rfid_public_usage_qr",
            ),
        ]
        return custom + urls
