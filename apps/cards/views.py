...
def scan_next(request):
    ...
    allow_control_anonymous_get = (
        role_name == "Control" and request.method == "GET"
    )

    user = request.user
    wants_json = _request_wants_json(request) or request.method == "POST"
    if not user.is_authenticated and not allow_control_anonymous_get:
        if wants_json:
            return JsonResponse({"error": "Authentication required"}, status=401)
        return redirect_to_login(request.get_full_path(), reverse("pages:login"))
    if not allow_control_anonymous_get:
        if not (
            user.is_staff
