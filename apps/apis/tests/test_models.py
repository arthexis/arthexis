...
def test_apiexplorer_natural_key_roundtrip_regression() -> None:
    ...
    api = APIExplorer.objects.create(name="Evergo API", base_url="https://portal.example.com")

    assert api.natural_key() == ("Evergo API",)
    assert APIExplorer.objects.get_by_natural_key("Evergo API") == api
