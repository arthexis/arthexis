...
class FetchFromGitHubMixin(DjangoObjectActions):
    ...
    def get_dashboard_actions(self, request):
        actions = list(getattr(self, "dashboard_actions", []))
        if not request.user.has_perms(REPOSITORY_WORK_VIEW_PERMISSIONS):
            actions = [
                action_name
                for action_name in actions
                if action_name != "repository_work_dashboard"
            ]
        return actions
    ...
    repository_work_dashboard.label = _("Work")
    repository_work_dashboard.short_description = _("Work")
...
class RepositoryIssueAdmin(
    ...
    dashboard_actions = ["repository_work_dashboard"]
...
@admin.register(RepositoryWorkAssignment)
class RepositoryWorkAssignmentAdmin(admin.ModelAdmin):
    retarget_identity_fields = ("repository_id", "target_type", "number", "node_id")
    list_display = (
        "repository",
        "target_type",
        "number",
    ...
    raw_id_fields = ("repository", "node", "assigned_by")
    search_fields = (
        "repository__owner",
        "repository__name",
        "number",
    ...
    def delete_model(self, request, obj):
        self._mark_queryset_removed(RepositoryWorkAssignment.objects.filter(pk=obj.pk))

    def delete_queryset(self, request, queryset):
        self._mark_queryset_removed(queryset)

    def save_model(self, request, obj, form, change):
        previous = None
        if change and obj.pk:
            previous = RepositoryWorkAssignment.objects.filter(pk=obj.pk).first()

        super().save_model(request, obj, form, change)

        if previous is None or not self._retargeted(previous, obj):
            return
        self._preserve_retarget_tombstone(previous)

    def _retargeted(self, previous, obj) -> bool:
        return any(
            getattr(previous, field) != getattr(obj, field)
            for field in self.retarget_identity_fields
        )

    def _preserve_retarget_tombstone(self, previous) -> None:
        if previous.status == RepositoryWorkAssignment.Status.REMOVED:
            return
        tombstone, _created = RepositoryWorkAssignment.objects.update_or_create(
            ...
            defaults={
                ...
                "reason": "Retargeted in the repository work assignment admin.",
                "assigned_by": previous.assigned_by,
            },
        )
        RepositoryWorkAssignment.objects.filter(pk=tombstone.pk).update(
            assigned_at=previous.assigned_at,
            updated_at=timezone.now(),
        )

    def _mark_queryset_removed(self, queryset) -> None:
        queryset.update(
            patchwork_authorized=False,
            status=RepositoryWorkAssignment.Status.REMOVED,
            updated_at=timezone.now(),
...
@admin.register(RepositoryWorkNodeSnapshot)
class RepositoryWorkNodeSnapshotAdmin(admin.ModelAdmin):
    list_display = ("node", "reported_at", "updated_at", "upstream_url")
    raw_id_fields = ("node",)
    readonly_fields = ("updated_at",)
    search_fields = ("node__hostname", "node__public_endpoint", "upstream_url")


@admin.register(GitHubRepository)
class GitHubRepositoryAdmin(FetchFromGitHubMixin, admin.ModelAdmin):
    dashboard_actions = ["repository_work_dashboard", "setup_token"]
    ...
    def get_dashboard_actions(self, request):
        actions = list(super().get_dashboard_actions(request))
        if not self._can_setup_token(request):
            actions = [
                action_name for action_name in actions if action_name != "setup_token"
            ]
        return actions
