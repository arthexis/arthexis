...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        ...
        status_parser = subparsers.add_parser(
            "status",
            help="Report readiness plus queued and active monitor items.",
        )
        status_parser.add_argument(
            "--limit",
            type=int,
            default=10,
            help="Maximum number of recent monitor items to include.",
        )
    ...
    def _run_action(self, action: str, options: dict[str, Any]) -> Any:
        ...
        if action == "status":
            return github_monitor.monitor_status(limit=int(options.get("limit") or 10))
        if action == "configure":
