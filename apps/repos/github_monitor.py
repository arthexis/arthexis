...
def _skill_model():
    if not django_apps.is_installed("apps.skills"):
        return None
    return django_apps.get_model("skills", "Skill")
...
4. Attempt local PR development only when the generated local AGENTS context identifies this node as `Node Role: Terminal`, or as `Node Role: Control` with operator-authorized patchwork on a matching assignment, and an assigned patchwork directory already exists for this node. Control nodes such as `gway-001` are report/test/upgrade evidence nodes by default, not development workers.
5. If the Terminal role, authorized Control assignment, or assigned patchwork directory is missing, perform only no-local-development oversight: inspect, gate, comments, CI, hygiene, test-plan inference, hardware evidence, upgrade/latest-version checks, and readiness reporting.
6. When local development is disallowed and code changes are required, raise or update a GitHub issue with the PR number, blockers, head SHA, required changes, likely files/tests, and upstream-worktree context.
7. Use an isolated patchwork worktree for code edits or local validation only after the local development gate passes.
8. For Linux/POSIX-only validation blocked by Windows, tell the operator to open a fresh WSL-native patchwork environment with `ARTHEXIS_PATCHWORK_DIR=$HOME/patchwork-linux` and run `prx review <pr>` there. Do not reuse the Windows patchwork checkout, `.venv` junctions, or a `/mnt/c` workspace for that fix.
9. Merge only when the deterministic gate is clean and the operator request includes merge authority.

## Guardrails

- Never reset or clean an unrelated dirty checkout.
- Do not decide away substantive review comments.
- Do not install or bootstrap a new dev instance to satisfy PR work unless an assigned patchwork directory already marks this node safe for development.
...
3. For Linux/POSIX-only install failures blocked by Windows, tell the operator to open a fresh WSL-native patchwork environment with `ARTHEXIS_PATCHWORK_DIR=$HOME/patchwork-linux` and run `prx work <issue>` there. Do not reuse the Windows patchwork checkout, `.venv` junctions, or a `/mnt/c` workspace for that fix.
4. Create a focused branch whose name includes the affected app or subsystem.
5. Open a PR linked to the issue after the fix is pushed.
6. Use the PR oversee policy to watch checks and reviewer feedback.
...
If the local development gate in the policy is not satisfied, do not create a
worktree, run local validation, patch, or install a dev instance. Use only
read-only PR oversight and raise or update a GitHub issue with upstream-worktree
context for required code changes.
...
def patchwork_dir() -> Path:
    configured = os.environ.get("ARTHEXIS_PATCHWORK_DIR") or getattr(
        settings, "ARTHEXIS_PATCHWORK_DIR", ""
    )
    if configured:
...
def local_node_role() -> str:
    env_role = str(os.environ.get("NODE_ROLE", "")).strip()
    if env_role:
        return env_role

    generated_agents = Path(settings.BASE_DIR) / "work" / "codex" / "AGENTS.md"
    node_role_pattern = re.compile(r"^\s*[-*]?\s*Node Role:\s*`?([^`]+?)`?\s*$")
    try:
        for line in generated_agents.read_text(encoding="utf-8").splitlines():
            match = node_role_pattern.match(line)
            if match:
...
def evaluate_readiness() -> dict[str, Any]:
    ...
    node_role = local_node_role()
    local_development_ready = (
        node_role.strip().lower() == "terminal" and patchwork.exists()
    )
    ...
    return {
        ...
        "ready": bool(
            ...
            and codex_available
            and gh_available
            and terminal_available
            and local_development_ready
            and git_clean
            and pr_oversee_available
        ),
    }


def _monitor_item_summary(item: GitHubMonitorItem) -> dict[str, Any]:
    return {
        "id": item.pk,
        "task": item.task.name,
        "taskDisplay": item.task.display,
        "targetType": item.target_type,
        "number": item.issue_number,
        "title": item.issue_title,
        "url": item.issue_url,
        "status": item.status,
        "queuedAt": item.queued_at.isoformat() if item.queued_at else "",
        "launchedAt": item.launched_at.isoformat() if item.launched_at else "",
        "lastActivityAt": (
            item.last_activity_at.isoformat() if item.last_activity_at else ""
        ),
        "approvedHeadSha": item.approved_head_sha,
        "terminalPidFile": item.terminal_pid_file,
        "failureMessage": item.failure_message,
    }


def monitor_status(*, limit: int = 10) -> dict[str, Any]:
    """Return a read-only status dashboard for GitHub monitor coordination."""

    status_order = {
        GitHubMonitorItem.Status.ACTIVE: 0,
        GitHubMonitorItem.Status.QUEUED: 1,
        GitHubMonitorItem.Status.FAILED: 2,
        GitHubMonitorItem.Status.TIMED_OUT: 3,
        GitHubMonitorItem.Status.CLOSED: 4,
        GitHubMonitorItem.Status.COMPLETED: 5,
        GitHubMonitorItem.Status.DISMISSED: 6,
    }
    status_priority = Case(
        *(
            When(status=status, then=Value(priority))
            for status, priority in status_order.items()
        ),
        default=Value(99),
    ...
    items = list(
        ...
        .order_by("status_priority", "queued_at", "id")[: max(int(limit or 0), 0)]
    )
    counts = dict(
        GitHubMonitorItem.objects.values_list("status").annotate(total=Count("id"))
    )
    item_counts = {
        status: counts.get(status, 0) for status in GitHubMonitorItem.Status.values
    }
    tasks = [
        {
            "name": task.name,
            ...
            "codexCommand": task.codex_command,
            "queued": task.queued_count,
            "active": task.active_count,
        }
        for task in GitHubMonitorTask.objects.select_related("repository")
        .annotate(
            queued_count=Count(
                "items",
                filter=Q(items__status=GitHubMonitorItem.Status.QUEUED),
            ),
            active_count=Count(
                "items",
                filter=Q(items__status=GitHubMonitorItem.Status.ACTIVE),
            ),
        )
        .order_by("name")
    ]
    return {
        ...
        "tasks": tasks,
        "items": [_monitor_item_summary(item) for item in items],
    }
...
def _policy_skill_markdown(item: GitHubMonitorItem) -> str:
    Skill = _skill_model()
    if Skill is None:
        return "No policy skill content is installed."
