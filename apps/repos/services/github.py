...
OCPP_CRITICAL_PRIORITY_LABEL = "priority: critical"
ISSUE_PRIORITY_LABEL_PREFIX = "priority:"
OCPP_ISSUE_SIGNAL_PATTERNS = (
    re.compile(r"\bocpp\b", re.IGNORECASE),
    re.compile(r"\bcharger\b", re.IGNORECASE),
    re.compile(r"\bcharge[\s_-]?point\b", re.IGNORECASE),
    re.compile(r"\bevse\b", re.IGNORECASE),
)
IMAGER_ISSUE_SIGNAL_PATTERNS = (
    re.compile(r"\bimager\b", re.IGNORECASE),
    re.compile(r"\bimage\s+burn\b", re.IGNORECASE),
    re.compile(r"\bburn\s+image\b", re.IGNORECASE),
    re.compile(r"\bgway\s+image\b", re.IGNORECASE),
    re.compile(r"\bbase\s+image\b", re.IGNORECASE),
)
OCPP_TEMPLATE_FIELD_PATTERN = re.compile(
    r"###\s*OCPP protocol version\s*([\s\S]*?)(?=\n#+|$)",
    re.IGNORECASE,
)
...
def fetch_repository_labels(
    *,
    ...
    endpoint = f"{API_ROOT}/repos/{owner}/{name}/labels"
...
def _issue_has_ocpp_signal(title: str | None, body: str | None) -> bool:
    title_text = title or ""
    body_text = body or ""
    template_match = OCPP_TEMPLATE_FIELD_PATTERN.search(body_text)
    if template_match:
        template_value = template_match.group(1).strip().lower()
        if template_value and not any(
            marker in template_value for marker in ("not sure", "not applicable")
        ):
            return True

    issue_text = f"{title_text}\n{OCPP_TEMPLATE_FIELD_PATTERN.sub('', body_text)}"
    return any(pattern.search(issue_text) for pattern in OCPP_ISSUE_SIGNAL_PATTERNS)


def _issue_has_priority_signal(title: str | None, body: str | None) -> bool:
    title_text = title or ""
    body_text = body or ""
    if _issue_has_ocpp_signal(title_text, body_text):
        return True
    issue_text = f"{title_text}\n{OCPP_TEMPLATE_FIELD_PATTERN.sub('', body_text)}"
    return any(pattern.search(issue_text) for pattern in IMAGER_ISSUE_SIGNAL_PATTERNS)


def is_issue_priority_label(label: str | None) -> bool:
    """Return whether ``label`` belongs to the GitHub issue priority family."""

    return str(label or "").strip().lower().startswith(ISSUE_PRIORITY_LABEL_PREFIX)


def collapse_issue_priority_labels(labels: Iterable[str] | None) -> list[str]:
    """Return labels with duplicates removed and at most one priority label.

    When multiple priority labels are supplied, the last one wins. That matches
    label replacement flows where applying a new priority should supersede the
    previous priority.
    """
    ...
    for label in labels or ():
        ...
        if is_issue_priority_label(cleaned_label):
            collapsed = [
                existing
                for existing in collapsed
                if not is_issue_priority_label(existing)
            ]

        if cleaned_label not in collapsed:
            collapsed.append(cleaned_label)
...
def _resolve_issue_labels(
    ...
    *,
    apply_ocpp_priority: bool = True,
) -> list[str]:
    deduped = collapse_issue_priority_labels(labels)
    if (
        apply_ocpp_priority
        and _issue_has_priority_signal(title, body)
        and OCPP_CRITICAL_PRIORITY_LABEL not in deduped
    ):
        deduped.append(OCPP_CRITICAL_PRIORITY_LABEL)
    return collapse_issue_priority_labels(deduped)
...
def resolve_issue_labels(
    ...
    *,
    ...
    """Return GitHub labels with the suite OCPP priority policy applied."""

    return _resolve_issue_labels(
        title,
        body,
        labels,
        apply_ocpp_priority=apply_ocpp_priority,
    )


def issue_labels_include_auto_ocpp_priority(
    title: str | None,
    body: str | None,
    labels: Iterable[str] | None = None,
) -> bool:
    """Return whether the OCPP policy adds a priority label beyond caller labels."""

    base_labels = resolve_issue_labels(title, body, labels, apply_ocpp_priority=False)
    resolved_labels = resolve_issue_labels(title, body, base_labels)
    return (
        OCPP_CRITICAL_PRIORITY_LABEL in resolved_labels
        and OCPP_CRITICAL_PRIORITY_LABEL not in base_labels
    )


def _response_mentions_issue_labels(response: requests.Response) -> bool:
    if response.status_code not in {400, 422}:
        return False

    message_parts = [getattr(response, "text", "") or ""]
    payload = _safe_json(response)
    if isinstance(payload, Mapping):
        message_parts.append(str(payload.get("message") or ""))
            ...
            for entry in errors:
                if isinstance(entry, Mapping):
                    message_parts.extend(
                        ...
                        for key in ("field", "resource", "code", "message")
                    )
                else:
                    message_parts.append(str(entry))
...
def build_issue_payload(
    ...
    *,
    ...
    title_text = title or ""
    body_text = body or ""
    payload: dict[str, object] = {"title": title_text, "body": body_text}

    resolved_labels = _resolve_issue_labels(
        title_text,
        body_text,
        labels,
        apply_ocpp_priority=apply_ocpp_priority,
...
def create_issue(
    ...
    *,
    ...
    auto_ocpp_priority = issue_labels_include_auto_ocpp_priority(title, body, labels)
    ...
    try:
        ...
        if (
            ...
            and auto_ocpp_priority
            and _response_mentions_issue_labels(response)
        ):
            logger.warning(
                "Retrying GitHub issue creation for %s/%s without auto-added "
                "priority label after label validation failed",
                owner,
                repository,
            )
            with contextlib.suppress(Exception):
            ...
            fallback_labels = resolve_issue_labels(
                title,
                body,
                labels,
                apply_ocpp_priority=False,
            )
            if fallback_labels:
...
def remove_issue_label(
    *,
    ...
    """Remove one label from an issue or pull request and return the API response."""
    ...
    headers = build_headers(token, user_agent="arthexis-runtime-reporter")
    quoted_label = quote(cleaned_label, safe="")
    url = (
        f"{API_ROOT}/repos/{owner}/{repository}/issues/"
        f"{issue_number}/labels/{quoted_label}"
    )
    response = None
    try:
    ...
    if response.status_code == 404 and ignore_missing:
        logger.info(
            "GitHub issue label was already absent for %s/%s#%s",
            owner,
            repository,
            issue_number,
        )
    ...
    logger.info(
        "GitHub issue label removed for %s/%s#%s with status %s",
        owner,
        repository,
        issue_number,
        response.status_code,
...
def reopen_issue(
    *,
    ...
    """Reopen an issue and return the API response."""
    ...
    logger.info(
        "GitHub issue %s/%s#%s reopened with status %s",
        owner,
        repository,
        issue_number,
        response.status_code,
