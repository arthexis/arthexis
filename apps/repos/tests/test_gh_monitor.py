...
def test_configure_default_monitoring_writes_feature_tasks_and_policy_skills():
    ...
    policy_markdown = "\n".join(
        Skill.objects.filter(slug__in=github_monitor.DEFAULT_POLICY_SKILLS)
        .order_by("slug")
        .values_list("markdown", flat=True)
    )
    assert "ARTHEXIS_PATCHWORK_DIR=$HOME/patchwork-linux" in policy_markdown
    assert "Node Role: Control` with operator-authorized patchwork" in policy_markdown
    assert "prx review <pr>" in policy_markdown
    assert "prx work <issue>" in policy_markdown
    assert "/mnt/c" in policy_markdown


@pytest.mark.django_db
def test_configure_default_monitoring_skips_policy_skills_without_skills_app(
    monkeypatch,
):
    original_is_installed = github_monitor.django_apps.is_installed

    def fake_is_installed(app_label):
        if app_label == "apps.skills":
            return False
        return original_is_installed(app_label)

    monkeypatch.setattr(github_monitor.django_apps, "is_installed", fake_is_installed)
    ...
    assert result["policy_skills"] == {
        "write": True,
        "skills": [],
        "skipped": "apps.skills is not installed",
    }
    assert GitHubRepository.objects.get(owner="octo", name="demo")
    assert GitHubMonitorTask.objects.filter(name="install-health").exists()
...
@pytest.mark.django_db
def test_monitor_lcd_frames_prioritize_gway_prs():
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    issue_task = GitHubMonitorTask.objects.get(name="install-health")
    GitHubMonitorItem.objects.create(
        task=issue_task,
        fingerprint=hashlib.sha256(b"issue").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.ISSUE,
        issue_number=7,
    ...
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"general-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=8,
        issue_title="Refine docs workflow",
        issue_url="https://github.example/pulls/8",
        status=GitHubMonitorItem.Status.QUEUED,
    )
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"gway-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=9,
        issue_title="Fix gway USB LCD",
        issue_url="https://github.example/pulls/9",
        status=GitHubMonitorItem.Status.QUEUED,
    )

    frames = github_monitor.monitor_lcd_frames(limit=2)

    assert frames == [
        ("GWAY PR 9", "Fix gway USB LCD"),
        ("NEXT PR 8", "Refine docs work"),
    ]


@pytest.mark.django_db
def test_monitor_lcd_frames_prioritize_active_items_before_queue():
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    issue_task = GitHubMonitorTask.objects.get(name="install-health")
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"queued-gway-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=9,
        issue_title="Fix gway USB LCD",
        issue_url="https://github.example/pulls/9",
    ...
    GitHubMonitorItem.objects.create(
        task=issue_task,
        fingerprint=hashlib.sha256(b"active-issue").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.ISSUE,
        issue_number=6,
        issue_title="Install health check active",
        issue_url="https://github.example/issues/6",
        status=GitHubMonitorItem.Status.ACTIVE,
    )
    ...
    assert frames == [
        ("ACTIVE ISS 6", "Install health c"),
        ("GWAY PR 9", "Fix gway USB LCD"),
    ]


@pytest.mark.django_db
def test_write_monitor_lcd_writes_and_clears_channel(settings, tmp_path):
    settings.BASE_DIR = tmp_path
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"lcd-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=10,
        issue_title="Improve gway status",
        issue_url="https://github.example/pulls/10",
        status=GitHubMonitorItem.Status.QUEUED,
    )

    result = github_monitor.write_monitor_lcd(force=True)

    lock_file = tmp_path / ".locks" / github_monitor.GITHUB_MONITOR_LCD_LOCK_NAME
    assert result["written"] == 1
    assert lock_file.read_text(encoding="utf-8").splitlines()[:2] == [
        "GWAY PR 10",
        "Improve gway sta",
    ]

    GitHubMonitorItem.objects.update(status=GitHubMonitorItem.Status.COMPLETED)
    result = github_monitor.write_monitor_lcd(force=True)
...
@pytest.mark.django_db
def test_write_monitor_lcd_accepts_legacy_lcd_feature_lock(settings, tmp_path):
    settings.BASE_DIR = tmp_path
    lock_dir = tmp_path / ".locks"
    lock_dir.mkdir()
    (lock_dir / LCD_LEGACY_FEATURE_LOCK).touch()
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"legacy-lcd-lock").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=14,
        issue_title="Show gway PR queue",
        issue_url="https://github.example/pulls/14",
        status=GitHubMonitorItem.Status.QUEUED,
    )

    result = github_monitor.write_monitor_lcd()

    assert result["written"] == 1
    assert (lock_dir / github_monitor.GITHUB_MONITOR_LCD_LOCK_NAME).exists()


@pytest.mark.django_db
def test_gh_monitor_lcd_command_writes_json(settings, tmp_path):
    settings.BASE_DIR = tmp_path
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"lcd-command-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=12,
        issue_title="Show gway PR queue",
        issue_url="https://github.example/pulls/12",
        status=GitHubMonitorItem.Status.QUEUED,
    )
    out = StringIO()

    call_command("gh_monitor", "--json", "lcd", "--force", stdout=out)

    payload = json.loads(out.getvalue())
    assert payload["written"] == 1
    assert payload["frames"] == [["GWAY PR 12", "Show gway PR que"]]


@pytest.mark.django_db
def test_gh_monitor_lcd_command_preserves_explicit_zero_limit(settings, tmp_path):
    settings.BASE_DIR = tmp_path
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"lcd-command-zero").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=13,
        issue_title="Show gway PR queue",
        issue_url="https://github.example/pulls/13",
        status=GitHubMonitorItem.Status.QUEUED,
    )
    out = StringIO()

    call_command("gh_monitor", "--json", "lcd", "--force", "--limit", "0", stdout=out)

    payload = json.loads(out.getvalue())
    assert payload["written"] == 0
    assert payload["frames"] == []
    assert not (
        tmp_path / ".locks" / github_monitor.GITHUB_MONITOR_LCD_LOCK_NAME
    ).exists()


@pytest.mark.django_db
def test_monitor_cycle_projects_queue_to_lcd_when_runtime_is_enabled(
    settings, tmp_path, monkeypatch
):
    settings.BASE_DIR = tmp_path
    lock_dir = tmp_path / ".locks"
    lock_dir.mkdir()
    (lock_dir / "lcd_screen.lck").touch()
    _enable_monitor_feature()
    _configure_defaults(include_pr=True)
    pr_task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    GitHubMonitorItem.objects.create(
        task=pr_task,
        fingerprint=hashlib.sha256(b"cycle-lcd-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=11,
        issue_title="Prepare gway Kindle handling",
        issue_url="https://github.example/pulls/11",
        status=GitHubMonitorItem.Status.QUEUED,
    )
    monkeypatch.setattr(github_monitor, "sync_monitor_items", lambda: {"matched": 0})
    monkeypatch.setattr(github_monitor, "desktop_ui_enabled", lambda: False)

    result = github_monitor.run_monitor_cycle()

    assert result["lcd"]["written"] == 1
    assert (lock_dir / "lcd-github").read_text(encoding="utf-8").splitlines()[:2] == [
        "GWAY PR 11",
        "Prepare gway Kin",
    ]
...
def test_gh_monitor_command_configure_and_evaluate(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(github_monitor, "local_node_role", lambda: "Terminal")
    ...
    assert result["local_development_ready"] is True
    assert result["configured_tasks"] == 3
    assert result["patchwork_dir_exists"] is True


@pytest.mark.django_db
def test_gh_monitor_status_includes_readiness_tasks_and_items(monkeypatch, tmp_path):
    _enable_monitor_feature()
    _configure_defaults(include_pr=True)
    monkeypatch.setattr(
        github_monitor.github_service, "get_github_issue_token", lambda: "token"
    ...
    monkeypatch.setattr(github_monitor, "local_node_role", lambda: "Terminal")
    monkeypatch.setattr(github_monitor.shutil, "which", lambda command: command)
    task = GitHubMonitorTask.objects.get(name="approved-pr-oversee")
    GitHubMonitorItem.objects.create(
        task=task,
        fingerprint=hashlib.sha256(b"status-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=42,
        issue_title="Improve PR tools",
        issue_url="https://github.example/pulls/42",
        status=GitHubMonitorItem.Status.QUEUED,
    )
    out = StringIO()

    call_command("gh_monitor", "--json", "status", "--limit", "5", stdout=out)

    result = json.loads(out.getvalue())
    assert result["readiness"]["ready"] is True
    assert result["itemCounts"]["queued"] == 1
    assert result["items"][0]["number"] == 42
    assert any(task["name"] == "approved-pr-oversee" for task in result["tasks"])


@pytest.mark.django_db
def test_gh_monitor_status_prioritizes_active_items_before_limit(monkeypatch, tmp_path):
    _enable_monitor_feature()
    _configure_defaults(include_pr=True)
    monkeypatch.setattr(
        github_monitor.github_service, "get_github_issue_token", lambda: "token"
    ...
    for index in range(5):
        GitHubMonitorItem.objects.create(
            task=task,
            fingerprint=hashlib.sha256(f"closed-{index}".encode()).hexdigest(),
            target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
            issue_number=100 + index,
            issue_title=f"Closed PR {index}",
            issue_url=f"https://github.example/pulls/{100 + index}",
            status=GitHubMonitorItem.Status.COMPLETED,
            queued_at=now - timedelta(minutes=index),
        )
    GitHubMonitorItem.objects.create(
        task=task,
        fingerprint=hashlib.sha256(b"active-pr").hexdigest(),
        target_type=GitHubMonitorTask.TargetType.PULL_REQUEST,
        issue_number=42,
        issue_title="Active PR",
        issue_url="https://github.example/pulls/42",
        status=GitHubMonitorItem.Status.ACTIVE,
    ...
    result = json.loads(out.getvalue())
    assert result["items"][0]["number"] == 42
    assert 42 in {item["number"] for item in result["items"]}
...
@pytest.mark.django_db
def test_gh_monitor_evaluate_requires_terminal_role(monkeypatch, tmp_path):
    _enable_monitor_feature()
    github_monitor.configure_default_monitoring(repository="octo/demo", write=True)
    monkeypatch.setattr(
        github_monitor.github_service, "get_github_issue_token", lambda: "token"
    ...
    monkeypatch.setattr(github_monitor, "_terminal_launcher_available", lambda: True)
    monkeypatch.setattr(github_monitor, "_git_status_clean", lambda: True)
    monkeypatch.setattr(github_monitor, "local_node_role", lambda: "Satellite")
    monkeypatch.setattr(github_monitor.shutil, "which", lambda command: command)

    result = github_monitor.evaluate_readiness()

    assert result["node_role"] == "Satellite"
    assert result["local_development_ready"] is False
    assert result["ready"] is False
...
def test_local_node_role_uses_structured_agents_line(monkeypatch, tmp_path: Path):
    monkeypatch.delenv("NODE_ROLE", raising=False)
    monkeypatch.setattr(settings, "BASE_DIR", tmp_path)
    monkeypatch.setattr(settings, "NODE_ROLE", "")

    agents_path = tmp_path / "work" / "codex" / "AGENTS.md"
    agents_path.parent.mkdir(parents=True)
    agents_path.write_text(
        "Policy prose with Node Role: Terminal should be ignored.\n"
        "- Node Role: `Terminal`\n",
        encoding="utf-8",
    )

    assert github_monitor.local_node_role() == "Terminal"


def test_local_node_role_prefers_role_lock_over_settings(monkeypatch, tmp_path: Path):
    monkeypatch.delenv("NODE_ROLE", raising=False)
    monkeypatch.setattr(settings, "BASE_DIR", tmp_path)
    monkeypatch.setattr(settings, "NODE_ROLE", "Terminal")

    role_lock = tmp_path / ".locks" / "role.lck"
    role_lock.parent.mkdir(parents=True)
    role_lock.write_text("Satellite", encoding="utf-8")

    assert github_monitor.local_node_role() == "Satellite"
