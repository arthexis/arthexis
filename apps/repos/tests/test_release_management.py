...
@pytest.mark.django_db
def test_release_management_binary_issue_create_labels_ocpp_fallback(monkeypatch):
    """Binary issue creation should preserve OCPP priority labeling."""
    ...
    def fake_run(self, args: list[str]) -> str:
        captured["args"] = args
        return "https://github.com/octo/demo/issues/45"
    ...
    assert result == "https://github.com/octo/demo/issues/45"
...
@pytest.mark.django_db
def test_release_management_suite_issue_create_lets_service_add_ocpp_priority(
    monkeypatch,
):
    """Suite issue creation should not pre-resolve the auto OCPP label."""
    ...
    class DummyCreateResponse:
        def json(self) -> dict[str, str]:
            return {"html_url": "https://github.com/octo/demo/issues/45"}
    ...
    assert result == "https://github.com/octo/demo/issues/45"
    assert captured["owner"] == "octo"
    assert captured["repository"] == "demo"
    assert captured["token"] == "token-1"
    assert "labels" not in captured


@pytest.mark.django_db
def test_release_management_binary_issue_create_retries_without_missing_auto_label(
    monkeypatch,
):
    """Binary issue creation should not fail when the auto OCPP label is absent."""
    ...
    def fake_run(self, args: list[str]) -> str:
        calls.append(args)
        if "--label" in args:
            raise ReleaseManagementError("could not add label: priority: critical")
        return "https://github.com/octo/demo/issues/46"
    ...
    assert result == "https://github.com/octo/demo/issues/46"
    assert calls == [
        [
            "issue",
            "create",
