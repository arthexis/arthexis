...
def extract_label_names(item: Mapping[str, object]) -> list[str]:
    """Return unique GitHub label names from an issue or pull request payload."""

    labels = item.get("labels")
    if not isinstance(labels, Iterable) or isinstance(labels, (str, bytes, Mapping)):
        return []
...
def fetch_issue_label_names() -> list[str]:
    """Return existing label names for the active issue repository."""
