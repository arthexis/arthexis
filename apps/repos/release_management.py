...
class ReleaseManagementClient:
    ...
    def create_issue(self, repository: RepositoryRef, *, title: str, body: str) -> str:
        ...
        token = self._resolve_token()
        labels = github_service.resolve_issue_labels(title, body)
        auto_ocpp_priority = github_service.issue_labels_include_auto_ocpp_priority(
            title,
            body,
        )
        if not self._should_use_binary_first() and token and self._can_use_suite_api():
        ...
        try:
            return self._run_gh(command)
        except ReleaseManagementError as exc:
            if auto_ocpp_priority and "label" in str(exc).lower():
                return self._run_gh(base_command)
            raise
