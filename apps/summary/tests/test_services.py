...
from apps.summary import tasks as task_services
...
def test_build_summary_prompt_excludes_dedicated_resource_screens() -> None:
    ...
    assert "OK, WARN, ERROR, CHECK, FIX, or WAIT" in prompt
    assert "Keep a visible space between the count and operator word" in prompt
...
def test_get_summary_sources_falls_back_when_groups_are_unknown(
    ...
    assert [source.group for source in sources] == [
        "logs",
        "state",
        "journal",
        "journal",
...
def test_parse_screens_preserves_preamble_before_screen() -> None:
    output = "Here are the screens:\nSCREEN 1:\nIssue\n1 ln/5m ERROR"

    assert services.parse_screens(output) == [
        ("Here are the screens:", "Issue 1 ln/5m ERROR")
    ]
...
def test_normalize_screens_adds_window_to_repeat_counts() -> None:
    ...
    assert frames == [("refresh usb lcd ", "10x/60m       OK")]
...
def test_deterministic_summary_round_trips_thirty_two_cell_buffer() -> None:
    from apps.summary.tasks import LocalLLMSummarizer
...
def test_summary_context_window_respects_configured_bounds_under_heat(
    ...
    monkeypatch.setattr(
        services, "get_summary_context_window_bounds", lambda: (10, 120)
    )
...
def test_generation_file_target_creates_parent_and_writes_artifacts(
    monkeypatch, settings, tmp_path
) -> None:
    from apps.nodes.models import Node
    ...
    monkeypatch.setattr(services, "get_summary_config", lambda: config)
    monkeypatch.setattr(
        services,
        "resolve_summary_context_window",
        lambda: _summary_window(minutes=5),
    ...
    text_path = tmp_path / "logs" / "summary" / "latest.txt"
    json_path = tmp_path / "logs" / "summary" / "latest.json"
    assert result == "wrote-file:2"
    assert text_path.exists()
    assert json_path.exists()
    assert "Raw output:\nSCREEN 1:\nIssue\n1 line error" in text_path.read_text(
        encoding="utf-8"
    )
    payload = json.loads(json_path.read_text(encoding="utf-8"))
    assert payload["window"]["label"] == "5m"
    assert payload["frames"][0]["subject"] == "Issue"
    assert config.last_prompt == "prompt"
    assert config.last_output == "SCREEN 1:\nIssue\n1 line error"
    assert config.last_output_file_path == str(text_path)
    assert "last_output_file_path" in config.update_fields
    assert not (lock_dir / "lcd-summary").exists()


def test_resolve_summary_output_file_path_rejects_absolute_paths(tmp_path) -> None:
    config = SimpleNamespace(output_file_path=str(tmp_path / "outside.txt"))

    with pytest.raises(ValueError, match="must be relative"):
        services.resolve_summary_output_file_path(config, base_dir=tmp_path)


def test_resolve_summary_output_file_path_rejects_traversal(tmp_path) -> None:
    config = SimpleNamespace(output_file_path="../outside.txt")

    with pytest.raises(ValueError, match="cannot contain"):
        services.resolve_summary_output_file_path(config, base_dir=tmp_path)


def test_resolve_summary_output_file_path_rejects_parent_symlink_escape(
    tmp_path,
) -> None:
    outside = tmp_path / "outside"
    outside.mkdir()
    summary_dir = tmp_path / "logs" / "summary"
    summary_dir.mkdir(parents=True)
    (summary_dir / "linked").symlink_to(outside, target_is_directory=True)
    config = SimpleNamespace(output_file_path="linked/latest.txt")

    with pytest.raises(ValueError, match="escapes"):
        services.resolve_summary_output_file_path(config, base_dir=tmp_path)


def test_write_summary_output_files_stages_both_formats_before_promoting(
    monkeypatch, tmp_path
) -> None:
    class FakeConfig:
        output_file_path = "latest.txt"
        output_file_format = "both"

    text_path = tmp_path / "logs" / "summary" / "latest.txt"
    text_path.parent.mkdir(parents=True)
    text_path.write_text("old text\n", encoding="utf-8")
    original_write_temp = services._write_text_temp

    def fail_json_stage(path: Path, content: str) -> Path:
        if path.suffix == ".json":
            raise OSError("json unavailable")
        return original_write_temp(path, content)

    monkeypatch.setattr(services, "_write_text_temp", fail_json_stage)

    with pytest.raises(OSError, match="json unavailable"):
        services.write_summary_output_files(
            FakeConfig(),
            generated_at=datetime(2026, 5, 3, tzinfo=timezone.utc),
            window=_summary_window(minutes=5),
            output="SCREEN 1:\nIssue\n1 line error",
            frames=[("Issue", "1 line error")],
            base_dir=tmp_path,
        )

    assert text_path.read_text(encoding="utf-8") == "old text\n"
    assert not (tmp_path / "logs" / "summary" / "latest.json").exists()
    assert list(text_path.parent.glob("*.tmp")) == []


def test_generation_file_target_inactive_does_not_write(
    monkeypatch, settings, tmp_path
) -> None:
    from apps.nodes.models import Node
    ...
    assert not (tmp_path / "logs" / "summary" / "latest.txt").exists()


def test_generation_file_target_handles_non_oserror_write_failure(
    monkeypatch, settings, tmp_path
) -> None:
    from apps.nodes.models import Node
    ...
    class FakeConfig:
        ...
        last_output_file_path = "logs/summary/old.txt"
    ...
    monkeypatch.setattr(services, "write_summary_output_files", raise_write_failure)
    ...
    assert result == "error:file-output"
    assert config.last_prompt == "prompt"
    assert config.last_output == "SCREEN 1:\nIssue\n1 line error"
    assert config.last_output_file_path == ""
    assert "last_output_file_path" in config.update_fields


def test_generation_summary_error_restores_offsets(
    monkeypatch, settings, tmp_path
) -> None:
    from apps.nodes.models import Node
    ...
    class FakeConfig:
        is_active = True
        last_run_at = None
        log_offsets = {"logs/arthexis.log": 12}
    ...
    class FakeSummarizer:
        ...
        def summarize(self, prompt: str) -> str:
            raise task_services.LocalLLMSummaryError("summary failed")
    ...
    monkeypatch.setattr(
        services,
        "collect_recent_logs",
        lambda config, *, since, attention_since=None: _mutate_offsets_and_chunk(
            config,
            tmp_path,
        ),
    )
    ...
    assert result == "error:summary-generation"
    assert config.last_prompt == "prompt"
    assert config.last_output == ""
    assert config.log_offsets == {"logs/arthexis.log": 12}
    assert "log_offsets" in config.update_fields


def _mutate_offsets_and_chunk(config, tmp_path):
    config.log_offsets = {"logs/arthexis.log": 99}
    return [services.LogChunk(path=tmp_path / "journal.log", content="ERR hi")]
...
def test_dense_frames_from_prompt_builds_summary_frame() -> None:
    ...
    assert frames[0] == ("2 ln/5m", "ERR 1")
    assert frames[1] == ("ERR journal", "worker failed to")
    assert frames[2] == ("WRN journal", "retry scheduled")


def test_dense_frames_prioritize_attention_over_routine_sources() -> None:
    prompt = "\n".join(
        [
            "LCD_CONTEXT_WINDOW_LABEL: 60m",
            "LOGS:",
            "[journal:celery-beat-arthexis.service]",
            "INF celery.beat: Scheduler: Sending due task heartbeat "
            "(apps.core.tasks.heartbeat)",
            "INF celery.beat: Scheduler: Sending due task heartbeat "
            "(apps.core.tasks.heartbeat)",
            "[journal:lcd-arthexis.service]",
            "ERR apps.screens: LCD write failed for lcd-summary",
        ]
    )

    frames = dense_lcd.dense_frames_from_prompt(prompt)

    assert frames[0] == ("3 ln/60m", "ERR 1")
    assert frames[1] == ("ERR lcd", "LCD write failed")
    assert frames[2] == ("OK beat", "2x/60m")


def test_dense_frames_do_not_label_attention_sources_ok() -> None:
    prompt = "\n".join(
        [
            "LOGS:",
            "[journal:lcd-arthexis.service]",
            "ERR apps.screens: LCD write failed",
        ]
    )
    ...
    assert frames == [
        ("1 ln/60m", "ERR 1"),
        ("ERR lcd", "LCD write failed"),
    ]


def test_dense_frames_collapse_normal_sources() -> None:
    prompt = "\n".join(
        [
            "LOGS:",
            "[journal:celery-beat-arthexis.service]",
            "INF celery.beat: Scheduler: Sending due task heartbeat",
            "INF celery.beat: Scheduler: Sending due task heartbeat",
            "[journal:celery-arthexis.service]",
            "INF celery.worker.strategy: Task apps.core.tasks.heartbeat[abc] received",
        ]
    )
