...
from .models import Agent, CodexFeatureFreeze, CodexTokenBudget, Hook, Skill, SkillFile
...
@admin.register(CodexFeatureFreeze)
class CodexFeatureFreezeAdmin(admin.ModelAdmin):
    list_display = (
        "policy_version",
        "reason",
        "active",
    ...
    def active_now(self, freeze: CodexFeatureFreeze) -> bool:
        return freeze.is_currently_active


@admin.register(CodexTokenBudget)
class CodexTokenBudgetAdmin(admin.ModelAdmin):
    list_display = (
        "name",
        "enabled",
    ...
    list_filter = ("enabled", "last_status", "auto_freeze")
    readonly_fields = ("created_at", "updated_at", "last_checked_at", "last_freeze")
    search_fields = ("name",)
