...
RETIRED_OPERATOR_FRAMEWORK_CORE_HOOK_SLUGS = frozenset(
    {
        "ci-unchanged-wait",
        "codex-freeze-guard",
        "github-api-budget",
        "local-heavy-process-lock",
...
RETIRED_OPERATOR_FRAMEWORK_CORE_HOOK_COMMAND_MARKER = (
    "apps/skills/packages/operator-framework-core/skills/work/scripts/"
)
...
def _retired_operator_framework_core_hooks(manifest: dict) -> list[Hook]:
    ...
    if "operator-framework-align" not in skill_slugs:
        return []
    retired_hooks = []
    for hook in Hook.all_objects.filter(
        slug__in=RETIRED_OPERATOR_FRAMEWORK_CORE_HOOK_SLUGS,
        enabled=True,
        is_deleted=False,
    ):
        normalized_command = hook.command.replace("\\", "/")
        if RETIRED_OPERATOR_FRAMEWORK_CORE_HOOK_COMMAND_MARKER in normalized_command:
            retired_hooks.append(hook)
    return retired_hooks


def _retired_operator_framework_core_hooks_for_preview(manifest: dict) -> list[Hook]:
    try:
        return _retired_operator_framework_core_hooks(manifest)
...
def _disable_retired_operator_framework_core_hooks(manifest: dict) -> list[str]:
    hooks = _retired_operator_framework_core_hooks(manifest)
    if not hooks:
        return []
    hook_ids = [hook.pk for hook in hooks]
    Hook.all_objects.filter(pk__in=hook_ids).update(enabled=False)
    for hook in hooks:
        hook.enabled = False
    return [hook.slug for hook in hooks]
    ...
    *,
    ...
    with ZipFile(package_source) as package:
        ...
        with transaction.atomic():
            ...
            summary["retired_hooks"] = [
                {"slug": slug}
                for slug in _disable_retired_operator_framework_core_hooks(manifest)
            ]
        return summary
