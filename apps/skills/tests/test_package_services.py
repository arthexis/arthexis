...
from apps.skills import package_services
...
def test_bundled_operator_framework_core_package_imports_from_directory():
    ...
    assert summary["retired_hooks"] == []


@pytest.mark.django_db
def test_bundled_operator_framework_core_import_disables_retired_hooks():
    package_dir = (
        Path(__file__).resolve().parents[3]
        / "apps"
        / "skills"
    ...
    stale_hook = Hook.objects.create(
        slug="codex-freeze-guard",
        title="Codex Feature Freeze Guard",
        event=Hook.Event.BEFORE_PROMPT,
        command=(
            'python "[CONF.BASE_DIR]/apps/skills/packages/operator-framework-core/'
            'skills/work/scripts/codex_freeze_cache_hook.py"'
        ),
        enabled=True,
    )
    custom_hook = Hook.objects.create(
        slug="custom-before-prompt",
        title="Custom Before Prompt",
        event=Hook.Event.BEFORE_PROMPT,
        command="python manage.py check",
        enabled=True,
    )

    dry_run_summary = import_codex_skill_package(package_dir, dry_run=True)

    stale_hook.refresh_from_db()
    assert stale_hook.enabled is True
    assert dry_run_summary["retired_hooks"] == [{"slug": "codex-freeze-guard"}]

    summary = import_codex_skill_package(package_dir, dry_run=False)

    stale_hook.refresh_from_db()
    custom_hook.refresh_from_db()
    assert summary["retired_hooks"] == [{"slug": "codex-freeze-guard"}]
    assert stale_hook.enabled is False
    assert custom_hook.enabled is True


@pytest.mark.django_db
def test_disable_retired_operator_framework_core_hooks_updates_in_memory_instances(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        package_services,
        "_retired_operator_framework_core_hooks",
        lambda _manifest: [hook],
    )

    summary = package_services._disable_retired_operator_framework_core_hooks({})

    assert summary == ["codex-freeze-guard"]
