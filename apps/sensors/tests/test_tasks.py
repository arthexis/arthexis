...
def test_sample_thermometers_records_ambient_and_soc_history(monkeypatch) -> None:
    ambient = Thermometer.objects.create(
        name="Ambient",
        slug="ambient-thermometer",
        unit="C",
    )
    soc = Thermometer.objects.create(
    ...
    monkeypatch.setattr(
        "apps.sensors.tasks.read_temperature",
        lambda *, source, w1_paths, i2c_paths: Decimal("29.25"),
    )
    monkeypatch.setattr(
        "apps.sensors.tasks.read_soc_temperature",
        lambda: Decimal("72.062"),
    )
    ...
    assert result == {"sampled": 2, "skipped": 0, "failed": 0}
    assert ambient.kind == Thermometer.Kind.AMBIENT
    assert ambient.last_reading == Decimal("29.25")
    assert ambient.readings.count() == 1
    assert soc.kind == Thermometer.Kind.SOC
    assert soc.last_reading == Decimal("72.06")
...
def test_sample_thermometers_skips_missing_soc_sensor_quietly(monkeypatch, caplog) -> None:
    ...
    monkeypatch.setattr(
        "apps.sensors.tasks.read_soc_temperature",
        fake_read_soc_temperature,
    )

    with caplog.at_level(logging.INFO, logger="apps.sensors.tasks"):
        result = sample_thermometers()
        second_result = sample_thermometers()
    thermometer.refresh_from_db()

    assert result == {"sampled": 0, "skipped": 1, "failed": 0}
    assert second_result == {"sampled": 0, "skipped": 1, "failed": 0}
    assert thermometer.last_read_at is not None
    assert calls == 1
    assert "no reading returned" not in caplog.text
