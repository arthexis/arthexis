...
def test_render_usb_lcd_lines_fits_four_fixed_icon_slots() -> None:
    statuses = [
        UsbPortStatus(port_number=1, label="KEY", connected=True),
    ...
    assert "KEY" in line1
    assert "FREE" in line1
    assert "BASTION" in line2
    assert "USB-KEY" in line2
    assert len(line1) == 16
    assert len(line2) == 16


def test_build_usb_lcd_statuses_uses_local_tracker_mappings() -> None:
    ...
    assert [status.connected for status in statuses] == [True, False, False, False]
...
def test_write_usb_lcd_status_writes_lock_file(tmp_path: Path) -> None:
    ...
    assert "BASTION" in result["line1"]
    assert "FREE" in result["line1"]
    assert "FREE" in result["line2"]
    assert message is not None
    assert "BASTION" in message.subject
