...
DEFAULT_PI_IMAGE_LABELS = ("bootfs", "rootfs")
DEFAULT_IMAGER_MEDIA_EXCLUDED_CLAIM_ROLES = ("bastion-unlock", "kindle-postbox")
...
def pi_image_labels() -> tuple[str, ...]:
    labels = getattr(settings, "USB_INVENTORY_PI_IMAGE_LABELS", DEFAULT_PI_IMAGE_LABELS)
    return tuple(str(label).strip().lower() for label in labels if str(label).strip())


def imager_media_excluded_claim_roles() -> tuple[str, ...]:
    roles = getattr(
        settings,
        "USB_INVENTORY_IMAGER_MEDIA_EXCLUDED_CLAIM_ROLES",
        DEFAULT_IMAGER_MEDIA_EXCLUDED_CLAIM_ROLES,
    )
...
def _flatten_lsblk(
    ...
    *,
    parent_usb: bool = False,
    parent_identity_paths: tuple[str, ...] = (),
) -> list[dict[str, Any]]:
    ...
    for device in devices:
        ...
        flattened.extend(
            _flatten_lsblk(
                children,
                parent_usb=current_usb,
                parent_identity_paths=(
                    *parent_identity_paths,
                    *identity_paths,
                ),
            )
...
def _lsblk_tree_has_pi_image(device: dict[str, Any]) -> bool:
    if _is_pi_image_media(device):
        return True
...
def _device_identity_paths(
    device: dict[str, Any], *, include_parents: bool = True
) -> list[str]:
    paths = _state_text_values(device.get("path"))
    paths.extend(_state_text_values(device.get("name")))
    paths.extend(_state_text_values(device.get("kname")))
    paths.extend(_state_text_values(device.get("source")))
    if include_parents:
        paths.extend(_state_text_values(device.get("_parent_identity_paths")))
    return paths


def _resolved_path_tokens(path_text: str) -> set[str]:
    tokens = {path_text}
    path = Path(path_text)
    if path.name:
    ...
    if path.is_absolute():
        try:
            tokens.add(str(path.resolve(strict=False)))
        except (OSError, RuntimeError, ValueError):
            pass
    return tokens


def _configured_imager_burner_tokens() -> set[str]:
    tokens: set[str] = set()
    for name in ("IMAGER_GWAY_BURN_DEVICE", "IMAGER_BURN_DEVICE"):
        for value in (getattr(settings, name, ""), os.environ.get(name, "")):
            path_text = str(value or "").strip()
            if path_text:
                tokens.update(_resolved_path_tokens(path_text))
    return tokens


def _is_configured_imager_burner(
    device: dict[str, Any],
    configured_tokens: set[str] | None = None,
) -> bool:
    if configured_tokens is None:
        configured_tokens = _configured_imager_burner_tokens()
    if not configured_tokens:
        return False
    device_tokens: set[str] = set()
    for path_text in _device_identity_paths(device):
        device_tokens.update(_resolved_path_tokens(path_text))
    return bool(configured_tokens & device_tokens)


def _is_pi_image_media(device: dict[str, Any]) -> bool:
    if device.get("_child_pi_image"):
        return True
    labels = set(pi_image_labels())
    if not labels:
        return False
    label_values = [
        str(device.get("label") or ""),
        str(device.get("partlabel") or ""),
    ]
    return any(label.strip().lower() in labels for label in label_values if label)
...
def match_claim(device: dict[str, Any], claim: dict[str, Any]) -> bool:
    ...
    if role in imager_media_excluded_claim_roles() and device.get("imager_media"):
        return False
...
def _normalize_device(
    ...
    *,
    configured_imager_burner_tokens: set[str] | None = None,
) -> dict[str, Any]:
    ...
    normalized["pi_image"] = _is_pi_image_media({**device, **normalized})
    normalized["configured_imager_burner"] = _is_configured_imager_burner(
        {**device, **normalized},
        configured_imager_burner_tokens,
    )
    normalized["imager_media"] = bool(
        normalized["pi_image"] or normalized["configured_imager_burner"]
    )
    normalized["id"] = (
        normalized["uuid"]
...
def inventory_devices() -> list[dict[str, Any]]:
    ...
    configured_imager_burner_tokens = _configured_imager_burner_tokens()
    devices = []
    for device in _flatten_lsblk(lsblk_data.get("blockdevices", [])):
        if not isinstance(device, dict) or not _is_usb_device(device):
            continue
        normalized = _normalize_device(
            device,
            mounts,
            configured_imager_burner_tokens=configured_imager_burner_tokens,
        )
