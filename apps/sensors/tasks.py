...
def read_configured_thermometer(thermometer: Thermometer) -> Decimal | None:
    if thermometer.kind == Thermometer.Kind.SOC:
        return read_soc_temperature()
...
def sample_thermometers() -> dict[str, int]:
    ...
    for thermometer in Thermometer.objects.filter(is_active=True).iterator():
        ...
        reading = read_configured_thermometer(thermometer)
        if reading is None:
            if thermometer.kind == Thermometer.Kind.SOC:
                thermometer.last_read_at = now
                thermometer.save(update_fields=["last_read_at"])
                skipped += 1
                logger.debug(
                    "SoC thermometer sample skipped; no local sensor for %s",
                    thermometer.slug,
                )
            else:
                failed += 1
        ...
        thermometer.record_reading(reading, read_at=read_at)
        evaluate_temperature_alarm(thermometer, reading, read_at=read_at)
        sampled += 1
