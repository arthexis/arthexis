...
ROTATION_SCRIPT_LOCK_NAME = LCD_ROTATION_SCRIPT_LOCK_FILE
ROTATION_SCRIPT_CHANNEL_NAME = "script"
DEFAULT_SCROLL_MS = 0
...
CHANNEL_BASE_NAMES = {
    ...
    "github": LCD_GITHUB_LOCK_FILE,
    "clock": CLOCK_LOCK_NAME,
    "uptime": UPTIME_LOCK_NAME,
    "stats": LCD_STATS_LOCK_FILE,
    "usb": LCD_USB_LOCK_FILE,
}
KNOWN_CHANNEL_NAMES = frozenset((*CHANNEL_BASE_NAMES, ROTATION_SCRIPT_CHANNEL_NAME))
...
class RotationScriptError(ValueError):
    """Raised when the LCD rotation script cannot be parsed safely."""
...
def parse_rotation_script(text: str) -> list[LockPayload]:
    """Parse a data-only LCD rotation script.

    The default auto-discovered path is ``.locks/lcd-rotation.script``. Each
    non-empty, non-comment line must be ``frame "LINE 1" "LINE 2"``; the second
    LCD line is optional.
    """
    ...
    for line_num, raw_line in enumerate(text.splitlines(), 1):
        ...
        try:
            ...
            raise RotationScriptError(f"line {line_num}: {exc}") from exc
        if not parts:
            continue
        command = parts[0].casefold()
        if command != "frame":
            raise RotationScriptError(
                f"line {line_num}: unsupported command '{parts[0]}'"
            )
        if len(parts) not in {2, 3}:
            raise RotationScriptError(
                f"line {line_num}: expected frame \"LINE 1\" \"LINE 2\""
            )
        line1 = parts[1][:64]
        line2 = parts[2][:64] if len(parts) == 3 else ""
        payloads.append(LockPayload(line1, line2, DEFAULT_SCROLL_MS))
    return payloads


def _load_rotation_script_payloads(
    lock_dir: Path = LOCK_DIR,
) -> tuple[list[LockPayload], tuple[tuple[int, float], ...]]:
    script_file = lock_dir / ROTATION_SCRIPT_LOCK_NAME
    try:
        ...
        logger.debug("Unable to read LCD rotation script", exc_info=True)
        return [], ()
    ...
    try:
        payloads = parse_rotation_script(text)
    except RotationScriptError as exc:
        logger.warning("Ignoring invalid LCD rotation script %s: %s", script_file, exc)
        return [], ()

    if not payloads:
        return [], ()
    return payloads, ((0, stat.st_mtime),)
