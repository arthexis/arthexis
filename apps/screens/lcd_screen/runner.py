...
class LCDRunner:
    ...
    def payload_for_state(
        ...
        *,
        ...
        if (
            state_label in {"summary", "github", "usb", locks.ROTATION_SCRIPT_CHANNEL_NAME}
            and channel_state
        ):
    ...
    def configure_rotation_order(
        ...
        def _channel_available(label: str) -> bool:
            ...
            if label in {"summary", "github", "usb"}:
                return channel_text.get(label, False)
            if label == locks.ROTATION_SCRIPT_CHANNEL_NAME:
                return channel_text.get(label, False)
            if label == "clock":
        ...
        if configured_order:
            ...
            if script_available:
                normal_order = (locks.ROTATION_SCRIPT_CHANNEL_NAME,)
            elif high_available:
                normal_order = (
...
def _load_channel_states(
    ...
    script_payloads, script_signature = locks._load_rotation_script_payloads(
        locks.LOCK_DIR
    )
    existing = current_states.get(locks.ROTATION_SCRIPT_CHANNEL_NAME)
    if (
        existing is None
        or existing.signature != script_signature
        or script_payloads != existing.payloads
    ):
        next_index = 0
        if existing and script_payloads:
            next_index = existing.index % len(script_payloads)
        existing = ChannelCycle(
            payloads=script_payloads,
            signature=script_signature,
            index=next_index,
        )
    current_states[locks.ROTATION_SCRIPT_CHANNEL_NAME] = existing
    channel_info[locks.ROTATION_SCRIPT_CHANNEL_NAME] = existing
    channel_text[locks.ROTATION_SCRIPT_CHANNEL_NAME] = any(
        _payload_has_text(payload) for payload in existing.payloads
    )
