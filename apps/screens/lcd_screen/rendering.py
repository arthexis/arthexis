...
def _split_temperature_label_prefix(label: str) -> tuple[str, str]:
    stripped = label.strip()
    prefix, separator, rest = stripped.partition(" ")
    if separator and prefix.lower() in {"amb", "soc"}:
        return prefix, rest.strip()
    return "", stripped


def _parse_temperature_label(label: str) -> tuple[Decimal | None, str]:
    _prefix, value_label = _split_temperature_label_prefix(label)
    if not value_label:
        return None, ""

    unit = value_label[-1]
    try:
        value = Decimal(value_label[:-1])
...
def _clock_payload(
    ...
    *,
    ...
    temp_prefix, temp_label = _split_temperature_label_prefix(temperature or "")
    ...
    if temp_value is not None:
        if use_fahrenheit and unit.upper() == "C":
            ...
            temp_label = _format_temperature_value(temp_value, "F")
        else:
            temp_label = _format_temperature_value(temp_value, unit)
        temperature = f"{temp_prefix} {temp_label}" if temp_prefix else temp_label
