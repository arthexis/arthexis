...
def test_rotation_order_includes_github_channel_when_active(monkeypatch):
    coordinator = runner.LCDRunner()
    ...
    assert coordinator.rotation.order == ("low", "stats", "github", "clock")


def test_github_channel_loads_lcd_github_lock(monkeypatch, tmp_path):
    now_dt = datetime(2026, 5, 17, tzinfo=timezone.utc)
    lock_dir = tmp_path / ".locks"
    lock_dir.mkdir()
    (lock_dir / "lcd-github").write_text(
        "NEXT PR 7818\nweb split view\n", encoding="utf-8"
    )
    ...
    assert channel_text["github"] is True
    assert channel_info["github"].payloads[0].line1 == "NEXT PR 7818"
    assert channel_info["github"].payloads[0].line2 == "web split view"
...
def test_github_payload_uses_channel_lock_text() -> None:
    coordinator = runner.LCDRunner()
    now_dt = datetime(2026, 5, 18, tzinfo=timezone.utc)
    github_cycle = runner.ChannelCycle(
        payloads=[
            runner.locks.LockPayload("NEXT PR 7818", "web split view", 0),
        ],
        signature=((0, 0.0),),
        index=0,
    )
    channel_info = {"github": github_cycle}
    channel_text = {"github": True}

    payload = coordinator.payload_for_state(
        ("github",),
        0,
        channel_info,
        channel_text,
    ...
    assert payload.line1 == "NEXT PR 7818"
    assert payload.line2 == "web split view"
