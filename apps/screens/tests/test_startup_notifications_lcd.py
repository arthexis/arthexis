...
class LCDStartupNotificationTests(TestCase):
    ...
    def test_lcd_feature_enablement_toggles_startup_behavior(self):
        """Verify runtime lock presence toggles LCD feature enablement checks."""
    ...
    def test_lcd_feature_enabled_accepts_feature_locks(self):
        with TemporaryDirectory() as tmpdir:
            ...
            (lock_dir / LCD_LEGACY_FEATURE_LOCK).unlink()
            (lock_dir / LCD_RUNTIME_LOCK_FILE).touch()
            self.assertTrue(lcd_feature_enabled(lock_dir))

    def test_lcd_feature_enabled_ignores_message_locks(self):
        with TemporaryDirectory() as tmpdir:
            lock_dir = Path(tmpdir) / ".locks"
            lock_dir.mkdir(parents=True)
