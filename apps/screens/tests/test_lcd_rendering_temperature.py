...
def test_lcd_temperature_label_from_sensors_distinguishes_soc_reading() -> None:
    ...
    ambient.record_reading(
        Decimal("31.2"),
        read_at=timezone.now() - timedelta(minutes=5),
    )
    soc.record_reading(Decimal("72.1"), read_at=timezone.now())

    assert rendering._lcd_temperature_label_from_sensors() == "soc 72.1C"
...
def test_clock_payload_converts_prefixed_soc_temperature_to_fahrenheit(
    monkeypatch,
) -> None:
    monkeypatch.setattr(lcd_package, "_lcd_temperature_label", lambda: "soc 72.1C")

    _line1, line2, _speed, _kind = rendering._clock_payload(
        timezone.now(),
        use_fahrenheit=True,
        choose_fate=lambda: False,
    )

    assert line2.endswith("@ soc 162F")
