...
class Command(BaseCommand):
    help = (
        "Preview the next 60 seconds of LCD output from current lock files. "
        f"Rotation override script: .locks/{lcd_screen.ROTATION_SCRIPT_LOCK_NAME} "
        'with lines like frame "LINE 1" "LINE 2".'
    )
        ...
        *,
        ...
        def payload_for_state(
            ...
            *,
            ...
            if state_label == lcd_screen.ROTATION_SCRIPT_CHANNEL_NAME:
                payload = script_cycle.next_payload()
                if payload and lcd_screen._payload_has_text(payload):
                    return payload, clock_cycle, stats_cycle
                return (
        ...
        def compute_state_order(
            ...
            if script_available:
                return (lcd_screen.ROTATION_SCRIPT_CHANNEL_NAME,)
            if high_available:
                return (
        ...
        def load_payloads(
            ...
            script_payloads, script_signature = lcd_screen._load_rotation_script_payloads(
                lock_dir
            )
            if (
                script_cycle.signature != script_signature
                or script_cycle.payloads != script_payloads
            ):
                next_index = 0
                if script_cycle.payloads and script_payloads:
                    next_index = script_cycle.index % len(script_payloads)
                script_cycle = lcd_screen.ChannelCycle(
                    payloads=script_payloads,
                    signature=script_signature,
                    index=next_index,
                )
            script_available = any(
                lcd_screen._payload_has_text(payload)
                for payload in script_cycle.payloads
            )
            return (
                high_payload,
                low_payload,
        ...
        def refresh_state_order(
            high_available: bool, low_available: bool, script_available: bool
        ) -> None:
            nonlocal state_order, state_index
            previous_order = state_order
            state_order = compute_state_order(
                high_available, low_available, script_available
            )
        ...
        stats_cycle = 0
        script_cycle = lcd_screen.ChannelCycle(payloads=[], signature=())
        ...
        while current_offset < duration:
            ...
            if event_state is not None:
                ...
                step = (
                    event_state.scroll_sec
                    if event_state
                    else lcd_screen.DEFAULT_FALLBACK_SCROLL_SEC
                )
            else:
