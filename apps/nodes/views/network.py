...
from ..models.net_message import LEGACY_REMOTE_UPGRADE_CONTROL_QUEUE_MARKER
from ..services.propagation import is_remote_upgrade_control_message
...
def _ocpp_unavailable_response():
    return JsonResponse({"detail": "OCPP app is not installed."}, status=404)


def _load_ocpp_charger_model():
    if not django_apps.is_installed("apps.ocpp"):
        return None
    try:
...
def _normalize_requested_chargers(
    values,
    *,
    charger_model=None,
    ...
    Charger = charger_model or _load_ocpp_charger_model()
    if Charger is None:
        return []
...
def network_chargers(request):
    ...
    Charger = _load_ocpp_charger_model()
    if Charger is None:
        return _ocpp_unavailable_response()
    try:
    ...
    requested = _normalize_requested_chargers(
        body.get("chargers") or [],
        charger_model=Charger,
    )
...
@csrf_exempt
def import_chargers(request):
    """Import charger metadata and transactions from trusted peers."""
...
def net_message(request):
    ...
    try:
        ...
        logger.warning("Rejected invalid network message from node %s", node.pk)
...
def net_message_pull(request):
    ...
    if not requester:
        return JsonResponse({"detail": "requester required"}, status=400)
    try:
        requester_uuid = UUID(str(requester))
    except (TypeError, ValueError):
        return JsonResponse({"detail": "invalid requester UUID"}, status=400)

    requester_mac = _clean_requester_hint(data.get("requester_mac"))
    requester_public_key = _clean_requester_hint(
        data.get("requester_public_key"), strip=False
    )
    ...
    if requester_uuid != node.uuid:
        if Node.objects.exclude(pk=node.pk).filter(uuid=requester_uuid).exists():
            return JsonResponse({"detail": "requester UUID already in use"}, status=409)
        try:
            Node.objects.filter(pk=node.pk).update(uuid=requester_uuid)
        except IntegrityError:
            return JsonResponse({"detail": "requester UUID already in use"}, status=409)
        node.uuid = requester_uuid
    ...
    for entry in entries:
        ...
        seen = [
            str(value)
            for value in entry.seen
            if str(value) != LEGACY_REMOTE_UPGRADE_CONTROL_QUEUE_MARKER
        ]
        allow_local_control = (
            is_remote_upgrade_control_message(message)
            and message.node_origin_id == local.pk
            and entry.local_remote_upgrade_control
        )
        if is_remote_upgrade_control_message(message) and not allow_local_control:
            expired_ids.append(entry.pk)
            if not message.complete:
                message.complete = True
                message.save(update_fields=["complete"])
