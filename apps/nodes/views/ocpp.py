...
from apps.ocpp.public_routes import (
...
def _toggle_rfid(
    ...
    detail = (
        ...
        if require_rfid
        else "RFID authentication disabled"
    )
    return (
        True,
        detail,
...
def _prepare_diagnostics_upload_payload(
    ...
    upload_path = reverse_public_ocpp_route(
        "ocpp:media-bucket-upload",
        kwargs={"slug": bucket.slug},
    )
    if not upload_path:
        return prepared, str(PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE)
...
def _request_diagnostics_remote(
    ...
    if request is not None:
        payload, prepare_error = _prepare_diagnostics_upload_payload(
            request, charger, payload
        )
        if prepare_error:
...
def network_charger_action(request):
    ...
    if request.method != "POST":
        return json_api_error(
            status=405, code="method_not_allowed", message="POST required"
        )
    ...
    if not serial or Charger.is_placeholder_serial(serial):
        return json_api_error(
            status=400, code="charger_invalid", message="invalid charger"
        )
    ...
    if connector in ("", None):
        ...
        try:
            ...
            return json_api_error(
                status=400, code="connector_invalid", message="invalid connector"
            )
    ...
    if not charger:
        return json_api_error(
            status=404, code="charger_not_found", message="charger not found"
        )

    if not charger.allow_remote:
        return json_api_error(
            status=403,
            code="remote_actions_disabled",
            message="remote actions disabled",
        )

    if not _require_local_origin(charger):
        return json_api_error(
            status=403,
            code="charger_not_local",
            message="charger is not managed by this node",
        )
    ...
    if authorized_node_ids and node and node.pk not in authorized_node_ids:
        return json_api_error(
            status=403,
            code="requester_not_manager",
            message="requester does not manage this charger",
        )
    ...
    if handler is None:
        return json_api_error(
            status=400, code="action_unsupported", message="unsupported action"
        )
