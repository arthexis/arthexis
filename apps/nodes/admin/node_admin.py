...
class NodeAdmin(SaveBeforeChangeAction, EntityModelAdmin):
    ...
    RELATION_SHORT_LABELS = {
        Node.Relation.UPSTREAM: _("Up"),
        Node.Relation.DOWNSTREAM: _("Down"),
        Node.Relation.PEER: _("Peer"),
        Node.Relation.SIBLING: _("Sibling"),
        Node.Relation.SELF: _("Self"),
    }
    ...
    if OCPP_ADMIN_AVAILABLE:
        actions.append(download_evcs_firmware)
    ...
    @admin.display(description=_("Rel"), ordering="current_relation")
    def relation(self, obj):
        ...
        short_label = self.RELATION_SHORT_LABELS.get(relation, label)
        icon = self.RELATION_ICONS.get(relation, "❔")
        return format_html('<span title="{}">{} {}</span>', label, icon, short_label)
    ...
    def _process_firmware_download(self, request, node: Node, cleaned_data) -> bool:
        ...
        from apps.ocpp.models import CPFirmware, CPFirmwareRequest, DataTransferMessage

        charger = cleaned_data["charger"]
        ...
        if status_value.lower() != "accepted":
            self.message_user(
                request,
                _("Firmware request for %(charger)s was %(status)s.")
    ...
    def _apply_remote_node_info(self, node, payload):
        ...
        def apply_field(field: str, value):
            ...
            if isinstance(
                field_obj, (models.CharField, models.TextField, models.SlugField)
            ):
                value = value or ""
                current = current or ""
            if current != value:
        ...
        if revision_value is not sentinel:
            cleaned_revision = (
                "" if revision_value is None else str(revision_value)[:40]
            )
    ...
    def register_visitor_view(self, request):
        ...
        if request.method == "POST":
            log_visible = True
            registration_result = VisitorRegistrationService(
                user=request.user
            ).register(parsed_request)
        ...
        if not is_https_request(request):
            https_warnings.append(
                _(
                    "This admin session is not using HTTPS. Visitor registration requires HTTPS on both nodes."
                )
            )
        base_site_profile = (
        ...
        if node.base_site_id and not bool(
            ...
            https_warnings.append(
                _(
                    "This host node is not configured to require HTTPS. Update Sites settings before continuing."
                )
            )
        ...
        context = {
            **self.admin_site.each_context(request),
            ...
            "local_node": {
                ...
                "address": node.get_base_domain()
                or node.address
                or node.network_hostname,
                "port": (
                    node._preferred_site_port(True)
    ...
    def public_key(self, request, node_id):
        ...
        self.message_user(
            request,
            "Public key not found on node record or local security file.",
            messages.ERROR,
        )
        return redirect("..")
    ...
    def _send_charger_import_payload(
        self,
        request,
        target,
        chargers,
        ...
        if not chargers:
            _safe_message(messages.INFO, _("No chargers available to export."))
            return True
        ...
        if private_key:
            ...
            if error or not signature:
                _safe_message(
                    messages.ERROR,
                    _("Failed to sign charger export payload: %(error)s")
                    % {"error": error},
                )
                return False
        ...
        errors: list[str] = []
        for url in self._iter_remote_urls(target, "/nodes/network/chargers/import/"):
            if not url:
                continue
            try:
                response = requests.post(
                ...
                errors.append(
                    _(
                        "Failed to send charger export to %(node)s via %(url)s (%(error)s)."
                    )
            ...
            if response.ok and isinstance(data, Mapping) and data.get("status") == "ok":
                _safe_message(
                    messages.SUCCESS,
                    _("Charger export sent to %(node)s via %(url)s.")
                    % {"node": target, "url": url},
                )
                return True
            ...
            errors.append(
                _(
                    "Charger export to %(node)s via %(url)s failed: %(status)s %(detail)s"
                )
                % {
                    "node": target,
                    "url": url,
    ...
    def _summarize_rfid_results(self, results):
        return {
            ...
            "missing_accounts": sum(len(item["missing_accounts"]) for item in results),
        }
    ...
    def _run_rfid_import(self, request, queryset):
        ...
        if error or not signature:
            ...
            return self._render_rfid_sync(request, "import", [], setup_error=message)
        headers = {
