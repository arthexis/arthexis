...
LEGACY_REMOTE_UPGRADE_CONTROL_QUEUE_MARKER = "__arthexis_local_remote_upgrade_control__"
...
class NetMessage(Entity):
    ...
    class Kind(models.TextChoices):
        ...
        REMOTE_UPGRADE_REQUEST = "remote_upgrade_request", "Remote upgrade request"
        REMOTE_UPGRADE_RESPONSE = "remote_upgrade_response", "Remote upgrade response"
    ...
    kind = models.CharField(
        ...
        help_text="Typed control payload kind. Plain operator messages use message.",
    )
    control_payload = models.JSONField(
        blank=True,
        null=True,
        help_text="Signed typed payload used by node control workflows.",
    )
    ...
    def _build_payload(
        ...
        *,
        ...
        if self.kind and self.kind != self.Kind.MESSAGE:
    ...
    def queue_for_node(
        ...
        *,
        ...
        local_remote_upgrade_control = allow_remote_upgrade_control and self.kind in {
            self.Kind.REMOTE_UPGRADE_REQUEST,
            self.Kind.REMOTE_UPGRADE_RESPONSE,
        }
        entry, created = PendingNetMessage.objects.get_or_create(
            ...
            defaults={
                ...
                "local_remote_upgrade_control": local_remote_upgrade_control,
                "stale_at": expires_at,
            },
        )
        if created:
            ...
            entry.local_remote_upgrade_control = local_remote_upgrade_control
            entry.stale_at = expires_at
            entry.queued_at = now
            entry.save(
                update_fields=[
                    "seen",
                    "local_remote_upgrade_control",
                    "stale_at",
                    "queued_at",
                ]
            )
