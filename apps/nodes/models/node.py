...
class Node(NodeFeatureMixin, NodeNetworkingMixin, Entity):
    ...
    _local_cache: dict[str, tuple[Node | None, datetime]] = {}
    ROLE_BADGE_COLORS = {
    ...
    def is_same_host_as(self, other: object | None) -> bool:
        """Return whether ``other`` appears to run on this physical host."""

        if other is None:
            return False

        host_instance_id = (self.host_instance_id or "").strip()
        other_host_instance_id = str(
            getattr(other, "host_instance_id", "") or ""
        ).strip()
        if host_instance_id and other_host_instance_id:
            return host_instance_id == other_host_instance_id

        mac_address = "".join(
            char.lower() for char in str(self.mac_address or "") if char.isalnum()
        )
        other_mac_address = "".join(
            char.lower()
            for char in str(getattr(other, "mac_address", "") or "")
            if char.isalnum()
        )
        if mac_address and other_mac_address:
            return mac_address == other_mac_address

        return False

    def get_local_transport_status(
        self, *, local_node: object | None = None
    ) -> dict[str, object]:
        """Return same-host local transport readiness for this node."""

        if local_node is None:
            local_node = type(self).get_local()

        same_host = self.is_same_host_as(local_node)
            ...
            status.update(
                {
                    ...
                    "configured": False,
                    "ready": False,
                    "path": str(self.get_ipc_socket_path() or ""),
                }
            )
            return status

        from apps.nodes.services.transport import get_unix_socket_status

        status.update(get_unix_socket_status(self))
        return status
    ...
    def get_local(cls):
        ...
        if cached:
            ...
            if timezone.is_naive(expires_at) != timezone.is_naive(now):
        ...
        try:
            ...
            self_nodes = cls.objects.filter(current_relation=cls.Relation.SELF)
            host_instance_id = cls.get_host_instance_id()
            node = None
            if host_instance_id:
                node = self_nodes.filter(host_instance_id=host_instance_id).first()
            if node is None:
                self_node_count = self_nodes.count()
                if self_node_count == 0:
                    return None
                if self_node_count > 1:
                    logger.warning(
                        "nodes.Node.get_local skipped stale self-node MAC refresh because multiple self nodes exist",
                        extra={
            ...
            if stored_mac != current_mac:
                ...
                try:
                    ...
                    refreshed_stale_self = True
                    matching_node = (
                        cls.objects.filter(mac_address__iexact=mac)
                        .exclude(pk=node.pk)
                        .first()
                    )
                    if matching_node is not None:
                        cls.objects.filter(pk=node.pk).update(mac_address=stored_mac)
                        refreshed_stale_self = False
