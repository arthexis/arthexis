...
REMOTE_UPGRADE_CONTROL_KINDS = {
    "remote_upgrade_request",
    "remote_upgrade_response",
}


def _mark_complete(message) -> None:
...
def is_remote_upgrade_control_message(message) -> bool:
    return message.kind in REMOTE_UPGRADE_CONTROL_KINDS
...
def receive_payload(message_model, data: dict[str, object], *, sender: Node):
    ...
    kind = str(data.get("kind") or message_model.Kind.MESSAGE)[:40]
    control_payload = data.get("control_payload")
    ...
    if not created:
        ...
        if is_remote_upgrade_control and msg.node_origin_id != sender.pk:
            msg.node_origin = sender
            update_fields.append("node_origin")
        elif msg.node_origin_id is None and origin_node:
    ...
    if msg.kind == message_model.Kind.REMOTE_UPGRADE_REQUEST:
        if control_payload:
            try:
                from apps.nodes.services.remote_upgrade import (
                    receive_remote_upgrade_request,
                )

                receive_remote_upgrade_request(control_payload, sender=sender)
            except ValidationError as exc:
                logger.warning(
                    "Discarded remote upgrade request from node %s: %s",
                    sender.pk,
                    "; ".join(exc.messages),
                )
        _mark_complete(msg)
        return msg
    elif msg.kind == message_model.Kind.REMOTE_UPGRADE_RESPONSE:
        if control_payload:
            from apps.nodes.services.remote_upgrade import (
                receive_remote_upgrade_response,
            )

            receive_remote_upgrade_response(control_payload, sender=sender)
        _mark_complete(msg)
        return msg
...
def propagate(
    ...
    *,
    ...
    if is_remote_upgrade_control_message(message) and not allow_remote_upgrade_control:
        logger.warning(
            "Blocked propagation of remote upgrade control NetMessage %s",
            message.pk,
        )
    ...
    for node in selected:
        ...
        if success:
            ...
            message.queue_for_node(
                node,
                payload_seen,
                allow_remote_upgrade_control=allow_remote_upgrade_control,
            )
        message.propagated_to.add(node)
