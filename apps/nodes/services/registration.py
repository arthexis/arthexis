...
def register_current(node_model: type["Node"], notify_peers: bool = True) -> tuple["Node", bool]:
    ...
    if node is None:
        node = node_model.objects.filter(public_endpoint=slug).first()
    if node is None:
        node_qs = node_model.objects.filter(
            current_relation=node_model.Relation.SELF,
            mac_address=mac,
        )
        if host_instance_id is not None and host_instance_id != "":
            node_qs = node_qs.filter(host_instance_id__in=["", host_instance_id])
        node = node_qs.first()
