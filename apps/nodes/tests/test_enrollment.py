...
@pytest.mark.django_db
def test_submit_enrollment_public_key_allows_public_endpoint_match():
    node = Node.objects.create(
        hostname="node-endpoint-enrollment",
        mac_address="aa:bb:cc:dd:ee:73",
        address="198.51.100.73",
        port=8888,
        public_endpoint="node-endpoint-enrollment",
    )
    _, token = issue_enrollment_token(node=node)

    payload = {
        "mac_address": "aa:bb:cc:dd:ee:74",
        "public_endpoint": node.public_endpoint,
        "public_key": "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQCendpoint",
        "enrollment_token": token,
    }
    request = RequestFactory().post(
        "/nodes/register/enrollment-public-key/",
