...
def test_start_server_noops_when_unix_stream_server_is_unavailable(monkeypatch):
    monkeypatch.setattr(sibling_ipc, "_SERVER_THREAD", None)
    monkeypatch.setattr(sibling_ipc, "_SiblingIPCServer", None)
    monkeypatch.setattr(
        sibling_ipc.Node,
        "get_local",
        lambda: pytest.fail("local node lookup should not run without socket support"),
    )

    assert sibling_ipc.start_server() is None
