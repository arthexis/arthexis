...
def test_detect_auto_feature_systemd_detection_matrix(
    ...
    role = NodeRole(name="Control") if slug == "rfid-scanner" else None
    node = Node(
...
def test_control_only_feature_assignment_is_inactive_on_non_control_node(
    tmp_path, slug, display
):
    ...
    feature = NodeFeature.objects.create(slug=slug, display=display)
    NodeFeatureAssignment.objects.bulk_create(
        [NodeFeatureAssignment(node=node, feature=feature)]
    )

    assert node.has_feature(slug) is False
    assert feature.is_enabled is False


@pytest.mark.django_db
def test_detect_auto_feature_skips_control_only_hook_on_non_control_node(
    monkeypatch, tmp_path
):
    """Role-ineligible features should not call app detection hooks."""

    node = Node(
        hostname="satellite-rfid-node",
        base_path=str(tmp_path),
        public_endpoint="satellite-rfid-node",
        role=NodeRole(name="Satellite"),
    )
    monkeypatch.setattr(
        node_feature_detection_registry,
        "detect",
        lambda *args, **kwargs: pytest.fail("RFID detection hook should not run"),
    )
