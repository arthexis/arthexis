...
from apps.ocpp.public_routes import PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE


@pytest.mark.django_db
def test_network_charger_action_accepts_ocpp_control_enrollment_token(
    client, monkeypatch
):
    ...
    charger = Charger.objects.create(
        charger_id="CP-1", manager_node=manager_node, allow_remote=True
    )
    ...
    monkeypatch.setattr(
        "apps.nodes.views.ocpp._require_local_origin", lambda _charger: True
    )
    monkeypatch.setitem(
        ocpp_views.REMOTE_ACTIONS,
        "noop",
        lambda _charger, _payload=None: (True, "sent", {}),
    )
...
@pytest.mark.django_db
def test_network_charger_action_reports_disabled_diagnostics_callback(
    client, monkeypatch
):
    manager_role = NodeRole.objects.create(name="Gateway")
    manager_node = Node.objects.create(hostname="manager-node", role=manager_role)
    ...
    monkeypatch.setattr("apps.nodes.views.ocpp._require_local_origin", lambda _: True)
    monkeypatch.setattr(
        "apps.nodes.views.ocpp.reverse_public_ocpp_route",
    ...
    response = client.post(
        "/nodes/network/chargers/action/",
        data=json.dumps(
            {"charger_id": charger.charger_id, "action": "request-diagnostics"}
        ),
        content_type="application/json",
        HTTP_AUTHORIZATION=f"Bearer {token}",
    )
    ...
    assert response.json() == {
        "status": "error",
        "detail": str(PUBLIC_OCPP_ROUTES_DISABLED_MESSAGE),
        "updates": {},
    }
