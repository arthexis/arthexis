...
def test_should_enqueue_startup_message_for_runserver_and_asgi_entrypoints(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.nodes.apps.sys.argv", ["daphne", "config.asgi:application"]
    )
    assert config._should_enqueue_startup_message() is True

    monkeypatch.setattr(
        "apps.nodes.apps.sys.argv", ["gunicorn", "config.asgi:application"]
    )
    assert config._should_enqueue_startup_message() is True

    monkeypatch.setattr(
        "apps.nodes.apps.sys.argv", ["python", "manage.py", "runserver"]
    )
    assert config._should_enqueue_startup_message() is True


def test_ready_sends_startup_net_message_without_celery_enqueue(monkeypatch):
    config = _nodes_config()
...
def test_ready_sends_startup_net_message(monkeypatch):
    config = _nodes_config()
    captured: dict[str, str | None] = {}
