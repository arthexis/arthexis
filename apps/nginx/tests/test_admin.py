...
@pytest.mark.django_db
def test_generate_certificates_view_provisions_linked_certificate(
    ...
    config = SiteConfiguration.objects.create(
        name="with-cert", protocol="https", certificate=certificate
    )
    ...
    url = (
        reverse("admin:nginx_siteconfiguration_generate_certificates")
        + f"?ids={config.pk}"
    )
    response = admin_client.post(url, {"ids": str(config.pk)}, follow=True)
...
@pytest.mark.django_db
def test_generate_certificates_view_creates_certificate_when_missing(
...
def test_generate_certificates_view_creates_godaddy_certificate_when_selected(
    ...
    config = SiteConfiguration.objects.create(name="missing-godaddy", protocol="https")
    ...
    response = admin_client.post(
        url, {"ids": str(config.pk), "certificate_type": "godaddy"}, follow=True
    )
    ...
    assert isinstance(certbot, CertbotCertificate)
