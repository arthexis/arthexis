"""Certificate renewal flow for the https command."""
    ...
    *,
    ...
    for certificate in candidate_list:
        ...
        certificate_file_missing = (
            bool(certificate.certificate_path) and not Path(certificate.certificate_path).exists()
        )
        if refreshed_expiration is None and (
            certificate_file_missing or (stored_expiration is not None and stored_expiration <= now)
        ):
            due_certificates.append(certificate)
