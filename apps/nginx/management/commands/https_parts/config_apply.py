"""Site configuration lookup and apply helpers for the https command."""
...
from .constants import NGINX_CONFIGURE_REMEDIATION_TEMPLATE


def _get_existing_config(domain: str) -> SiteConfiguration | None:
    """Return an existing site configuration for a domain."""
...
def _get_or_create_config(domain: str, *, protocol: str) -> SiteConfiguration:
    """Fetch or initialize a site configuration with the target protocol."""
    ...
    if created:
        ...
        config.save()
    elif config.protocol != protocol or not config.enabled:
        config.protocol = protocol
        config.enabled = True
...
def _apply_config(service, config: SiteConfiguration, *, reload: bool) -> None:
    """Apply nginx config and normalize error/status messaging."""
    ...
    if not result.reloaded:
        service.stdout.write("nginx reload/start did not complete automatically; check the service status.")
