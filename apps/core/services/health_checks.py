...
def run_check_lcd_send(
    ...
    **_kwargs,
    ...
    normalized_type = manager._normalize_channel_type(
        lcd_channel_type, sticky=lcd_sticky
    )
    ...
    stdout.write(
        f"Sending test message to LCD: subject='{lcd_subject}' body='{lcd_body}'"
    )
...
def run_check_lcd_service(
    *, stdout, style, lcd_confirmed: bool = False, **_kwargs
) -> None:
    ...
    if service_file.exists():
        ...
        try:
            result = subprocess.run(
                ["systemctl", "is-active", lcd_service], capture_output=True, text=True
            )
            ...
            stdout.write(
                style.WARNING("systemctl not available; cannot verify service")
            )
    else:
        stdout.write(
            style.WARNING("Service lock file missing; cannot determine LCD service")
        )
    ...
    stdout.write(
        style.WARNING(
            "LCD display not confirmed by user; run with --lcd-confirmed once validated"
        )
    )
