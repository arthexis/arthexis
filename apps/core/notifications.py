...
CUSTOM_CHANNEL_PATTERN = re.compile(r"^[a-z0-9_-]+$")
...
class NotificationManager:
    ...
    @staticmethod
    def _normalize_channel_type(
        channel_type: str | None, *, sticky: bool = False
    ) -> str:
        ...
        if normalized in {channel.value for channel in LcdChannel}:
            return normalized
        if CUSTOM_CHANNEL_PATTERN.fullmatch(normalized):
            return normalized
        return default_type
