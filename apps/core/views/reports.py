...
def _sync_release_with_revision(
    ...
    if version_path.exists():
        ...
        if current_version and current_version != release.version:
            ...
            if conflict_release:
                return updated, previous_version, conflict_release
...
def _handle_release_sync(
    ...
    updated, previous_version, conflict_release = _sync_release_with_revision(release)
    if conflict_release:
        return _render_release_progress_error(
            request,
            release,
            action,
            _(
                "Another release already exists for version %(version)s. "
                "Select the existing release to publish instead."
            )
            % {"version": conflict_release.version},
            status=409,
            debug_info={
                "release_version": release.version,
                "repository_version": repo_version_before_sync,
                "conflict_release_id": conflict_release.pk,
            },
        )
    if updated:
