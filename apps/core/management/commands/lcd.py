...
class Command(BaseCommand):
    ...
    action_modules = {
        "debug": "apps.screens.management.commands.lcd_actions.debug",
        "replay": "apps.screens.management.commands.lcd_actions.replay",
        "write": "apps.screens.management.commands.lcd_actions.write",
        "animate": "apps.screens.management.commands.lcd_actions.animate",
        "plan": "apps.screens.management.commands.lcd_actions.plan",
        "calibrate": "apps.screens.management.commands.lcd_actions.calibrate",
    }

    def _load_action_commands(self):
        """Load available LCD action commands, skipping retired model-bound actions."""

        action_commands = {}
        for action, module_path in self.action_modules.items():
            try:
                module = import_module(module_path)
            except (ModuleNotFoundError, RuntimeError):
                continue
            action_commands[action] = module.Command
        return action_commands

    def add_arguments(self, parser):
        action_commands = self._load_action_commands()
        ...
        for action, command_class in action_commands.items():
    ...
    def handle(self, *args, **options):
        ...
        command_class = action_commands.get(action)
