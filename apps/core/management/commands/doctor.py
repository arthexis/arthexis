...
class Command(BaseCommand):
    ...
    help = (
        "Run Arthexis diagnostics and optionally invoke dedicated app doctor commands."
    )
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            "--all", action="store_true", help="Run every doctor target across groups."
        )
        parser.add_argument(
            "--list-targets",
            action="store_true",
            help="List available doctor targets and exit.",
        )
        parser.add_argument(
            "--force",
            action="store_true",
    ...
    def handle(self, *args, **options) -> None:
        ...
        if unknown:
            raise CommandError(
                f"Unknown doctor target/group selector(s): {', '.join(unknown)}"
            )
    ...
    def _list_targets(self) -> None:
        for definition in sorted(DOCTOR_TASKS.values(), key=lambda item: item.target):
            self.stdout.write(
                f"{definition.target} [group={definition.group}] - {definition.description}"
            )

    def _resolve_tasks(
        self, *, groups: list[str], targets: list[str]
    ) -> tuple[list[DoctorTaskDefinition], list[str]]:
    ...
    def _run_tasks(self, tasks: list[DoctorTaskDefinition], *, force: bool) -> int:
        ...
        for definition in tasks:
            if skip_reason := profile_skip_reason(
                ...
                self.stdout.write(
                    self.style.WARNING(f"[skipped] {definition.target}: {skip_reason}")
                )
                continue

            self.stdout.write(
                self.style.MIGRATE_HEADING(f"[doctor] {definition.target}")
            )
