...
def test_group_django_settings_normalizes_empty_prefix_to_other() -> None:
    """Settings with an empty prefix should not create a blank-named section."""
    grouped = _group_django_settings(
        [
            ("_SECRET", "x"),
            ("DEBUG", True),
    ...
    assert grouped == [
        {"name": "Other", "settings": [("DEBUG", True), ("_SECRET", "x")]}
    ]
...
def test_config_view_renders_grouped_sections(db, monkeypatch) -> None:
    ...
    assert '<nav aria-label="Config sections"' in response.rendered_content
    ...
    assert '<ul>' not in response.rendered_content
