...
def _invoke_startup_orchestrate(
    ...
    *,
    ...
    monkeypatch.setattr(settings, "NODE_ROLE", node_role, raising=False)
...
def test_startup_orchestrate_outputs_json_contract_and_writes_locks(tmp_path, monkeypatch):
    ...
    assert payload["features"]["lcd_enabled"] is True
    assert payload["features"]["lcd_lock_enabled"] is True
    assert payload["features"]["lcd_role_allowed"] is True
...
def test_startup_orchestrate_skips_lcd_when_feature_disabled(tmp_path, monkeypatch):
    ...
    assert payload["features"]["lcd_lock_enabled"] is False
    assert payload["features"]["lcd_role_allowed"] is True
    assert payload["launch"]["lcd_embedded"] is False


def test_startup_orchestrate_skips_lcd_on_non_control_role(tmp_path, monkeypatch):
    _, payload = _invoke_startup_orchestrate(
        tmp_path,
        monkeypatch,
        lcd_enabled=True,
    ...
    assert payload["startup_message_status"] == "skipped:lcd-disabled"
    assert payload["features"]["lcd_enabled"] is False
    assert payload["features"]["lcd_lock_enabled"] is True
    assert payload["features"]["lcd_role_allowed"] is False
    assert payload["launch"]["lcd_embedded"] is False
