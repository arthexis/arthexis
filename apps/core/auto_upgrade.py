...
AUTO_UPGRADE_FALLBACK_INTERVAL = AUTO_UPGRADE_INTERVAL_MINUTES[
...
AUTO_UPGRADE_FAILURE_GUIDE = (
    ...
    },
    {
        ...
        "details": "Post-upgrade health checks did not succeed; the failing revision is recorded to be skipped.",
        "action": "Check service logs, resolve the failure manually, then rerun the upgrade path.",
    },
    {
        "code": "PROC-ERR",
...
def _resolve_policy_interval_minutes() -> int:
    ...
    try:  # pragma: no cover - optional dependency failures
        ...
        return AUTO_UPGRADE_INTERVAL_MINUTES.get(
            "unstable", AUTO_UPGRADE_FAST_LANE_INTERVAL_MINUTES
        )
    ...
    intervals = [
        policy.interval_minutes for policy in policies if policy.interval_minutes
    ]
    if not intervals:
        return AUTO_UPGRADE_INTERVAL_MINUTES.get(
            "unstable", AUTO_UPGRADE_FAST_LANE_INTERVAL_MINUTES
...
def _get_or_create_interval_schedule(*, every: int, period: str):
    ...
    queryset = IntervalSchedule.objects.filter(every=every, period=period).order_by(
        "pk"
    )
...
def ensure_auto_upgrade_periodic_task(
    ...
    try:
        ...
        needs_update = (
            task.interval_id != schedule.pk
            or task.crontab_id is not None
            or task.solar_id is not None
            or task.clocked_id is not None
            or task.task != AUTO_UPGRADE_TASK_PATH
            or task.description != description
            or task.enabled != feature_enabled
        )
        if not needs_update:
            return
        ...
        task.solar = None
        task.clocked = None
        task.task = AUTO_UPGRADE_TASK_PATH
        task.description = description
        task.enabled = feature_enabled
