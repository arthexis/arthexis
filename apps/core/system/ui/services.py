"""Service status aggregation for the system UI.

Data flow: lifecycle unit metadata is loaded first, then each configured unit
is resolved to either embedded PID status or systemd status and returned as a
flat list consumed by status templates.

Expected input formats: service metadata dictionaries must contain ``unit`` and
``configured`` keys and may include a ``pid_file`` key for embedded mode.
"""
...
def _embedded_service_status(lock_dir: Path, pid_file: str) -> dict[str, object]:
    """Return synthetic status for embedded services tracked by PID files."""
...
def _build_services_report() -> dict[str, object]:
    """Build the full services section payload for the system UI."""
