from __future__ import annotations

import argparse

from apps.core.system import lifecycle
from apps.core.system.adoption import (
    execute_adoption,
    inspect_adoption,
    rollback_adoption,
)


class AdoptionArgumentError(ValueError):
    """Raised when managed adoption flags do not describe a safe operation."""


def _adoption_arguments(arguments: tuple[str, ...]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(add_help=False, exit_on_error=False)
    parser.add_argument("--adopt", action="store_true")
    parser.add_argument("--from", dest="source")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--allow-dirty-source", action="store_true")
    try:
        namespace, unknown = parser.parse_known_args(arguments)
    except argparse.ArgumentError as exc:
        raise AdoptionArgumentError(str(exc)) from exc
    if unknown:
        raise AdoptionArgumentError(
            f"unsupported adoption arguments: {' '.join(unknown)}"
        )
    return namespace


def install(
    *arguments: str,
    layout: lifecycle.InstallationLayout | None = None,
) -> lifecycle.InstallationLayout | dict[str, object]:
    """Run a normal managed install, adoption preflight, or adoption execution."""
    if "--adopt" not in arguments:
        return lifecycle.install(*arguments, layout=layout)

    options = _adoption_arguments(arguments)
    if not options.source:
        raise AdoptionArgumentError("--adopt requires --from PATH")

    target = lifecycle._resolve_layout(layout)
    if options.dry_run:
        if options.allow_dirty_source:
            raise AdoptionArgumentError(
                "--allow-dirty-source is only valid for adoption execution"
            )
        return inspect_adoption(options.source, target_root=target.root)

    if not target.checkout.is_dir() or not (target.root / ".venv").is_dir():
        raise AdoptionArgumentError(
            "adoption execution is not implemented outside a GWAY-created managed scaffold"
        )

    plan = execute_adoption(
        options.source,
        target_root=target.root,
        allow_dirty_source=options.allow_dirty_source,
    )
    try:
        return lifecycle.install("--role", str(plan["role"]), layout=target)
    except Exception:
        rollback_adoption(plan)
        raise
