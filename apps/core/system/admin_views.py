...
def _actions_app_installed() -> bool:
    if django_apps.apps_ready:
        return django_apps.is_installed("apps.actions")
    return "apps.actions" in settings.INSTALLED_APPS
...
def _ensure_default_staff_tasks_exist() -> None:
    if not _actions_app_installed():
        return
...
def _user_can_access_staff_task(user, task) -> bool:
    from apps.actions.staff_tasks import user_can_access_staff_task

    return user_can_access_staff_task(user, task)


def resolve_ws_scheme(*, request=None):
    from apps.ocpp.utils import resolve_ws_scheme as _resolve_ws_scheme

    return _resolve_ws_scheme(request=request)
...
def _system_view(request):
    ...
    _ensure_default_staff_tasks_exist()
    tasks = list(StaffTask.objects.filter(is_active=True).order_by("order", "label"))
    task_pref_map = {
        pref.task_id: pref
        for pref in StaffTaskPreference.objects.filter(
            user=request.user, task__in=tasks
        )
    }
    ...
    def _walk(patterns: list[URLPattern | URLResolver]) -> None:
        for pattern in patterns:
            ...
            if (
                ...
                and not _can_trigger_upgrade_checks(user)
            ):
                continue
...
def _system_restart_server_view(request):
    ...
    if not service_status["configured"] or not service_status["is_active"]:
        messages.error(
            request,
            _(
                "Server restart is only available when the suite is active under systemd."
            ),
        )
    ...
    try:
        ...
        messages.success(
            request, _("Restart requested for %(unit)s.") % {"unit": unit_name}
        )
