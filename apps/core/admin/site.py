...
def _get_application_priority_map(Application):
    ...
    return {
        name: priority
        for name, priority in Application.objects.filter(
            order__isnull=False
...
def _get_original_app_list_safely(self, request, app_label=None):
    """Return Django's admin app list while tolerating stale app labels."""

    try:
        return list(_original_admin_get_app_list(self, request, app_label=app_label))
    ...
    candidate_labels = sorted(
        {model._meta.app_label for model in getattr(self, "_registry", {})}
    )
    for candidate_label in candidate_labels:
        try:
            reverse(
                "admin:app_list",
                kwargs={"app_label": candidate_label},
                current_app=self.name,
            )
        except NoReverseMatch:
            continue
        entries.extend(
            _get_original_app_list_safely(self, request, app_label=candidate_label)
        )
    return entries


def get_app_list_with_application_priorities(self, request, app_label=None):
    """Sort the admin app list using optional Application priorities first."""
    try:
        Application = django_apps.get_model("app", "Application")
    except LookupError:
        return _get_original_app_list_safely(self, request, app_label=app_label)

    full_list = _get_original_app_list_safely(self, request, app_label=None)
    result = full_list
    ...
    ordered_result.sort(
        key=lambda entry: (
            ...
            (
                entry.get("_sort_priority")
                if entry.get("_sort_priority") is not None
                else 0
            ),
            entry.get("name"),
            entry.get("app_label"),
...
admin.AdminSite.get_app_list = get_app_list_with_application_priorities
