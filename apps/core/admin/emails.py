...
class EmailCollectorAdmin(EntityModelAdmin):
    list_display = (
        ...
        "body",
        "fragment",
        *(("odoo_customer_status",) if ODOO_APP_INSTALLED else ()),
        "notification_mode",
    )
    list_filter = ("is_enabled", "notification_mode")
    list_select_related = (
        ("inbox", "odoo_profile") if ODOO_APP_INSTALLED else ("inbox",)
    )
    search_fields = (
        "name",
        "subject",
    ...
    def odoo_customer_status(self, obj):
        if not getattr(obj, "odoo_profile_id", None):
            return "-"
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if (
            ODOO_APP_INSTALLED
            and db_field.name == "odoo_profile"
