...
def _service_docs_url(doc: str) -> str:
    """Return the external source URL for a service document slug."""

    normalized = doc.strip().lstrip("/")
    if not normalized:
        return ""
    if not Path(normalized).suffix:
        normalized = f"{normalized}.md"
    return f"https://github.com/arthexis/arthexis/blob/main/docs/{normalized}"
...
def _systemd_unit_exists(unit_name: str) -> bool:
    """Return whether systemd currently knows a unit file."""
    ...
    try:
        result = subprocess.run(
            [systemctl, "list-unit-files", "--no-legend", "--no-pager", unit_name],
            capture_output=True,
            check=False,
            text=True,
            timeout=10,
        ...
        logger.warning("Unable to inspect systemd unit %s", unit_name, exc_info=True)
        return False

    return any(
        line.split(maxsplit=1)[0] == unit_name
...
def build_lifecycle_service_units(
    ...
    lifecycle_services = _iter_lifecycle_services()
    ...
    if lifecycle_services:
        known_units = {
            str(unit["unit_display"])
            for unit in service_units
            if unit.get("unit_display")
        }
        for unit_name in _read_extra_systemd_units(locks / SYSTEMD_UNITS_LOCK):
            if unit_name not in known_units and _systemd_unit_exists(unit_name):
                _add_unit(
                    service_units,
                    unit_name,
                    configured=True,
...
def reconcile_node_features_and_services(
    ...
    try:
        ...
        if node is not None:
            ...
            try:
                from apps.nodes.models import NodeFeature, NodeFeatureAssignment

                celery_feature = NodeFeature.objects.filter(slug="celery-queue").first()
                if (
                    celery_feature is not None
                    and not node.has_feature("celery-queue")
                    and node._detect_auto_feature(
                        "celery-queue",
                        base_dir=resolved_base,
                        base_path=node.get_base_path(),
                    )
                ):
                    _, created = NodeFeatureAssignment.objects.get_or_create(
                        node=node,
                        feature=celery_feature,
                    )
                    if created:
                        node.sync_feature_tasks()
            except Exception:
                logger.exception(
                    "Failed to reconcile celery-queue feature drift on local node"
                )
