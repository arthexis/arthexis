"""Regression coverage for retired blog application cleanup migration."""

from __future__ import annotations

from importlib import import_module

migration = import_module("apps.app.migrations.0011_remove_retired_blog_application")
...
def test_remove_retired_blog_applications_deletes_both_historical_names() -> None:
    """Regression: cleanup should remove both label and module-form blog rows."""

    manager = StubApplicationManager()
    StubApplication.objects = manager

    migration.remove_retired_blog_applications(StubApps(), schema_editor=None)

    assert manager.filtered_names == migration.RETIRED_BLOG_APPLICATION_NAMES
