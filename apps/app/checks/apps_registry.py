...
def get_apps_registry_configuration_errors() -> list[Error]:
    ...
    optional_project_local_apps = list(
        getattr(settings, "OPTIONAL_PROJECT_LOCAL_APPS", [])
    )
    ...
    installed_project_local_apps = [
        app_path
        for app_path in installed_apps
        if app_path in {*project_local_apps, *optional_project_local_apps}
    ]
    for app_path in installed_project_local_apps:
        try:
            import_module(_get_base_module(app_path))
        except ImportError as exc:
            errors.append(
                Error(
                    f"Installed PROJECT_LOCAL_APPS entry '{app_path}' could not be imported.",
...
def enforce_apps_registry_configuration() -> None:
    ...
    raise ImproperlyConfigured("App registry configuration is invalid:\n" + message)
