...
from apps.core.admin import ProfileAdminMixin
...
def test_user_get_profile_resolves_user_owned_profile():
    user = User.objects.create_user(username="diag-profile-owner")
    profile = UserDiagnosticsProfile.objects.create(
        user=user,
        is_enabled=True,
    ...
    assert user.get_profile(UserDiagnosticsProfile) == profile


@pytest.mark.django_db
def test_profile_admin_info_resolves_user_owned_profile():
    user = User.objects.create_superuser(username="diag-profile-admin")
    profile = UserDiagnosticsProfile.objects.create(
        user=user,
        is_enabled=True,
        collect_diagnostics=True,
    ...
    request = RequestFactory().get("/admin/users/userdiagnosticsprofile/")
    request.user = user

    class UserDiagnosticsProfileTestAdmin(ProfileAdminMixin, django_admin.ModelAdmin):
        pass

    admin = UserDiagnosticsProfileTestAdmin(UserDiagnosticsProfile, AdminSite())

    assert admin._get_user_profile_info(request) == (user, profile, 1)
...
def test_manual_feedback_and_bundle_generation_command():
    ...
    create_manual_feedback(
        ...
        details="Observed after software update on control node.",
    )
    create_manual_feedback(
        user=user,
