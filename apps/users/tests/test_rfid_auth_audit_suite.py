...
class RFIDAuthAuditSuiteTests(TestCase):
    ...
    def test_rfid_login_returns_safe_redirect(self) -> None:
        self._set_audit_feature(enabled=False)
        tag = RFID.objects.create(rfid="A1B2C3D4", allowed=True)
        user = get_user_model().objects.create_user(
            username="rfid_redirect_user",
            password="password123",
            login_rfid=tag,
        )
        ...
        response = self.client.post(
            reverse("rfid-login"),
            data='{"rfid":"a1b2c3d4", "next":"/release-checklist/"}',
            content_type="application/json",
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["redirect"], "/release-checklist/")
