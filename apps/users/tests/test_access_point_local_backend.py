...
from apps.users.backends import AccessPointLocalUserBackend, LocalhostAdminBackend


class LocalhostAdminBackendTests(TestCase):
    def setUp(self):
        self.factory = RequestFactory()
        self.backend = LocalhostAdminBackend()
    ...
    def test_rejects_default_admin_from_recovery_ap_network(self):
        with self.assertRaises(PermissionDenied):
            self.backend.authenticate(
                self._request("10.42.0.23"),
                username="admin",
                password="admin",
            )

    def test_rejects_default_admin_from_recovery_ap_network_before_password_backend(
        self,
    ):
        get_user_model().objects.create_user(username="admin", password="admin")

        authenticated = authenticate(
            request=self._request("10.42.0.23"),
            username="admin",
            password="admin",
        )
    ...
    def test_accepts_default_admin_from_loopback(self):
