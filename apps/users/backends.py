...
_RECOVERY_AP_NETWORK = ".".join(("10", "42", "0", "0")) + "/16"
...
class LocalhostAdminBackend(ModelBackend):
    ...
    _ALLOWED_NETWORKS = (
        ipaddress.ip_network(_RECOVERY_AP_NETWORK),
    ...
    _DEFAULT_ADMIN_EXCLUDED_NETWORKS = (ipaddress.ip_network(_RECOVERY_AP_NETWORK),)
    ...
    def _is_default_admin_remote_allowed(self, ip):
        if any(ip in net for net in self._DEFAULT_ADMIN_EXCLUDED_NETWORKS):
            return False
        return self._is_remote_allowed(ip)
