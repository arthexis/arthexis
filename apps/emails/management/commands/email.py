...
class Command(BaseCommand):
    ...
    def _add_inbox_arguments(
        ...
        password_flags = ["--inbox-password", "--set-password"]
        password_env_flags = ["--inbox-password-env", "--set-password-env"]
        password_file_flags = ["--inbox-password-file", "--set-password-file"]
        prompt_password_flags = ["--inbox-prompt-password", "--prompt-password"]
        ...
        parser.add_argument(
            *password_env_flags,
            ...
            help=help_text or "Environment variable containing the inbox password.",
        )
        parser.add_argument(
            *password_file_flags,
            dest="inbox_password_file",
            help=help_text or "File containing the inbox password.",
        )
        parser.add_argument(
            *prompt_password_flags,
            dest="inbox_prompt_password",
            action="store_true",
    ...
    def _normalize_options(self, options: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "inbox" and normalized.get("inbox_selector") is not None:
            normalized["inbox"] = normalized["inbox_selector"]
        ...
        self._normalize_inbox_password(normalized)
        if normalized.get("inbox") is not None:
            normalized["inbox"] = self._resolve_inbox_selector(normalized["inbox"])
        return normalized

    def _normalize_inbox_password(self, options: dict[str, Any]) -> None:
        """Map all inbox password input variants onto ``inbox_password``."""

        inline_password = options.get("inbox_password")
        password_env = options.get("inbox_password_env")
        password_file = options.get("inbox_password_file")
        prompt_password = bool(options.get("inbox_prompt_password"))
        source_count = sum(
            [
                inline_password is not None,
                bool(password_env),
                bool(password_file),
                prompt_password,
            ]
        ...
        if source_count > 1:
            raise CommandError(
                "Provide only one of --set-password, --set-password-env, "
                "--set-password-file, or --prompt-password."
            )
        ...
        if password_env:
            ...
            options["inbox_password"] = env_password
            return
        if password_file:
            try:
                file_password = (
                    Path(password_file).read_text(encoding="utf-8").rstrip("\r\n")
                )
            except OSError as exc:
                raise CommandError(
                    f"Unable to read password file {password_file}: {exc}"
                ) from exc
            if not file_password:
                raise CommandError(f"Password file {password_file} is empty.")
            options["inbox_password"] = file_password
            return

        prompt_value = getpass.getpass("Inbox password: ")
        if not prompt_value:
            raise CommandError("No inbox password was provided.")
        options["inbox_password"] = prompt_value

    def _resolve_inbox_selector(self, selector: Any) -> int:
        """Resolve an inbox id or username/email selector to a primary key."""
        ...
        if not raw_selector:
            raise CommandError("Inbox selector cannot be blank.")
        ...
        matches = list(
            EmailInbox.objects.filter(username__iexact=raw_selector).order_by("id")
        )
        if not matches:
            raise CommandError(
                f"No inbox found for username/email '{raw_selector}'. "
                "Create or configure the inbox first, then rerun the update."
            )
        if len(matches) > 1:
            matching_ids = ", ".join(str(inbox.pk) for inbox in matches)
            raise CommandError(
                f"Multiple inboxes match username/email '{raw_selector}': "
                f"{matching_ids}. "
                "Use the numeric inbox id instead."
            )
        return matches[0].pk
    ...
    def _configure_inbox(self, options: dict[str, Any]) -> EmailInbox:
        ...
        if inbox.pk is None and inbox.user_id is None and inbox.group_id is None:
    ...
    def _configure_outbox(self, options: dict[str, Any]) -> EmailOutbox:
        ...
        if outbox.pk is None and outbox.user_id is None and outbox.group_id is None:
    ...
    def _report_inboxes(self, inbox_id: int | None = None) -> None:
        ...
        inboxes = EmailInbox.objects.select_related("user", "group")
        if inbox_id is not None:
    ...
    def _report_outboxes(self, outbox_id: int | None = None) -> None:
        ...
        outboxes = EmailOutbox.objects.select_related("user", "group", "node")
        if outbox_id is not None:
    ...
    def _report(self) -> None:
        ...
        report = {
            "inboxes": [
                ...
                for inbox in EmailInbox.objects.select_related("user", "group")
            ],
            "outboxes": [
                {
                    "id": outbox.pk,
