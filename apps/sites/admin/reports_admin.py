...
def _build_log_dashboard(
    logs_dir: Path, available_logs: list[str]
) -> dict[str, object]:
    ...
    for filename in available_logs:
        ...
        try:
            ...
            modified_label = timezone.localtime(modified_at).strftime(
                "%Y-%m-%d %H:%M:%S %Z"
            )
        except OSError:
            modified_label = ""
    ...
    level_rows = [
        {"level": level, "count": count} for level, count in level_totals.items()
    ]
    return {
...
def _read_recent_log_content(
    file_path: Path, max_lines: int = VIEWER_LINE_LIMIT
) -> str:
...
def log_viewer(request):
    ...
    dashboard = (
        _build_log_dashboard(logs_dir, available_logs)
        if available_logs
        else {
            "total_files": 0,
            "total_lines": 0,
    ...
    if selected_log:
        if selected_log in available_logs:
            ...
            try:
                ...
                logger.warning(
                    "Unable to read log file %s", selected_path, exc_info=exc
                )
                log_error = _(
