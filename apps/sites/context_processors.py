...
from .favicons import FAVICON_FILENAMES, load_favicon_data_uri
from .models import SiteHighlight, SiteModuleVisibility, SiteTemplate

_FAVICON_FILENAMES = FAVICON_FILENAMES
_DEFAULT_FAVICON = load_favicon_data_uri(_FAVICON_FILENAMES["default"])
_ROLE_FAVICONS = {
    role: (load_favicon_data_uri(filename) or _DEFAULT_FAVICON)
...
def _build_funding_banner(request):
    ...
    resolved_issue_url = str(
        getattr(settings, "ARTHEXIS_FUNDING_ISSUE_URL", "") or DEFAULT_FUNDING_ISSUE_URL
    ).strip()
    if not resolved_issue_url or not _github_issue_api_url(resolved_issue_url):
        return None
...
def _site_module_visibility_audiences(user) -> set[str]:
    """Return site visibility audiences matching the current request user."""

    if getattr(user, "is_authenticated", False):
        return {
            SiteModuleVisibility.AUDIENCE_ALL,
            SiteModuleVisibility.AUDIENCE_AUTHENTICATED,
        }
    return {
        SiteModuleVisibility.AUDIENCE_ALL,
        SiteModuleVisibility.AUDIENCE_ANONYMOUS,
    }


def _load_site_module_visibility_overrides(
    site, user, feature_checker, user_group_ids: set[int]
) -> tuple[list[Module], set[int]]:
    """Return explicit show modules and hidden module ids for ``site`` and ``user``."""
    ...
    try:
        rules = list(
            SiteModuleVisibility.objects.filter(
                site=site,
                is_enabled=True,
                audience__in=_site_module_visibility_audiences(user),
                module__is_deleted=False,
            )
            .select_related("module", "module__application", "module__security_group")
            .order_by("module__priority", "module__path", "audience", "visibility")
        )
    except (OperationalError, ProgrammingError):
        return [], set()

    show_modules: list[Module] = []
    hidden_module_ids: set[int] = set()
    seen_show_ids: set[int] = set()
    for rule in rules:
        module = rule.module
        module_id = module.pk
        if rule.visibility == SiteModuleVisibility.VISIBILITY_HIDE:
            hidden_module_ids.add(module_id)
            continue
        if (
            rule.visibility == SiteModuleVisibility.VISIBILITY_SHOW
            and module_id not in seen_show_ids
        ):
            if not module.meets_feature_requirements(feature_checker.is_enabled):
                continue
            if not _module_allows_site_visibility_show(module, user, user_group_ids):
                continue
            show_modules.append(module)
            seen_show_ids.add(module_id)
    return show_modules, hidden_module_ids


def _module_allows_site_visibility_show(module, user, user_group_ids: set[int]) -> bool:
    """Return whether a site show rule may add ``module`` for ``user``."""

    if module.security_group_id and module.security_mode == Module.SECURITY_EXCLUSIVE:
        if not user:
            return False
        return (
...
def _merge_site_module_visibility_overrides(
    modules: list[Module], show_modules: list[Module], hidden_module_ids: set[int]
) -> tuple[list[Module], set[int]]:
    """Apply site visibility overrides to role-filtered navigation modules."""

    module_by_id = {module.pk: module for module in modules if module.pk is not None}
    site_added_show_ids: set[int] = set()
    for module in show_modules:
        if module.pk is None:
            continue
        if module.pk not in module_by_id:
            site_added_show_ids.add(module.pk)
            module_by_id[module.pk] = module

    visible_modules = [
        module
        for module_id, module in module_by_id.items()
        if module_id not in hidden_module_ids
    ]
    return visible_modules, site_added_show_ids - hidden_module_ids


def _module_has_audience_visible_landing(module) -> bool:
    """Return whether an annotated module has a landing usable by this audience."""

    return any(
        not getattr(landing, "nav_is_invalid", False)
        and not getattr(landing, "nav_is_locked", False)
        for landing in getattr(module, "enabled_landings", [])
    )
...
def _assign_module_menu(module):
    ...
    if module.path.rstrip("/").lower() == "/man":
        module.menu = "Manual"
    return module
...
def _annotate_module_landings(
    ...
    *,
    ...
    for landing in module.landings.filter(enabled=True, is_deleted=False):
...
def _load_header_references(request, site, node):
    """Return header references for compatibility with existing templates."""
...
def nav_links(request):
    ...
    show_modules, hidden_module_ids = _load_site_module_visibility_overrides(
        site, user, feature_checker, user_group_ids
    )
    candidate_modules, explicit_show_ids = _merge_site_module_visibility_overrides(
        candidate_modules,
        show_modules,
        hidden_module_ids,
    )
    ...
    for module in candidate_modules:
        ...
        if annotated_module is None:
            continue
        if module.pk in explicit_show_ids and not _module_has_audience_visible_landing(
            annotated_module
        ):
            continue
        annotated_modules.append(annotated_module)
    ...
    context = {
        ...
        "version_check_allowed": user_is_staff or user_is_superuser or is_site_operator,
