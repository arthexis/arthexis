...
/**
 * Draw faint row dividers only when the module pill list wraps.
 */
const setupNavbarWrapSeparators = () => {
  const lists = Array.from(document.querySelectorAll('[data-module-pill-list]'));
  if (!lists.length) {
    return;
  }

  const dividerClass = 'navbar-nav-row-divider';
  let frame = null;

  const removeDividers = list => {
    Array.from(list.querySelectorAll(`.${dividerClass}`)).forEach(divider => {
  ...
  const getPillItems = list => Array.from(list.children).filter(child => (
    child.classList && child.classList.contains('nav-item')
  ));
  ...
  const renderDividers = list => {
    removeDividers(list);
    const rows = getRows(getPillItems(list));
    if (rows.length > 1) {
      list.classList.add('navbar-nav-wrap-has-rows');
    } else {
      list.classList.remove('navbar-nav-wrap-has-rows');
    }
    if (rows.length < 2) {
      return;
    }
    ...
    rows.slice(1).forEach((row, index) => {
      ...
      const gap = Math.max(row.top - previousRow.bottom, 0);
      divider.className = dividerClass;
      divider.setAttribute('aria-hidden', 'true');
      divider.setAttribute('role', 'presentation');
      divider.style.top = `${previousRow.bottom + Math.round(gap / 2)}px`;
