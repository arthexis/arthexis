(() => {
  ...
  const FEEDBACK_CACHE_PREFIX = 'arthexis:user-story-feedback:';
  const FEEDBACK_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
  const FEEDBACK_CACHE_WRITE_DELAY_MS = 500;
  ...
  const cacheFieldTypesToSkip = new Set(['button', 'file', 'hidden', 'reset', 'submit']);
  ...
  const feedbackCacheKey = `${FEEDBACK_CACHE_PREFIX}${window.location.pathname}${window.location.search}`;
  ...
  let writeCacheTimeout = null;

  const getFeedbackCacheFields = () =>
    Array.from(form.elements).filter(field => {
      if (!field.name || field.disabled || field.readOnly || copyFieldNamesToSkip.has(field.name)) {
        return false;
      }
      return !cacheFieldTypesToSkip.has(field.type);
    });

  const removeFeedbackCache = () => {
    if (writeCacheTimeout) {
      clearTimeout(writeCacheTimeout);
      writeCacheTimeout = null;
    }
    try {
      window.sessionStorage.removeItem(feedbackCacheKey);
    } catch (error) {
      return;
    }
  };

  const readFeedbackCache = () => {
    let payload = null;
    try {
      payload = window.sessionStorage.getItem(feedbackCacheKey);
    } catch (error) {
      return null;
    }
    if (!payload) {
    ...
    try {
      ...
      if (!cached || !cached.expiresAt || cached.expiresAt <= Date.now() || !cached.fields) {
        removeFeedbackCache();
        return null;
      }
  ...
  const fieldHasFeedbackValue = field => {
  ...
  const writeFeedbackCache = () => {
    const fields = getFeedbackCacheFields();
    if (!fields.some(fieldHasFeedbackValue)) {
      removeFeedbackCache();
      return;
    }
    ...
    try {
      window.sessionStorage.setItem(feedbackCacheKey, JSON.stringify({
  ...
  const debouncedWriteFeedbackCache = () => {
    if (writeCacheTimeout) {
      clearTimeout(writeCacheTimeout);
    }
    writeCacheTimeout = window.setTimeout(() => {
      writeCacheTimeout = null;
      writeFeedbackCache();
    }, FEEDBACK_CACHE_WRITE_DELAY_MS);
  };

  const restoreFeedbackCache = () => {
    const values = readFeedbackCache();
    if (!values) {
      return;
    }
    getFeedbackCacheFields().forEach(field => {
  ...
  const initializeFeedbackCache = () => {
    restoreFeedbackCache();
    getFeedbackCacheFields().forEach(field => {
      field.addEventListener('input', debouncedWriteFeedbackCache);
      field.addEventListener('change', writeFeedbackCache);
    });
  };
