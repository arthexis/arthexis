...
def _content_type_for_model(model: type[Model]) -> ContentType | None:
    ...
    return ContentType.objects.get_for_models(model, for_concrete_models=False).get(
        model
    )
...
def admin_staff_tasks(context):
    """Return visible staff dashboard tasks when the actions app is installed."""

    if not apps.is_installed("apps.actions"):
        return []
...
def include_if_exists(context, template_name: str) -> str:
    ...
    try:
        ...
        return ""
    return mark_safe(
        template_obj.render(context.flatten(), request=context.get("request"))
    )
...
def _model_admin_action_eligibility(capabilities, seen_actions):
    ...
    hidden_by_queryset = (
        capabilities["skip_queryset_actions"] and capabilities["requires_queryset"]
    )
    is_visible = not (
...
def _configured_dashboard_action_descriptors(model):
    ...
    descriptors = []
    for configured_action in dashboard_action_model.objects.filter(
...
def _build_model_admin_action_descriptor(
    ...
    *,
    ...
    descriptor = {
        ...
        "method": (
            getattr(func, "dashboard_method", "get") if source == "dashboard" else "get"
        ),
...
def _normalized_model_admin_actions(request, model_admin):
    ...
    for action_name in _model_admin_named_actions(
        model_admin, "get_changelist_actions", request
    ):
        ...
        descriptor = _build_model_admin_action_descriptor(
            ...
            default_label=getattr(
                func, "short_description", action_name.replace("_", " ")
            ),
            request=request,
            source="changelist",
            seen_actions=seen_actions,
    ...
    for action_name in _model_admin_named_actions(
        model_admin, "get_dashboard_actions", request
    ):
        func = getattr(model_admin, action_name, None)
        if func is None:
            continue
