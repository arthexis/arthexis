...
class _GraphvizDeprecationFilter(logging.Filter):
    ...
    ) -> bool:  # pragma: no cover - logging hook
        try:
...
def admin_model_graph(request, app_label: str):
    ...
    if engine_path is None:
        graph_error = _(
            "Graphviz executables are required to render this diagram. Install the Graphviz system package on the server (for example: apt install graphviz) and ensure the Python 'graphviz' requirement is installed, then try again."
        )
    else:
        try:
...
class CustomLoginView(LoginView):
    ...
    def get_context_data(self, **kwargs):
        ...
        nfc_login_enabled = is_suite_feature_enabled(
            NFC_LOGIN_FEATURE_SLUG, default=False
        )
        if node:
        ...
        if context["show_rfid_login"]:
            rfid_login_url = reverse("pages:rfid-login")
            if redirect_value:
                rfid_login_url = (
                    f"{rfid_login_url}?"
                    f"{urlencode({self.redirect_field_name: redirect_value})}"
                )
            context["rfid_login_url"] = rfid_login_url
        context["hide_default_footer"] = True
        return context

    def get_success_url(self):
        redirect_url = self.get_redirect_url()
        if redirect_url and _is_resolvable_login_redirect(redirect_url):
            return redirect_url
...
def _resolve_login_redirect(request, redirect_target: str) -> str:
    ...
    if (
        ...
        and _is_resolvable_login_redirect(redirect_target)
    ):
...
def _is_resolvable_login_redirect(redirect_url: str) -> bool:
    """Avoid returning an authenticated user to a retired or missing route."""

    try:
        resolve(urlparse(redirect_url).path)
    except Resolver404:
        return False
    return True
...
def passkey_login_verify(request):
    ...
    try:
        ...
        return JsonResponse(
            {"detail": _("Invalid passkey response payload.")}, status=400
        )
    ...
    redirect_url = _resolve_login_redirect(request, redirect_target)
