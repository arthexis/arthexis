...
def test_ensure_security_groups_exist_repairs_missing_child_rows(db):
    ...
    original_group = SecurityGroup.objects.create(name=SITE_OPERATOR_GROUP_NAME)
    with connection.cursor() as cursor:
        cursor.execute(
            f"DELETE FROM {SecurityGroup._meta.db_table} WHERE group_ptr_id = %s",
            [original_group.pk],
        )

    group = ensure_security_groups_exist([SITE_OPERATOR_GROUP_NAME])[
        SITE_OPERATOR_GROUP_NAME
    ]

    assert isinstance(group, SecurityGroup)
    assert group.pk == original_group.pk
    assert Group.objects.filter(
        pk=original_group.pk, name=SITE_OPERATOR_GROUP_NAME
    ).exists()
    assert SecurityGroup.objects.filter(
        pk=original_group.pk, name=SITE_OPERATOR_GROUP_NAME
    ).exists()
