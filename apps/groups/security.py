...
def ensure_security_groups_exist(
    ...
    ordered_names = tuple(
        dict.fromkeys(name.strip() for name in group_names if name and name.strip())
    )
    ...
    groups = {
        group.name: group
        for group in SecurityGroup.objects.filter(name__in=ordered_names)
    }
    missing_names = [name for name in ordered_names if name not in groups]
    if missing_names:
        with transaction.atomic():
            for name in missing_names:
                base_group, _created = Group.objects.get_or_create(name=name)
                group, _created = SecurityGroup.objects.get_or_create(
                    group_ptr_id=base_group.pk,
                    defaults={"name": name},
                )
                groups[name] = group
...
def get_default_staff_group_names(
    user, *, explicit_group_names: Iterable[str] | None = None
) -> tuple[str, ...]:
...
def ensure_camera_supervisor_group_for_user(username: str) -> bool:
    """Add the Camera Supervisor security group to an existing user by username."""

    username = (username or "").strip()
    if not username:
        return False

    group = ensure_security_groups_exist((CAMERA_SUPERVISOR_GROUP_NAME,))[
        CAMERA_SUPERVISOR_GROUP_NAME
    ]
    user_model = get_user_model()
...
def ensure_default_staff_groups(
    ...
    if getattr(user, "pk", None) is None:
        raise ValueError(
            "User must be saved before default staff groups can be ensured."
        )

    group_names = get_default_staff_group_names(
        user, explicit_group_names=explicit_group_names
    )
    if not group_names:
        return ()
    ...
    groups_to_add = [
        group for group in groups_by_name.values() if group.pk not in existing_group_ids
    ]
