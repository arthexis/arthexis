...
class Command(BaseCommand):
        ...
        try:
            ...
            for backend in backends:
                try:
                    ...
                    self._write_preview_index(captures=captures, output_dir=output_dir)
                    return
    ...
    def _build_capture_plan(
        ...
        *,
        ...
        use_legacy_output = len(paths) == 1 and output_dir == output.parent
        for path in paths:
    ...
    def _print_reports(self, captures: list[dict[str, object]]) -> None:
        ...
        self._print_capture_manifest(captures)

    def _print_capture_manifest(self, captures: list[dict[str, object]]) -> None:
        """Print a concise, reviewer-friendly summary of generated screenshot artifacts."""

        self.stdout.write("Preview manifest:")
        for capture in captures:
            output = capture["output"]
            self.stdout.write(
                f"- {capture['path']} [{capture['viewport_name']}]: "
                f"{self._display_path(output)}"
            )

    def _write_preview_index(self, *, captures: list[dict[str, object]], output_dir: Path) -> None:
        """Write a markdown index so preview artifacts are easy to reference in reviews."""

        index_path = output_dir / "README.md"
        if index_path.exists():
            existing_content = index_path.read_text(encoding="utf-8")
            generated_marker = "Generated by `python manage.py preview`."
            if generated_marker not in existing_content:
                index_path = output_dir / "PREVIEW_INDEX.md"
                self.stderr.write(
                    self.style.WARNING(
                        "Refusing to overwrite existing README.md in output directory; "
                        "wrote preview index to PREVIEW_INDEX.md instead."
                    )
                )
        lines = [
            "# Preview Index",
            "",
            "Generated by `python manage.py preview`.",
            "",
            "Use this file in PR descriptions or local review notes to link directly to each capture.",
            "",
        ]

        captures_by_path: dict[str, dict[str, Path]] = {}
        for capture in captures:
            captures_by_path.setdefault(capture["path"], {})[capture["viewport_name"]] = capture["output"]

        viewport_order = list(DEFAULT_VIEWPORTS)
        for path, viewport_map in captures_by_path.items():
            lines.extend([f"## `{path}`", ""])
            for viewport_name in viewport_order:
                output = viewport_map.get(viewport_name)
                if output is None:
                    continue
                try:
                    relative_link = output.relative_to(output_dir).as_posix()
                except ValueError:
                    relative_link = self._display_path(output)
                lines.append(
                    f"- **{viewport_name}**: "
                    f"[{relative_link}]({relative_link}) "
                    f"— `{self._display_path(output)}`"
                )
            lines.append("")

        index_path.write_text("\n".join(lines), encoding="utf-8")
        self.stdout.write(self.style.SUCCESS(f"Saved preview index to: {self._display_path(index_path)}"))

    def _display_path(self, path: Path) -> str:
        """Return a repo-relative path when possible, otherwise fall back to absolute."""
