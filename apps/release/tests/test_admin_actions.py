...
import apps.release.admin as release_admin
...
def test_publish_action_redirects_when_repos_absent(
    db,
    monkeypatch,
    tmp_path,
) -> None:
    monkeypatch.setattr(release_admin, "REPOS_APP_INSTALLED", False)
    with override_settings(BASE_DIR=tmp_path):
        release = _build_release()
        request = RequestFactory().get("/admin/release/packagerelease/")
        request.session = {}
        ...
        monkeypatch.setattr(
            admin_view,
            "message_user",
            lambda request, message, level=None, **kwargs: messages.append(
                (str(message), level)
            ),
        )

        assert admin_view._release_workflow_ongoing(request, release) is False
        response = admin_view.publish_action(request, release)

    assert response is not None
    assert response.status_code == 302
    assert response.url == reverse("admin:release_packagerelease_changelist")
    assert messages
    assert "Repos app" in messages[0][0]
