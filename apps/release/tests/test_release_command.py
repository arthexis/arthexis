...
def test_builder_release_uploads_before_pushing_git_state(
    ...
    monkeypatch.setattr(
        "apps.release.services.builder._git_has_staged_changes", lambda: True
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda base_dir, *, generate_wheels: (
            ...
            (base_dir / "dist" / "package.tar.gz").write_text(
    ...
    monkeypatch.setattr(
        "apps.release.services.network.fetch_pypi_releases", lambda package: {}
    )
    monkeypatch.setattr(
...
def test_pep639_license_metadata_handles_current_and_legacy_arg_names(
    license_name: str, expected_license: str | None
) -> None:
...
def test_release_command_allows_blank_package_license() -> None:
    package = PackageModel.objects.create(
        name="release-command-blank-license",
        license="",
    )

    release_command.Command()._validate_package(package)


def test_promote_stages_only_release_metadata_files(
...
def test_promote_fails_when_unexpected_modified_files_exist(
    ...
    monkeypatch.setattr(
        ...
        lambda package: (
            [Path("VERSION")],
            [Path("README.tmp"), Path("apps/release/tests/test_x.py")],
        ),
    )
    monkeypatch.setattr(
        "apps.release.services.builder._run",
...
def test_release_metadata_paths_for_promote_uses_package_version_path(
    ...
    monkeypatch.setattr(
        "apps.release.services.builder._git_modified_paths", lambda: modified
    )
...
def test_git_modified_paths_handles_rename_and_ignored_paths(
    ...
    assert _git_modified_paths() == {
        Path("pyproject.toml"),
        Path("new/name -> value.txt"),
    }
...
def test_run_tests_defaults_to_pytest(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    run_tests(log_path=tmp_path / "test.log")
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ".venv/bin/python -m pytest -m smoke -k billing",
            [
                ".venv/bin/python",
                "-m",
                "pytest",
...
def test_normalize_test_command_rejects_unsupported_wrapper_flags() -> None:
    with pytest.raises(ValueError, match="Unsupported flag"):
        normalize_test_command(".venv/bin/python -m pytest --pdb")
