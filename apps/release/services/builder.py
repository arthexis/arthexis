...
def _release_metadata_paths_for_promote(
    package: Package,
...
def run_tests(
    ...
    cmd = list(command) if command is not None else [sys.executable, "-m", "pytest"]
...
def _pep639_license_metadata(license_name: str) -> dict[str, object]:
    ...
    if license_name in ARG_LICENSE_NAMES:
        return {"license": ARG_LICENSE_REF}
    ...
    try:
        ...
        if dist:
            ...
            _build_in_sanitized_tree(
                Path.cwd(), generate_wheels=package.generate_wheels
            )
