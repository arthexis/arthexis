...
MANAGED_DEFAULT_TEST_ARGV = (sys.executable, "-m", "pytest")

TEST_COMMAND_POLICIES: tuple[TestCommandPolicy, ...] = (
    TestCommandPolicy(
        value=MANAGED_DEFAULT_TEST_COMMAND,
        label="Managed default (current Python -m pytest)",
        prefix=(),
        allowed_flags=frozenset(),
    ),
    TestCommandPolicy(
        value=".venv/bin/python -m pytest",
        label="Managed pytest wrapper (.venv/bin/python -m pytest)",
        prefix=(".venv/bin/python", "-m", "pytest"),
        allowed_flags=frozenset(
            {
                "-k",
                "-m",
...
def normalize_test_command(command: str | None) -> list[str] | None:
    ...
    Returns the managed pytest command for the managed default command.
    """

    value = (command or "").strip()
    if value == MANAGED_DEFAULT_TEST_COMMAND:
        return list(MANAGED_DEFAULT_TEST_ARGV)
