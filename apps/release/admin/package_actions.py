...
if REPOS_APP_INSTALLED:
    ...
    from apps.repos.task_utils import (
...
REPOS_REQUIRED_MESSAGE = _("Repository creation requires the Repos app.")
PACKAGE_CHANGELIST_ROUTE = "admin:release_package_changelist"
...
class PackageAdminActionsMixin:
    ...
    def create_repository_action(self, request, obj):
        if not REPOS_APP_INSTALLED:
            self.message_user(
                request,
                REPOS_REQUIRED_MESSAGE,
                messages.WARNING,
            )
            return redirect(PACKAGE_CHANGELIST_ROUTE)
