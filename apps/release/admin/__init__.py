...
REPOS_REQUIRED_MESSAGE = _("Release publishing requires the Repos app.")


class PackageAdmin(PackageAdminActionsMixin, SaveBeforeChangeAction, EntityModelAdmin):
    actions = ["create_repository_bulk_action"] if REPOS_APP_INSTALLED else []
    list_display = (
        "name",
        "description",
        "homepage_url",
    ...
    change_actions = (
        ["create_repository_action", "prepare_next_release_action"]
        if REPOS_APP_INSTALLED
        else ["prepare_next_release_action"]
    )


class PackageReleaseAdmin(SaveBeforeChangeAction, EntityModelAdmin):
    RELEASE_LOCKS_DIR = ".locks"
    ...
    actions = (
        ["publish_release", "validate_releases"]
        if REPOS_APP_INSTALLED
        else ["validate_releases"]
    )
    change_actions = ["release_action", "publish_action"] if REPOS_APP_INSTALLED else []
    ...
    refresh_from_pypi.label = _("Load from PyPI")
    refresh_from_pypi.short_description = _("Load from PyPI")
    ...
    def _publish_release(self, request, release):
        if not REPOS_APP_INSTALLED:
            self.message_user(request, REPOS_REQUIRED_MESSAGE, messages.WARNING)
            return redirect("admin:release_packagerelease_changelist")
    ...
    @classmethod
    def _release_workflow_ongoing(cls, request, release) -> bool:
        if not REPOS_APP_INSTALLED:
            return False
        ...
        lock_path = (
            ...
            / cls.RELEASE_LOCKS_DIR
            / f"release_publish_{release.pk}.json"
        )
        ...
        state = ReleaseContextState.from_dict(raw_ctx)
        done = state.step >= len(PUBLISH_STEPS) and not state.error
        return state.started and not done
