    ...
    *,
    ...
    if response.status_code == 404:
        ...
        response = request(
            ...
            token=token,
            expected_status={201},
            json={"tag_name": tag_name, "name": tag_name, "body": body},
        )
    ...
    if response.status_code == 200 and body and data.get("body") != body:
...
def upload_release_assets(
    *,
    ...
    for artifact in artifacts:
        ...
        upload_url = f"https://uploads.github.com/repos/{owner}/{repo}/releases/{release_id}/assets?name={name}"
    ...
    *,
    ...
    for run in runs:
        if not isinstance(run, dict):
            continue
        run_head_sha = (
            run.get("head_sha") if isinstance(run.get("head_sha"), str) else None
        )
    ...
    fallback_runs = (
        fallback_payload.get("workflow_runs")
