...
from . import pypi as pypi_checks
from .progress import (
    _build_release_progress_context,
    _build_release_progress_poll_response,
    _build_release_step_state,
    _build_release_step_states,
    _clean_redirect_path,
    _dedupe_preserve_order,
    _default_release_body,
    _finalize_release_progress_response,
    _github_release_body,
    _is_release_start_enabled,
    _normalize_release_summary,
    _render_release_progress_response,
    _resolve_next_step,
    _resolve_release_log_display,
    build_release_guidance,
)
from .pypi import (
    EXPECTED_PUBLISH_ENVIRONMENT,
    EXPECTED_PUBLISH_REF_PATTERN,
    EXPECTED_PUBLISH_WORKFLOW_FILE,
)

logger = logging.getLogger(__name__)
GIT_ADAPTER = SubprocessGitAdapter()
yaml = pypi_checks.yaml
...
def _check_release_version_not_on_pypi(release, log_path: Path) -> None:
    pypi_checks.check_release_version_not_on_pypi(
        release,
        log_path,
        append_log=_append_log,
        network_available=release_utils.network_available,
        request_timeout=PYPI_REQUEST_TIMEOUT,
...
def _versions_match(
    *,
    ...
    return pypi_checks.versions_match(
        candidate=candidate,
...
def _has_non_yanked_files(files: object) -> bool:
    return pypi_checks.has_non_yanked_files(files)
...
def _step_confirm_pypi_trusted_publisher_settings(
    ...
    _ = user
    pypi_checks.confirm_pypi_trusted_publisher_settings(
        release,
        ctx,
        log_path,
        append_log=_append_log,
        fail_release_gate=_fail_release_gate,
        now=timezone.now,
    )
...
def _step_export_and_dispatch(release, ctx, log_path: Path, *, user=None) -> None:
    ...
    release_data = _ensure_github_release(
        ...
        tag_name=tag_name,
        token=token,
        body=_github_release_body(release),
    )
...
def _pypi_release_available(release) -> bool:
    return pypi_checks.pypi_release_available(
        release,
        network_available=release_utils.network_available,
        request_timeout=PYPI_REQUEST_TIMEOUT,
        requests_get=requests.get,
...
def release_progress_impl(request, pk: int, action: str):
    ...
    if request.method == "POST" and (
        request.POST.get("set_release_summary") or "release_summary" in request.POST
    ):
        summary = _normalize_release_summary(request.POST.get("release_summary", ""))
        if release.release_summary != summary:
            release.release_summary = summary
            release.save(update_fields=["release_summary"])
        if request.POST.get("set_release_summary"):
            messages.success(request, _("Release description saved."))
