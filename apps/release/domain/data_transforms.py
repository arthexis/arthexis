...
_RETIRED_FEATURE_MODULE_PATHS = frozenset(
    {
        "/apps/docs/",
        "/awg/",
        "/docs/",
...
_RETIRED_FEATURE_LANDING_PATHS = frozenset(
    {
        "/apps/docs/",
        "/awg/",
        "/docs/library/",
...
_RETIRED_FEATURE_SLUGS = frozenset(
    {
...
_RETIRED_FEATURE_DISPLAYS = frozenset(
    {
        "Application Documents",
        "AWG Cable Sizing Calculator",
        "Calculator",
        "Calculators",
        "Camera Gallery",
        "Cameras",
...
def _delete_entities(queryset) -> int:
    deleted = 0
    for entity in list(queryset):
        entity.delete()
        deleted += 1
    return deleted


def _run_retired_feature_surface_cleanup(
    checkpoint: dict[str, object],
) -> TransformResult:
    """Remove retired feature pills and stale suite feature rows."""
    ...
    with transaction.atomic():
        landings = Landing.all_objects.filter(
            is_deleted=False,
            is_seed_data=True,
        ).filter(
            Q(module__path__in=_RETIRED_FEATURE_MODULE_PATHS)
            | Q(path__in=_RETIRED_FEATURE_LANDING_PATHS)
        )
        landing_count = _delete_entities(landings)

        modules = Module.all_objects.filter(
            is_deleted=False,
            is_seed_data=True,
            path__in=_RETIRED_FEATURE_MODULE_PATHS,
        )
        module_count = _delete_entities(modules)

        feature_pks = list(
            Feature.all_objects.filter(
                is_deleted=False,
                is_seed_data=True,
            )
            .filter(
                Q(slug__in=_RETIRED_FEATURE_SLUGS)
                | Q(display__in=_RETIRED_FEATURE_DISPLAYS)
            )
            .values_list("pk", flat=True)
        )
        feature_count = 0
        if feature_pks:
            retired_features = Feature.all_objects.filter(pk__in=feature_pks)
            retired_features.update(is_enabled=False)
            feature_count = retired_features.count()
            # Feature.objects does not hide soft-deleted rows, so these retired
            # seed rows must be physically removed instead of soft-deleted.
            models.QuerySet.delete(retired_features)

        processed = landing_count + module_count + feature_count
        return TransformResult(updated=processed, processed=processed, complete=True)
...
TRANSFORMS: dict[str, TransformRunner] = {
    ...
    "sites.remove_retired_feature_surfaces": _run_retired_feature_surface_cleanup,
