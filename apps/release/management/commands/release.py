...
def _assert_relative_to(base: Path, candidate: Path, *, label: str) -> None:
    ...
    try:
        ...
        raise BundleVerificationError(
            f"{label} escapes bundle directory: {candidate}"
        ) from exc
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        build_parser.add_argument(
            "--dist", action="store_true", help="Build distribution"
        )
        build_parser.add_argument(
            "--twine", action="store_true", help="Upload with Twine"
        )
        build_parser.add_argument(
        ...
        check_parser = subparsers.add_parser(
            "check-pypi",
            help="Run release.pypi health check for the requested release.",
        )
        ...
        simulate_parser.add_argument(
            "--summary-file", help="Write markdown summary to this file."
        )
    ...
    def _handle_simulate(self, options: dict[str, object]) -> None:
        ...
        emit_result(
            result,
            github_output=(
                Path(str(options["github_output"]))
                if options.get("github_output")
                else None
            ),
            summary_file=(
                Path(str(options["summary_file"]))
    ...
    def _handle_apply_migrations(self, options: dict[str, object]) -> None:
        ...
        installed_version = self._resolve_installed_version(
            options.get("installed_version")
        )
        if not RELEASE_VERSION_PATTERN.fullmatch(installed_version):
            raise BundleVerificationError(
                f"Invalid installed version: {installed_version!r}"
            )

        try:
            bundle_dir = self._resolve_bundle_dir(
                target_version, options.get("bundle_dir")
            )
            self._verify_bundle(bundle_dir)
            if installed_version == target_version:
                call_command(
                    "migrate", "--noinput", stdout=self.stdout, stderr=self.stderr
                )
                call_command(
                    "migrate", "--check", stdout=self.stdout, stderr=self.stderr
                )
                self._run_deferred_data_transforms(
                    skip=bool(options["skip_data_transforms"])
                )
                self.stdout.write(
                    self.style.SUCCESS(
            ...
            manifest = self._load_manifest(
                bundle_dir, installed_version, target_version
            )
            ...
            self.stderr.write(
                self.style.WARNING(f"{exc}. Falling back to Django migrate.")
            )
    ...
    def _resolve_installed_version(self, explicit_version: object) -> str:
        ...
        if not version_file.exists():
            raise BundleVerificationError(
                "Installed version could not be determined: VERSION file missing"
            )

        version = version_file.read_text(encoding="utf-8").strip()
        if not version:
            raise BundleVerificationError(
                "Installed version could not be determined: VERSION file empty"
            )
        return version
    ...
    def _resolve_bundle_dir(self, target_version: str, explicit_dir: object) -> Path:
        if explicit_dir:
            ...
            bundle_dir = (
                Path(settings.BASE_DIR) / "releases" / target_version / "migrations"
            ).resolve()
    ...
    def _verify_bundle(self, bundle_dir: Path) -> None:
        ...
        for line in checksum_file.read_text(encoding="utf-8").splitlines():
            ...
            if not artifact_path.exists():
                raise BundleVerificationError(
                    f"Bundle artifact is missing: {relative_path}"
                )
    ...
    def _load_manifest(
        ...
        manifest_path = (
            bundle_dir / "manifests" / f"{installed_version}__to__{target_version}.json"
        )
    ...
    def _run_deferred_data_transforms(self, *, skip: bool) -> None:
        ...
        try:
            ...
            self.stderr.write(
                self.style.WARNING(f"Deferred data transforms failed: {exc}")
            )
