...
from .. import Credentials, RepositoryTarget
from .. import Package as ReleasePackage
...
class PackageRelease(Entity):
    ...
    release_summary = models.TextField(
        ...
        blank=True,
        help_text=_(
            "Short human-written paragraph shown at the top of the GitHub release."
        ),
    )
    ...
    def scheduled_datetime(self) -> datetime_datetime | None:
        ...
        if not self.scheduled_date or self.scheduled_time is None:
            return None
        combined = datetime_datetime.combine(self.scheduled_date, self.scheduled_time)
    ...
    def migration_number(self) -> int:
        ...
        return (v.major << self._MAJOR_SHIFT) | (v.minor << self._MINOR_SHIFT) | v.micro
