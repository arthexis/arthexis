...
def test_video_command_lists_devices(capsys):
    ...
    with patch("apps.content.video.management.commands.video.Node.get_local", return_value=node):
        call_command("video")
...
def test_video_command_discover(capsys):
    ...
    with (
        patch("apps.content.video.management.commands.video.Node.get_local", return_value=node),
        patch(
            "apps.content.video.management.commands.video.VideoDevice.refresh_from_system",
            return_value=(1, 0),
        ) as refresh_mock,
    ):
...
def test_video_command_errors_without_node():
    ...
    with patch("apps.content.video.management.commands.video.Node.get_local", return_value=None):
...
def test_video_command_sample_creates_video(capsys, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.content.video.management.commands.video.WORK_DIR", tmp_path, raising=False
    )
    monkeypatch.setattr(
        VideoDevice,
    ...
    monkeypatch.setattr(
        "apps.content.video.management.commands.video.Command._encode_video", fake_encode
    )
...
@patch("apps.content.video.management.commands.video.get_status")
@patch("apps.content.video.management.commands.video.get_frame")
@patch("apps.content.video.management.commands.video.get_frame_cache")
@patch("apps.content.video.management.commands.video.MjpegStream")
@patch("apps.content.video.management.commands.video.VideoDevice")
@patch("apps.content.video.management.commands.video.NodeFeatureAssignment")
@patch("apps.content.video.management.commands.video.NodeFeature")
@patch("apps.content.video.management.commands.video.Node")
...
def test_video_command_snapshot_auto_enables_feature(node_mock, feature_mock, device_mock):
    ...
    with patch("apps.content.video.management.commands.video.NodeFeatureAssignment") as assignment_mock:
...
def test_video_command_mjpeg_capture_variants(
    ...
    with patch("apps.content.video.management.commands.video.get_frame", return_value=frame_value):
        kwargs = {"mjpeg": True}
...
def test_video_command_sample_camera_service_scenarios(
    ...
    monkeypatch.setattr("apps.content.video.management.commands.video.WORK_DIR", tmp_path, raising=False)
    monkeypatch.setattr(
        "apps.content.video.management.commands.video.frame_cache_url",
    ...
    monkeypatch.setattr("apps.content.video.management.commands.video.Command._encode_video", fake_encode)
    ...
    if scenario == "inactive_service":
        monkeypatch.setattr("apps.content.video.management.commands.video.get_status", lambda _stream: None)
        monkeypatch.setattr("apps.content.video.management.commands.video.get_frame", lambda _stream: None)
    elif scenario == "stale_status":
        monkeypatch.setattr(
            "apps.content.video.management.commands.video.get_status",
        ...
        monkeypatch.setattr(
            "apps.content.video.management.commands.video.get_frame",
            lambda _stream: SimpleNamespace(frame_bytes=b"frame-1", frame_id=1),
        )
        monkeypatch.setattr(
            "apps.content.video.management.commands.video.Command._CAMERA_SERVICE_FRAME_TIMEOUT_SECONDS",
            0.01,
        )
        monkeypatch.setattr(
            "apps.content.video.management.commands.video.Command._CAMERA_SERVICE_FRAME_POLL_SECONDS",
            0.001,
        )
...
def test_video_subaction_modes_match_legacy_flags(mode, capsys):
    ...
    if mode == "mjpeg":
        stream = SimpleNamespace(store_frame_bytes=Mock(), slug="stream-1")
        with (
            patch("apps.content.video.management.commands.video.Node.get_local", return_value=None),
            patch("apps.content.video.management.commands.video.MjpegStream") as stream_mock,
            patch("apps.content.video.management.commands.video.get_frame", return_value=SimpleNamespace(frame_bytes=b"frame")),
        ):
    ...
    with patch("apps.content.video.management.commands.video.Command._run_doctor") as doctor_mock:
...
def test_video_service_subaction_invokes_service_runner():
    ...
    with patch("apps.content.video.management.commands.video.Command._run_service") as service_mock:
...
def test_video_command_setting_defaults_preserve_zero():
    ...
    from apps.content.video.management.commands.video import _setting_default_float
