...
def test_drop_upload_rejects_oversized_file_before_sample_creation(
    admin_client,
    settings,
    tmp_path,
):
    ...
    with patch("apps.content.admin.create_uploaded_content_sample") as create_sample:
        response = admin_client.post(
            reverse("admin:content_contentsample_drop_upload"),
            {
                "file": SimpleUploadedFile(
    ...
    assert response.status_code == 400
    assert response.json() == {"error": "File exceeds the allowed size."}
    create_sample.assert_not_called()
