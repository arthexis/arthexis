...
class AgentTerminal(Profile):
    """Terminal process definition assigned to users or security groups."""
    ...
    def assigned_to_any_user(cls):
        owner_filter = Q(user__isnull=False) | Q(group__user__isnull=False)
        return cls.objects.filter(owner_filter).distinct()
