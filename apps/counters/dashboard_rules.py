...
def _load_node_model():
    if not django_apps.is_installed("apps.nodes"):
        return None

    try:
        from apps.nodes.models import Node
    except ImportError:
        logger.exception("Unable to import node dashboard rule models")
        return None
...
def evaluate_cp_configuration_rules() -> dict[str, object] | None:
    ...
    configured = set(
        ChargerConfiguration.objects.filter(
            charger_identifier__in=charger_ids
...
def evaluate_cp_firmware_rules() -> dict[str, object] | None:
    ...
    missing = [
        identifier for identifier in charger_ids if identifier not in firmware_sources
    ]
    if missing:
        evcs_list = _format_evcs_list(missing)
        message = ngettext(
...
def evaluate_node_rules() -> dict[str, object]:
    Node = _load_node_model()
    if Node is None:
        return rule_failure(_("Node check failed: import err."))
...
def evaluate_email_profile_rules() -> dict[str, object]:
    if not django_apps.is_installed("apps.emails"):
...
def evaluate_user_story_assignment_rules() -> dict[str, object] | None:
    ...
    try:
        ...
        logger.exception("Unable to import user story dashboard rule models")
        return rule_failure(_("User story check failed: import err."))
