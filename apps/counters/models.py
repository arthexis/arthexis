...
class StoredCounter(CacheStoreMixin, Entity):
    ...
    def _content_type_for(cls, model_or_content_type):
        ...
        if isinstance(model_or_content_type, int):
            return model_or_content_type
    ...
    def get_cached_value(
        ...
        *,
        ...
        content_type_id = (
            content_type if isinstance(content_type, int) else content_type.pk
        )
        return super().get_cached_value(
            content_type_id, builder, force_refresh=force_refresh
        )
    ...
    def invalidate_model_cache(cls, model_or_content_type):
        ...
        cls.invalidate_cached_value(content_type_id)
...
class DashboardRule(StoredCounter):
    ...
    success_message = models.CharField(max_length=200, default=DEFAULT_SUCCESS_MESSAGE)
    ...
    def clean(self):
        ...
        if self.implementation == self.Implementation.PYTHON:
            ...
            if not (self.condition_source or "").strip():
                errors["condition"] = _("Provide a structured condition source.")
    ...
    def evaluate(self) -> dict[str, object]:
        if self.implementation == self.Implementation.PYTHON:
            ...
            try:
                ...
                logger.exception(
                    "Dashboard rule handler failed: %s", self.function_name
                )
                return rule_failure(_("Unable to evaluate dashboard rule."))

        if self.condition_requires_triage:
            message = self.failure_message or _(
                "Rule condition requires manual triage."
            )
...
def clear_dashboard_rule_cache(sender, instance: DashboardRule, **_kwargs):
    DashboardRule.invalidate_model_cache(instance.content_type_id)
