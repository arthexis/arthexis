...
if [[ -n "$file" ]]; then
  "$PYTHON_CMD" -m arthexis.scripts.resolve_sigils --file "$file"
elif [[ -n "$explicit_text" ]]; then
  "$PYTHON_CMD" -m arthexis.scripts.resolve_sigils --text "$explicit_text"
elif [[ ${#positional_text[@]} -gt 0 ]]; then
  "$PYTHON_CMD" -m arthexis.scripts.resolve_sigils --text "${positional_text[*]}"
else
  "$PYTHON_CMD" -m arthexis.scripts.resolve_sigils
fi
