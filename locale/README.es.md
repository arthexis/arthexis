...
Consulta el nuevo [artículo para desarrolladores](http://localhost:8888/articles/protoline-integration/) con los aspectos destacados del próximo lanzamiento y los detalles del despliegue.
...
- Compatible con el [Open Charge Point Protocol (OCPP) 1.6](https://www.openchargealliance.org/protocols/ocpp-16/) como sistema central. Las acciones soportadas se resumen a continuación.
  ...
  **Hoja de ruta OCPP.** Consulta el [cookbook de la hoja de ruta OCPP](docs/cookbooks/ocpp-roadmap.md) para conocer el trabajo planificado de los catálogos OCPP 1.6 y 2.1.
...
- Canales de actualización: la actualización automática está activada por defecto en el canal estable (revisión cada 24 horas y se aplica solo si la revisión coincide con el índice de paquetes). Cambia al canal inestable con `--unstable`/`--latest` para seguir las revisiones de la rama principal cada 10 minutos o usa `--fixed` para desactivar la automatización.
