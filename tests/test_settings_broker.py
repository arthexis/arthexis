...
def test_resolve_celery_broker_url_prefers_celery_broker_env(monkeypatch) -> None:
    """CELERY_BROKER_URL must override legacy BROKER_URL when both are set."""

    monkeypatch.setenv("CELERY_BROKER_URL", "redis://localhost:6379/1")
    monkeypatch.setenv("BROKER_URL", "redis://localhost:6379/9")

    assert resolve_celery_broker_url(node_role="watchtower") == "redis://localhost:6379/1"
