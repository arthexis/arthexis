...
def _run_static_hash(base_dir: Path, hash_script: Path, *, force: bool = False) -> dict:
    ...
    command = "\n".join(
        [
            ...
            '  echo "Expected staticfiles helper to be readable at $helper_path" >&2',
            "  exit 1",
            "fi",
            'source "$helper_path"',
            "if ! type arthexis_prepare_staticfiles_hash >/dev/null 2>&1; then",
            '  echo "Expected arthexis_prepare_staticfiles_hash to be defined after sourcing $helper_path" >&2',
            "  exit 1",
            "fi",
