...
_SQLITE_PATH_SOURCES: dict[str, str] = {}


def sqlite_worker_suffix() -> str:
...
def set_writable_sqlite_env(var_name: str, fallback: Path) -> None:
    ...
    _SQLITE_PATH_SOURCES[var_name] = "generated"
...
def sqlite_env_summary() -> dict[str, dict[str, str]]:
    """Return selected SQLite paths and whether callers or pytest chose them."""

    return {
        var_name: {
            "path": os.environ.get(var_name, ""),
            "source": _SQLITE_PATH_SOURCES.get(var_name, "unknown"),
        }
        for var_name in ("ARTHEXIS_SQLITE_PATH", "ARTHEXIS_SQLITE_TEST_PATH")
    }


def sqlite_env_summary_lines() -> list[str]:
    """Return concise human-readable SQLite path diagnostics."""

    return [
        f"{var_name}={details['path']} ({details['source']})"
        for var_name, details in sqlite_env_summary().items()
    ]
