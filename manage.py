...
def _is_terminal_node(base_dir: Path) -> bool:
    """Return whether the current node role resolves to ``Terminal``."""

    explicit_role = os.environ.get("NODE_ROLE", "").strip()
    if explicit_role:
        return explicit_role.lower() == "terminal"

    role_lock = base_dir / ".locks" / "role.lck"
    try:
        lock_role = role_lock.read_text(encoding="utf-8").strip()
    except (OSError, UnicodeDecodeError):
        return True
    if not lock_role:
        return True
    return lock_role.lower() == "terminal"
