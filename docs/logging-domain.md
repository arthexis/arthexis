...
* `ARTHEXIS_LOG_FORMAT` controls the formatter at runtime.
  ...
  * `json`: emits stable JSON keys for local tooling and one-off diagnostics.
...
* Use low-cardinality keys such as `level`, `logger`, `app`, and `hostname` for broad filtering.
* Use higher-cardinality correlation keys (`request_id`, `node_id`, `charger_id`, `session_id`) for targeted local investigations.
* Keep ad hoc parsing scripts tolerant of missing keys so older log files remain readable.
