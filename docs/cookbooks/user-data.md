...
2. Select **User Data** from the header shortcuts (the button routes to the `admin:user_data` view defined in [`core/user_data.py`](../../core/user_data.py)).
3. The data list view renders the template [`core/templates/admin/data_list.html`](../../core/templates/admin/data_list.html), providing a searchable table of exported fixtures grouped by app and model.
...
Click **Export** above the table to download a ZIP archive that contains every recorded user datum. The archive is generated by `_user_data_export` in [`core/user_data.py`](../../core/user_data.py) and includes one JSON fixture per object.

- The exported filename follows the pattern `user_data_<user-id>.zip` so you can track who generated the bundle.
- Fixtures are sourced from the per-user directory created by `_data_dir` in [`core/user_data.py`](../../core/user_data.py).
...
3. Submit the form to invoke `_user_data_import` in [`core/user_data.py`](../../core/user_data.py).

The import process loads each fixture and, when available, re-applies the `is_user_data` flag (see `_load_fixture` in [`core/user_data.py`](../../core/user_data.py)). Successful imports redirect back to the list view with the updated objects.
