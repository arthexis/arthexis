...
The commands use the same resolver grammar as the rest of the suite. In
addition to explicit fallback syntax such as `[FIRST;SECOND]`, a sigil branch
with exactly two clean operands can use whitespace shorthand:

- `[CP.PORT MAX]` and `[MAX CP.PORT]` apply the `MAX` pipeline action to
  `CP.PORT` when pipeline v2 is enabled.
- `[CP.TOTAL_KW CP.KW]` behaves like `[CP.TOTAL_KW;CP.KW]` when neither operand
  is a pipeline action.

The joined identifier lookup is always tried before whitespace becomes an
operator, so `[CP.TOTAL_KW MAX]` first tries `[CP.TOTAL_KW_MAX]`. Branches with
other explicit operators, quoted selectors, nested sigils, more than two
operands, or a non-identifier operand such as `[A -B]` keep the legacy parsing
path.
...
- `policy error: unsupported context \`viewer\``
