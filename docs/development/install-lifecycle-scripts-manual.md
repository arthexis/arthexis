...
| `--internal` | Forces the internal Nginx template (HTTP ports 80/8000/8080/8900). This is the default unless a role preset changes it. |
| `--public` | Enables the public HTTPS reverse proxy template while continuing to proxy to the backend on port 8888 unless overridden. |
...
| `--celery` | Forces Celery services on even if the preset would leave them disabled. Rarely needed because all presets already enable Celery. |
