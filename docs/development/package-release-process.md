...
- `.github/workflows/prepare-release.yml` prepares the reviewed `VERSION` change before publishing. It runs after a successful default-branch Release Simulator workflow when the open readiness report has no blockers, or by manual dispatch. Automatic runs first verify that the checked-out default branch still matches the simulator SHA, then the workflow applies `docs/development/versioning-maturity-policy.md`, writes the selected `VERSION` on a `release/prepare-vX.Y.Z` branch, and opens or updates a `Release vX.Y.Z` PR. Maintainers can dispatch it with a forced `patch`, `minor`, or `major` bump when the automatic file-path policy cannot infer a drastic interface change.
...
- Release tags are automatic release output. Do not create the publish tag by hand during the normal path; let the release workflow or `tag-from-version.yml` create `vVERSION` from the reviewed `VERSION` on `main`. A tag whose commit does not contain the same `VERSION` as the release is rejected before it can be pushed.
