...
Private repositories skip the CodeQL job unless the repository variable
`CODEQL_ENABLED_ON_PRIVATE` is set to `true`. This avoids failing pull requests
when GitHub code scanning is not available for the repository plan; enable the
variable after code scanning has been enabled in GitHub settings.
