...
- 2026-05-20: Use `.venv/bin/python manage.py test benchmark -- <pytest target>` for one-off suite timing reports. It writes a standalone JSON artifact under `work/test-benchmark-<run-id>.json` by default and does not require permanent test-result database storage.
