...
- `requirements.txt`: runtime-only packages for production installs and other execution environments that only need to run Arthexis.
- `.[dev]`: local debugging helpers such as `django-debug-toolbar`.
- `.[preview]`: browser automation packages for explicit local preview tooling.
