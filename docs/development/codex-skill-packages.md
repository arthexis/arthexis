...
```bash
python manage.py propose --prompt "Check system status"
...
### Codex Feature Freeze

Device-local Codex installations may register a `before_prompt` hook that checks
the local freeze cache and the configured central freeze endpoint before Codex is
launched. Operators can inspect the local decision path with:

```bash
python manage.py codex_freeze status --json
python manage.py codex_freeze check --hook-json --no-refresh
python manage.py propose --prompt "Check system status" --dry-run --json
```

Central nodes can start, end, and monitor freezes with:

```bash
python manage.py codex_freeze start --reason "Token reserve below threshold" --until 2026-06-20T00:00:00Z
python manage.py codex_freeze monitor --json
python manage.py codex_freeze end --reason "Budget reset confirmed"
```

Emergency local bypasses are environment-only and should be limited to
authorized operator shells. Set `ARTHEXIS_CODEX_FREEZE_BYPASS_UNTIL` to a UTC
timestamp and `ARTHEXIS_CODEX_FREEZE_BYPASS_REASON` to the incident reason; the
hook allows prompts while the bypass is current and emits an explicit warning.
...
The suite also carries a source package for core suite operation skills:
