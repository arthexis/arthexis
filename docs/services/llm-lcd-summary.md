# Deterministic LCD Summary

The deterministic LCD summary service writes dense log-summary frames for the
low-priority LCD channel on Control nodes. It builds on the suite summary
pipeline and keeps the generated frames in `.locks/lcd-low`, `.locks/lcd-low-1`,
and later rotation files.
