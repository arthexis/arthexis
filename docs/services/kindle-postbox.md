...
Use the curated operators manual when a person needs to read the field handbook
on a Kindle. It is generated from `docs/operators-manual.json`, keeps the source
order intentional, and renders Markdown into plain text for easier reading on
Kindle screens.

Use the full suite documentation bundle when a maintainer needs exhaustive
offline reference material. It includes broad documentation roots and is useful
for search and recovery, but it is not curated for a linear reading path.
