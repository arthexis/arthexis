...
## What it is
The Suite service is the primary systemd unit that runs the Arthexis software suite web application (Django/ASGI) on a node. It is the anchor service that other companion units (Celery, LCD, RFID) for that node can depend on.
