...
- **Structured logging**: standardize log formats for migration runs (migration name, duration, affected rows). Use `.venv/bin/python manage.py migrations benchmark` for one-off migration planning or explicit `--apply` timing reports, and emit metrics for successes/failures to detect systemic patterns.
