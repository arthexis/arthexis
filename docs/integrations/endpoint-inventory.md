...
| `POST /nodes/network/chargers/import/` | System integration | Import signed charger state/transactions from a trusted node | Complete |
...
## 6) `apps.odoo` (external ERP integration)
...
## 7) Remaining app HTTP surfaces

Additional retained app URL surfaces exist (core, links, docs, features, widgets, tasks, etc.) and are tracked in the completeness checklist page. They are currently marked mostly `Missing` until endpoint-level contracts are documented with examples and auth semantics.
