...
6. Launch embedded LCD only when orchestration says embedded mode is required;
   ...
   ownership.
7. Launch the Django server on the resolved bind host and port, using
   `--noreload` unless `--reload` was requested. The default bind host is
   `127.0.0.1`; Satellite and Control roles default to `0.0.0.0` unless
   `.locks/charger_facing_disabled.lck` is present. Explicit charger-facing
   locks (`.locks/charger_facing.lck` or `.locks/ocpp_gateway.lck`) also default
   to `0.0.0.0` so local appliance clients such as OCPP chargers can reconnect
   through either the charger AP address or another node LAN address.
   `ARTHEXIS_RUNSERVER_HOST` overrides the lock default and must contain only
   the host, not a port.
