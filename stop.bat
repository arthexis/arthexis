...
set "ARG=%~1"
if "%ARG:~0,1%"=="-" (
    echo Unsupported stop.bat option: %~1
    call :usage
    set EXIT_CODE=2
    goto cleanup
)
...
if not defined PORT_SET if exist %LOCK_DIR%\backend_port.lck (
...
if defined ALL (
    ...
    call :kill_by_commandline "manage.py runserver --ipv6 [::1]:%PORT%"
    call :kill_by_commandline "manage.py runserver -6 [::1]:%PORT%"
)
...
:usage
echo Usage: stop.bat [PORT] [--all] [--force] [--confirm] [--help]
echo.
echo Stops managed Arthexis services or local development processes.
echo.
echo Arguments:
echo   PORT       Stop the server associated with this port. Defaults to the detected backend port.
echo.
echo Options:
echo   --all      Stop all matching runserver instances.
echo   --force    Force stop path for guarded conditions.
echo   --confirm  Confirm stop flow in guarded conditions.
echo   -h, --help Show this help and exit without stopping services.
exit /b 0
