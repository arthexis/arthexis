...
usage() {
    echo "Usage: $0 [--service NAME] [--port PORT] [--latest|--unstable|--stable|--lts|--regular|--normal] [--fixed] [--check] [--auto-upgrade|--no-auto-upgrade] [--debug|--no-debug] [--celery|--no-celery] [--lcd-screen|--no-lcd-screen] [--rfid-service|--no-rfid-service] [--camera-service|--no-camera-service] [--imager-burner-service|--no-imager-burner-service] [--boot-upgrade|--no-boot-upgrade] [--feature SLUG [--kind suite|node] [--enabled|--disabled]] [--feature-param FEATURE:KEY=VALUE] [--email ADMIN_EMAIL] [--satellite|--terminal|--control|--watchtower] [--repair [--failover ROLE]]" >&2
    exit 1
}
...
apply_imager_burner_service_setting() {
    local mode="$1"
    local lock_dir="$2"
    local base_dir="$3"
    local service_name="$4"
    ...
    mkdir -p "$lock_dir"
    if [ "$mode" = "enable" ]; then
        if [ -z "$service_name" ] || ! arthexis_using_systemd_mode "$lock_dir"; then
            echo "Imager burner service requires a configured systemd service." >&2
            exit 1
        fi
        arthexis_install_imager_burner_service_unit "$base_dir" "$lock_dir" "$service_name"
        arthexis_start_systemd_unit_if_present "$ARTHEXIS_IMAGER_BURNER_SERVICE_UNIT"
        echo "Imager burner service enabled."
    else
        arthexis_remove_systemd_unit_if_present "$lock_dir" "$ARTHEXIS_IMAGER_BURNER_SERVICE_UNIT"
        echo "Imager burner service disabled."
    fi
}
...
require_redis() {
    ...
    local redis_env_path="$SCRIPTS_DIR/redis.env"
    ...
    if [ -f "$redis_env_path" ]; then
        local celery_broker_url=""
        local channel_redis_url=""
        local ocpp_state_redis_url=""

        celery_broker_url="$(read_env_value "$redis_env_path" "CELERY_BROKER_URL")"
        channel_redis_url="$(read_env_value "$redis_env_path" "CHANNEL_REDIS_URL")"
        ocpp_state_redis_url="$(read_env_value "$redis_env_path" "OCPP_STATE_REDIS_URL")"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --imager-burner-service)
            if [ "$IMAGER_BURNER_SERVICE_MODE" = "disable" ]; then
                echo "Cannot combine --imager-burner-service with --no-imager-burner-service" >&2
                usage
            fi
            IMAGER_BURNER_SERVICE_MODE="enable"
            shift
            ;;
        --no-imager-burner-service)
            if [ "$IMAGER_BURNER_SERVICE_MODE" = "enable" ]; then
                echo "Cannot combine --imager-burner-service with --no-imager-burner-service" >&2
                usage
            fi
            IMAGER_BURNER_SERVICE_MODE="disable"
            shift
            ;;
        --boot-upgrade)
...
if [ -n "$IMAGER_BURNER_SERVICE_MODE" ] && [ -z "$NODE_ROLE" ]; then
    ACTION_PERFORMED=true
    if [ -z "$SERVICE" ] && [ -f "$LOCK_DIR/service.lck" ]; then
        SERVICE=$(cat "$LOCK_DIR/service.lck")
    fi
    apply_imager_burner_service_setting "$IMAGER_BURNER_SERVICE_MODE" "$LOCK_DIR" "$BASE_DIR" "$SERVICE"
fi
...
elif [ "$IMAGER_BURNER_SERVICE_MODE" = "disable" ]; then
    ENABLE_IMAGER_BURNER_SERVICE=false
elif [ "$NODE_ROLE" = "Control" ] && command -v lsblk >/dev/null 2>&1; then
    ENABLE_IMAGER_BURNER_SERVICE=true
elif [ "$NODE_ROLE" = "Control" ] && [ "$EXISTING_IMAGER_BURNER_SERVICE" = true ]; then
    ENABLE_IMAGER_BURNER_SERVICE=true
else
    ENABLE_IMAGER_BURNER_SERVICE=false
fi
...
if [ -n "$SERVICE" ] && arthexis_using_systemd_mode "$LOCK_DIR"; then
    ...
    if [ "$ENABLE_IMAGER_BURNER_SERVICE" = true ]; then
