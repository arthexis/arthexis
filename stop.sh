...
Stops managed Arthexis services or local development processes.

Arguments:
  PORT       Stop the server associated with this port. Defaults to the detected backend port.
...
Options:
  ...
  -h, --help Show this help and exit without stopping services.
EOF
}
  ...
  for arg in "$@"; do
    case "$arg" in
      ...
      --all|--force|--confirm)
        ;;
      --*|-*)
        echo "Unsupported stop.sh option: $arg" >&2
        usage >&2
        exit 2
        ;;
      *)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        ...
        ;;
      --*|-*)
        echo "Unsupported stop.sh option: $1" >&2
        usage >&2
        exit 2
        ;;
      *)
