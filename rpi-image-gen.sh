...
Options:
  ...
  --rebuild           Rebuild the image even if a cached artifact already exists.
...
if [[ "$SKIP_BUILD" == false ]]; then
  ...
  echo "Using cached image: $FINAL_IMAGE (use --rebuild to force a rebuild)."
fi

if [[ -z "$FINAL_IMAGE" ]]; then
  error "Unable to determine final image path."
fi
