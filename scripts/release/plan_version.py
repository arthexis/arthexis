...
def plan_release_version(
    *,
    ...
    impact_report = build_release_impact_report(
        changes,
        app_sets=app_sets,
        requested_level=bump_level,
    )
    required_bump = BumpLevel.from_name(impact_report.required_bump)
    reasons = impact_report.reasons
    ...
    return VersionPlan(
        ...
        change_count=len(changes),
        reasons=reasons,
        impact_report=impact_report.as_dict(),
...
def write_github_output(plan: VersionPlan, path: Path) -> None:
    ...
    with path.open("a", encoding="utf-8") as handle:
        ...
        handle.write(
            "impact_report_json="
            f"{json.dumps(plan.impact_report, separators=(',', ':'))}\n"
        )
