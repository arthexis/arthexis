...
SOURCE_URL_SERVICE_ARG=""
if [[ -n "$SOURCE_URL" ]]; then
    SOURCE_URL_SERVICE_ARG=" --source-url $SOURCE_URL"
fi
...
ExecStart=$BASE_DIR/.venv/bin/python $BASE_DIR/scripts/ap_portal_server.py --bind 127.0.0.1 --port $PORTAL_PORT --state-dir $STATE_DIR --trusted-macs-path $TRUSTED_MACS_PATH$SOURCE_URL_SERVICE_ARG --suite-login-host $DEFAULT_CERT_DOMAIN
Restart=always
RestartSec=2
