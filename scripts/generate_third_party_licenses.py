...
def build_inventory() -> list[dict[str, str]]:
    ...
    dependencies = {
        name.lower(): (name, spec) for name, spec in resolve_transitive_dependencies()
    }
...
def render_markdown(inventory: list[dict[str, str]]) -> str:
    header = textwrap.dedent("""
        ...
        The following third-party components are used at runtime. License information is collected from
        the Python Package Index (PyPI), and links point to the upstream license texts or project pages
        so operators can review dependency notice obligations.
