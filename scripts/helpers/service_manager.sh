...
ARTHEXIS_IMAGER_BURNER_SERVICE_UNIT="arthexis-imager-burner.service"
...
arthexis_service_unit_names() {
  ...
  local include_imager_burner="${6:-false}"
  ...
  if [ "$include_rfid" = true ]; then
    units+=("rfid-${service_name}.service")
  fi
  if [ "$include_camera" = true ]; then
    units+=("camera-${service_name}.service")
  fi
  if [ "$include_imager_burner" = true ]; then
    units+=("$ARTHEXIS_IMAGER_BURNER_SERVICE_UNIT")
  fi
...
arthexis_stop_service_unit_stack() {
  ...
  done < <(arthexis_service_unit_names "$service_name" "$include_celery" "$include_lcd" "$include_rfid" "$include_camera" "$include_imager_burner")
}
...
arthexis_remove_service_unit_stack() {
  ...
  local include_imager_burner="${7:-false}"
