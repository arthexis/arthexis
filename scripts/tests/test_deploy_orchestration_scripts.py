...
WATCH_UPGRADE_PATH = (
    Path(__file__).resolve().parents[2] / "scripts/helpers/watch-upgrade.sh"
)
ORCHESTRATOR_PATH = (
...
def test_orchestrator_runs_migrations_before_service_switch() -> None:
    ...
    assert 'queue_post_upgrade_tasks "$python_bin"' in contents
    ...
    assert contents.index('queue_post_upgrade_tasks "$python_bin"') < contents.index(
...
def test_orchestrator_emits_structured_timestamps_and_marker() -> None:
    ...
    assert (
        'MIGRATION_MARKER_FILE="${LOCK_DIR}/predeploy_migrate_success.json"' in contents
    )
    assert '"status": "success"' in contents


def test_orchestrator_queues_deferred_node_migration_task_best_effort() -> None:
    """The orchestrator should enqueue deferred node migration work without blocking deploy."""
    contents = ORCHESTRATOR_PATH.read_text(encoding="utf-8")
    assert "queue_post_upgrade_tasks()" in contents
    assert "run_deferred_node_migrations" in contents
    assert "continuing deploy" in contents
    assert 'log_event "post_upgrade_task_enqueue" "success"' in contents
    assert 'log_event "post_upgrade_task_enqueue" "failed"' in contents
