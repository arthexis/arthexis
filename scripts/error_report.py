...
def run_binary_command(args: list[str], *, cwd: Path, timeout: int) -> tuple[int, str]:
    """Run a command and return redacted decoded output."""
    ...
    try:
        ...
        return (127, _("Command not available: {command}").format(command=args[0]))
    except subprocess.TimeoutExpired:
        return (
            124,
            _("Command timed out after {timeout}s: {command}").format(
